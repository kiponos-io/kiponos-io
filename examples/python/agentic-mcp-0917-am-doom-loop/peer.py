...
Hub: examples/agentic-mcp-0917-am-doom-loop/retry-max (default 2)
...
KEY = "retry-max"
DEFAULT = "2"
PATH = "examples/agentic-mcp-0917-am-doom-loop/retry-max"
...
def decide(value: str | None) -> dict:
    ...
    try:
        ...
        cap = 2
    proceed = cap > 0
    return {"path": PATH, "key": KEY, "value": str(cap), "action": "retry_within_cap" if proceed else "doom_loop_stopped", "proceed": proceed, "peers": PEERS}
...
def _live(d: dict) -> None:
    ...
    try:
        k.ensure_path("examples/agentic-mcp-0917-am-doom-loop")
