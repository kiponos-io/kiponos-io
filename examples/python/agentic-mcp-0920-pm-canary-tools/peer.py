...
Hub: examples/agentic-mcp-0920-pm-canary-tools/canary-percent (default 0)
...
KEY = "canary-percent"
DEFAULT = "0"
PATH = "examples/agentic-mcp-0920-pm-canary-tools/canary-percent"
...
def decide(value: str | None) -> dict:
    ...
    try:
        ...
        cap = 0
    proceed = cap > 0
    return {"path": PATH, "key": KEY, "value": str(cap), "action": "canary_tools_live" if proceed else "canary_tools_off", "proceed": proceed, "peers": PEERS}
...
def _live(d: dict) -> None:
    ...
    try:
        k.ensure_path("examples/agentic-mcp-0920-pm-canary-tools")
