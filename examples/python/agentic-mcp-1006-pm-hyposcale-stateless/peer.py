...
Hub: examples/agentic-mcp-1006-pm-hyposcale-stateless/shared-truth (default live)
...
KEY = "shared-truth"
DEFAULT = "live"
PATH = "examples/agentic-mcp-1006-pm-hyposcale-stateless/shared-truth"
...
def decide(value: str | None) -> dict:
    v = normalize(value)
    live = v.lower() in ("live", "yes", "on")
    return {"path": PATH, "key": KEY, "value": v, "action": "peers_share_live_hub" if live else "refuse_stale_host_argv", "proceed": live, "peers": PEERS}
...
def _live(d: dict) -> None:
    ...
    try:
        k.ensure_path("examples/agentic-mcp-1006-pm-hyposcale-stateless")
