...
class StateLiveOrderGoldenTest {
    ...
    void connectEnsureAndRun() {
        ...
        try {
            ...
            Folder policy = StateLiveOrderApp.ensureStateFolder(kiponos);
            assertNotNull(policy);
            assertTrue(policy.hasKey("current"));
            var r = StateLiveOrderApp.tryTransition(policy, policy.get("current"));
            assertTrue(r.ok());
            System.out.println("GOLDEN_OK StateLiveOrderApp");
        } finally {
            kiponos.disconnect();
        }
    }
