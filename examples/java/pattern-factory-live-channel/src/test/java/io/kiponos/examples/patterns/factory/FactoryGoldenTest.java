...
class FactoryLiveChannelGoldenTest {
    ...
    void connectEnsureAndRun() {
        ...
        try {
            ...
            Folder policy = FactoryLiveChannelApp.ensureFactoryFolder(kiponos);
            assertNotNull(policy);
            assertTrue(policy.hasKey("product"));
            var n = FactoryLiveChannelApp.createNotifier(policy);
            assertNotNull(n.channel());
            assertFalse(n.send("ping").isBlank());
            System.out.println("GOLDEN_OK FactoryLiveChannelApp");
        } finally {
            kiponos.disconnect();
        }
    }
