# agentic-med-0921-af-session-posture

I Restarted Cursor to Flip Session posture shared across hosts — The Mirror Phone Wall Already Knew
...
```text
examples/agentic-med-0921-af-session-posture/session-posture = focus=admin-wall,shopping-pause=off
```
...
| SDK | Path | How to run |
|-----|------|------------|
| Java | `examples/java/agentic-med-0921-af-session-posture` | `./gradlew test run` |
| Python | `examples/python/agentic-med-0921-af-session-posture` | `python3 -m pytest -q && python3 peer.py` |
| React (Node BFF) | `examples/node/agentic-med-0921-af-session-posture-react` | `npm install && npm test && node peer.mjs` |
| Angular (Node BFF) | `examples/node/agentic-med-0921-af-session-posture-angular` | `npm install && npm test && node peer.mjs` |
...
```bash
cd examples/node/agentic-med-0921-af-session-posture-angular
npm install
npm test
node peer.mjs
