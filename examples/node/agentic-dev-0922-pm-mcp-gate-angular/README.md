...
Cursor Finished the Turn Blind — MCP write tool gated live on the Travel Path
...
| SDK | Path | How to run |
|-----|------|------------|
| Java | `examples/java/agentic-dev-0922-pm-mcp-gate` | `./gradlew test run` |
| Python | `examples/python/agentic-dev-0922-pm-mcp-gate` | `python3 -m pytest -q && python3 peer.py` |
| React (Node BFF) | `examples/node/agentic-dev-0922-pm-mcp-gate-react` | `npm install && npm test && node peer.mjs` |
| Angular (Node BFF) | `examples/node/agentic-dev-0922-pm-mcp-gate-angular` | `npm install && npm test && node peer.mjs` |
...
```bash
cd examples/node/agentic-dev-0922-pm-mcp-gate-angular
npm install
npm test
node peer.mjs
