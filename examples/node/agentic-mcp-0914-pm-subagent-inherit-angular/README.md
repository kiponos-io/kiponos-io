# agentic-mcp-0914-pm-subagent-inherit

The Subagent Spawned Without the Parent's MCP Allow-List

**Product scene:** shopping  
**Agent host:** MCP host  
**Pain:** Subagent spawned with yesterday's tools-allow

Hub leaf (same Team tree for every SDK):

```text
examples/agentic-mcp-0914-pm-subagent-inherit/inherit-posture = on
```
...
| SDK | Path | How to run |
|-----|------|------------|
| Java | `examples/java/agentic-mcp-0914-pm-subagent-inherit` | `./gradlew test run` |
| Python | `examples/python/agentic-mcp-0914-pm-subagent-inherit` | `python3 -m pytest -q && python3 peer.py` |
| React (Node BFF) | `examples/node/agentic-mcp-0914-pm-subagent-inherit-react` | `npm install && npm test && node peer.mjs` |
| Angular (Node BFF) | `examples/node/agentic-mcp-0914-pm-subagent-inherit-angular` | `npm install && npm test && node peer.mjs` |
...
```bash
cd examples/node/agentic-mcp-0914-pm-subagent-inherit-angular
npm install
npm test
node peer.mjs
