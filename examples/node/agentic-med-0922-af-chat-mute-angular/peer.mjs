 ...
 * angular Node server peer — @kiponos/angular/server
...
const server = createServer((req, res) => {
  ...
  if (req.url === "/posture" || req.url === "/") {
    ...
    res.end(JSON.stringify({ ...d, sdk: "angular-node-bff" }));
    return;
  }
  res.writeHead(404);
  res.end();
...
server.listen(port, "127.0.0.1", () => {
  ...
  console.log("angular BFF listening", addr, "leaf examples/" + FOLDER + "/" + KEY);
