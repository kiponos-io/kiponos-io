...
export function resolveCredentialsFromEnv(
  ...
  if (isBrowserRuntime() || !isNodeRuntime()) {
    throw new Error(
      "[@kiponos/react] createFromEnv is Node/server-only. " +
  ...
  if (!idToken || !accessToken) {
    throw new Error(
      "[@kiponos/react] Missing KIPONOS_ID and/or KIPONOS_ACCESS in process environment. " +
  ...
  if (!profile) {
    throw new Error(
      "[@kiponos/react] Invalid or missing KIPONOS profile. " +
