---
main_image: https://iili.io/CprApVf.jpg
title: "MCP host Finished the Turn Blind — Shopping incident pause as a leaf on the Mirror Phone Path"
published: false
tags: java, python, devops, ai, kiponos
description: "MCP host kept finishing the turn blind on mirror-phone wall. Shopping incident pause as a leaf is a live hub leaf."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-dev-0911-am-shopping-pause.md
---

I have sat next to the mirror-phone wall at 00:40 while MCP host was *this close* to doing the wrong thing.

Not a model failure. A **posture** failure.

The host had started with `incident-pause` frozen in env / argv / a skill file. Someone on the floor said, out loud, **the phone already knew the posture — the host did not**. MCP host was still holding the old process. The only “safe” move anyone trusted was:
...
The mirror-phone wall already *knew*. MCP host did not, because it had started earlier.
...
| Restart MCP host — it is cheap | Cheap until 00:40 ate live device context and the current tool call |
...
Kiponos holds a nested tree. Java and Python SDKs keep the latest values **in memory**, patched over WebSocket deltas. The hot path inside a MCP host tool is a **local get** — no HTTP RTT per mirror phone lookup.

Hub leaf for this essay:

```text
examples/agentic-dev-0911-am-shopping-pause/incident-pause = paused
```

Runnable proof: [`examples/java/agentic-dev-0911-am-shopping-pause`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-dev-0911-am-shopping-pause)
...
```yaml
examples/
  agentic-dev-0911-am-shopping-pause/
...
Folder gate = kip.getRootFolder()
        ...
        .folderOrCreate("agentic-dev-0911-am-shopping-pause");
...
try:
    posture = k.get("examples/agentic-dev-0911-am-shopping-pause/incident-pause", "paused")
...
|-------|-----------------|--------------|
| Device leaf gone stale while the agent is mid-turn | Restart MCP host; lose live device context and the current tool call | Set `incident-pause` live; next MCP host tool call already obeys |
| Peer host still on old incident-pause | Paste the value into the other chat | One hub leaf; both processes `get()` locally |
| mirror-phone wall shows the new posture | MCP host started earlier so it writes anyway | Dashboard and tool share the same memory tree |
| Incident over, resume | Another MCP host restart | Set `incident-pause` back; session continues |
...
In staging: set a painful `incident-pause`, prove MCP host recovers without a host kill, prove clamps reject nonsense, prove last-known-good when the hub is firewalled. That drill ends half the architecture arguments about Shopping incident pause as a leaf.

## Why MCP host is the wrong restart target

MCP host is good at calling tools. It is not a control plane. Killing it to flip `incident-pause` teaches the on-call that judgment requires a process ID. The mirror-phone wall already disagrees.

## What the mirror phone operator actually said

At 00:40 someone said, out loud: **the phone already knew the posture — the host did not**. That sentence is the whole product. If it cannot land in the running MCP host process in seconds, you do not have posture. You have a wiki.
...
3. `cd examples/java/agentic-dev-0911-am-shopping-pause && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-dev-0911-am-shopping-pause/incident-pause=...`
5. In the dashboard, change `incident-pause`. Keep the process up. No rebuild.
6. Point your MCP host tool at the same leaf. Do not ship a new server binary to flip Shopping incident pause as a leaf.
...
If flipping **Shopping incident pause as a leaf** requires restarting MCP host, you do not have a gate. You have a hope with a process ID.
...
How to try: `examples/java/agentic-dev-0911-am-shopping-pause` and `./gradlew test`.
