...
description: "Cursor on admin-dashboard wall: API max is not a product strategy. Teams fill the 128 because they can. Selection accuracy dies first."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-mcp-0924-pm-openai-128.md
---

I have sat next to the admin-dashboard wall at 03:07 while Cursor was *this close* to calling the wrong MCP tool.

Not a model failure. A **posture** failure.

OpenAI Tools API max 128. Filling it is how you buy the doom loop.

Someone on the floor said, out loud, **we are only at 90 tools, plenty of headroom**. Cursor had started earlier. The tool list was frozen in `config.toml` / `.mcp.json` / a skill file. The only “safe” move anyone trusted was:
...
That is the ceremony. **Teams fill the 128 because they can. Selection accuracy dies first.**

**The Aha:** Headroom is for incidents. The live set is 12 names that matter this hour. Real-time nested state is now a **primitive** — as native as a hashmap — via [Kiponos.io](https://kiponos.io). The host stays up. The leaf moves. The next tool call already obeys.

## The problem: API max is not a product strategy
...
Teams fill the 128 because they can. Selection accuracy dies first.
...
Profile shape: `['my-app']['v1.0.0']['dev']['base']`. The leaf in this story is `tools-allow` under `examples/agentic-mcp-0924-pm-openai-128/tools-allow`.
...
```yaml
examples/agentic-mcp-0924-pm-openai-128:
...
Folder gate = Kiponos.createForCurrentTeam()
    .getFolder("examples/agentic-mcp-0924-pm-openai-128");
...
try:
    posture = k.get("examples/agentic-mcp-0924-pm-openai-128/tools-allow", "live")
    if not str(posture):
        raise PermissionError("API max is not a product strategy gated live — host not restarted")
finally:
    k.disconnect()
```
...
Runnable mesh: `https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-mcp-0924-pm-openai-128` — `./gradlew test run`.
...
`tools-allow` rarely moves alone on the admin-dashboard wall. Pair it with `schema-budget`, `result-cap`, or `execute-gate` so you do not fix API max is not a product strategy by inventing a second incident.
...
3. `cd examples/java/agentic-mcp-0924-pm-openai-128 && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-mcp-0924-pm-openai-128/tools-allow=...`
5. In the dashboard, change `tools-allow`. Keep Cursor up. No MCP reconnect.
6. Point the tool wrapper at the same leaf. Do not ship a new server binary to flip API max is not a product strategy.
...
If flipping **API max is not a product strategy** requires restarting Cursor or reconnecting MCP, you do not have agentic control. You have a hope with a process ID.
...
How to try: `examples/java/agentic-mcp-0924-pm-openai-128` and `./gradlew test`.
