---
title: "We Stopped Shipping Rag Top-K as a Constant (Cdn)"
published: false
...
Domain: edge rules, cache TTL, purge. This essay maps hub key `topK` to **RAG top-k** so the lesson stays concrete for cdn operators.
...
```java
Folder policy = kiponos.path("examples", "ops-cdn-rag-topk");
...
1. Name the hub path so humans find it under pressure (`examples/ops-cdn-rag-topk/topK`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: RAG top-k for cdn — numbers humans already change verbally in war rooms.
