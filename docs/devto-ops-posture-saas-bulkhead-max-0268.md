---
title: "The Saas War Room Already Knew bulkheadMax — The Jar Did Not"
published: false
tags: java, saas, architecture, kiponos
description: "Live bulkhead concurrency via Kiponos Java SDK — saas posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-saas-bulkhead-max-0268.md
main_image: ./devto-cover-ops-posture-saas-bulkhead-max-0268.jpg
---
...
Domain: multi-tenant quotas, feature posture. This essay maps hub key `bulkheadMax` to **bulkhead concurrency** so the lesson stays concrete for saas operators.
...
```java
var policy = kiponos.path("examples", "ops-saas-bulkhead-max");
String mode = policy.getString("degradeMode", "full");
int bulkheadMax = policy.getInt("bulkheadMax");
return router.decide(mode, bulkheadMax);
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-saas-bulkhead-max/bulkheadMax`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: bulkhead concurrency for saas — numbers humans already change verbally in war rooms.
