...
CI pipelines does not wait for your change-management window. Your `parallelism` did — until it lived in a hub.

## The problem: ceremony between judgment and effect

When **CI test parallelism** is frozen in a deploy unit, every incident becomes a process argument. Hotfixes still mean CI + roll. Feature flags often mean a second control plane with its own delay.
...
```java
Folder policy = kiponos.path("examples", "ci-test-tuning");
int parallelism = policy.getInt("parallelism"); // local get — no hub RTT
if (parallelism <= 0) {
    return failClosed();
}
...
Ops (or automation) sets `parallelism` in the hub. The **next** evaluation uses it. Same binary. Same tests for structure.
...
- Canonical source: [this article on GitHub](https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ci-test-tuning.md)
...
Feature flags are often product gates. This essay is about **ops posture** on CI pipelines: **CI test parallelism** — numbers war rooms already shout.
...
Treat `parallelism` as a slice of error budget. Raise when the world is healthy; lower when dependencies are sick. Write the number that survives a bad day.
...
**CI test parallelism** that requires a deploy is optimistic documentation.
