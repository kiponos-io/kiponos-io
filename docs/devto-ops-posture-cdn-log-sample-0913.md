---
title: "The Cdn War Room Already Knew logSamplePerThousand — The Jar Did Not"
published: false
...
Domain: edge rules, cache TTL, purge. This essay maps hub key `logSamplePerThousand` to **log sample per thousand** so the lesson stays concrete for cdn operators.
...
```java
var policy = kiponos.path("examples", "ops-cdn-log-sample");
...
1. Name the hub path so humans find it under pressure (`examples/ops-cdn-log-sample/logSamplePerThousand`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: log sample per thousand for cdn — numbers humans already change verbally in war rooms.
