---
title: "Inference Batch Size Should Not Wait for a Redeploy — Energy Live Posture"
published: false
tags: java, energy, architecture, kiponos
description: "Live inference batch size via Kiponos Python SDK — energy posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-energy-inference-batch-0585.md
main_image: https://files.catbox.moe/kmjeyk.jpg
---
...
Domain: grid dispatch, demand response. This essay maps hub key `batchSize` to **inference batch size** so the lesson stays concrete for energy operators.
...
```python
policy = kiponos.path("examples", "ops-energy-inference-batch")
...
1. Name the hub path so humans find it under pressure (`examples/ops-energy-inference-batch/batchSize`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: inference batch size for energy — numbers humans already change verbally in war rooms.
