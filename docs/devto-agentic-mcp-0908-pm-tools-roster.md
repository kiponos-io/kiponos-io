...
description: "Cursor on shopping-admin wall: Live tool roster instead of reconnecting servers. Quality drops past ~50 tools; Cursor hard-caps 40; OpenAI Tools API 128"
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-mcp-0908-pm-tools-roster.md
---

I have sat next to the shopping-admin wall at 11:41 while Cursor was *this close* to calling the wrong MCP tool.

Not a model failure. A **posture** failure.

Hajdas 2026: output degrades past 50 always-loaded tools. A safe ceiling is 30–40 with deferred loading.

Someone on the floor said, out loud, **uninstall the server — it is the only way to hide the tool**. Cursor had started earlier. The tool list was frozen in `config.toml` / `.mcp.json` / a skill file. The only “safe” move anyone trusted was:
...
That is the ceremony. **Quality drops past ~50 tools; Cursor hard-caps 40; OpenAI Tools API 128**

**The Aha:** The roster is `tools-allow`. Flip it live. The host stays up. Real-time nested state is now a **primitive** — as native as a hashmap — via [Kiponos.io](https://kiponos.io). The host stays up. The leaf moves. The next tool call already obeys.

## The problem: Live tool roster instead of reconnecting servers
...
Quality drops past ~50 tools; Cursor hard-caps 40; OpenAI Tools API 128
...
Profile shape: `['my-app']['v1.0.0']['dev']['base']`. The leaf in this story is `tools-allow` under `examples/agentic-mcp-0908-pm-tools-roster/tools-allow`.

## Architecture

![Architecture diagram](https://iili.io/n3JoRIa.png)
...
```yaml
examples/agentic-mcp-0908-pm-tools-roster:
...
Folder gate = Kiponos.createForCurrentTeam()
    .getFolder("examples/agentic-mcp-0908-pm-tools-roster");
...
try:
    posture = k.get("examples/agentic-mcp-0908-pm-tools-roster/tools-allow", "live")
    if not str(posture):
        raise PermissionError("Live tool roster instead of reconnecting servers gated live — host not restarted")
finally:
    k.disconnect()
```
...
Runnable mesh: `https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-mcp-0908-pm-tools-roster` — `./gradlew test run`.
...
`tools-allow` rarely moves alone on the shopping-admin wall. Pair it with `schema-budget`, `result-cap`, or `execute-gate` so you do not fix Live tool roster instead of reconnecting servers by inventing a second incident.

## Why Cursor is the wrong restart target

Cursor is good at calling tools. It is not a control plane. Killing it to flip `tools-allow` teaches the on-call that judgment requires a process ID. The shopping-admin wall already disagrees.
...
3. `cd examples/java/agentic-mcp-0908-pm-tools-roster && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-mcp-0908-pm-tools-roster/tools-allow=...`
5. In the dashboard, change `tools-allow`. Keep Cursor up. No MCP reconnect.
6. Point the tool wrapper at the same leaf. Do not ship a new server binary to flip Live tool roster instead of reconnecting servers.
...
If flipping **Live tool roster instead of reconnecting servers** requires restarting Cursor or reconnecting MCP, you do not have agentic control. You have a hope with a process ID.
...
How to try: `examples/java/agentic-mcp-0908-pm-tools-roster` and `./gradlew test`.
