---
title: "Ecommerce: Turn per-tenant quota Into Incident Posture"
published: false
...
Domain: cart, promo weights, stock. This essay maps hub key `quotaPerMin` to **per-tenant quota** so the lesson stays concrete for ecommerce operators.
...
```java
Folder policy = kiponos.path("examples", "ops-ecommerce-tenant-quota");
...
1. Name the hub path so humans find it under pressure (`examples/ops-ecommerce-tenant-quota/quotaPerMin`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: per-tenant quota for ecommerce — numbers humans already change verbally in war rooms.
