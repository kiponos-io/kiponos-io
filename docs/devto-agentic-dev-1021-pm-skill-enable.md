---
main_image: https://iili.io/nHnYq4n.jpg
title: "Claude Code Finished the Turn Blind — Skill enable set without restart on the Shopping Path"
published: false
tags: java, python, devops, ai, kiponos
description: "Claude Code kept finishing the turn blind on shopping-admin wall. Skill enable set without restart is a live hub leaf."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-dev-1021-pm-skill-enable.md
---

I have sat next to the shopping-admin wall at 21:03 while Claude Code was *this close* to doing the wrong thing.

Not a model failure. A **posture** failure.

The host had started with `enabled-set` frozen in env / argv / a skill file. Someone on the floor said, out loud, **disable inventory writes — keep search**. Claude Code was still holding the old process. The only “safe” move anyone trusted was:
...
| Restart Claude Code — it is cheap | Cheap until 21:03 ate cart forensics and the open SKU thread |
...
```text
examples/agentic-dev-1021-pm-skill-enable/enabled-set = live
```

Runnable proof: [`examples/java/agentic-dev-1021-pm-skill-enable`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-dev-1021-pm-skill-enable)
...
```yaml
examples/
  agentic-dev-1021-pm-skill-enable/
...
Folder gate = kip.getRootFolder()
        ...
        .folderOrCreate("agentic-dev-1021-pm-skill-enable");
...
try:
    posture = k.get("examples/agentic-dev-1021-pm-skill-enable/enabled-set", "live")
    if str(posture) == "live":
...
|-------|-----------------|--------------|
| Flash-freeze on sku writes | Restart Claude Code; lose cart forensics and the open SKU thread | Set `enabled-set` live; next Claude Code tool call already obeys |
...
| Incident over, resume | Another Claude Code restart | Set `enabled-set` back; session continues |
...
In staging: set a painful `enabled-set`, prove Claude Code recovers without a host kill, prove clamps reject nonsense, prove last-known-good when the hub is firewalled. That drill ends half the architecture arguments about Skill enable set without restart.

## Why Claude Code is the wrong restart target

Claude Code is good at calling tools. It is not a control plane. Killing it to flip `enabled-set` teaches the on-call that judgment requires a process ID. The shopping-admin wall already disagrees.

## What the shopping operator actually said

At 21:03 someone said, out loud: **disable inventory writes — keep search**. That sentence is the whole product. If it cannot land in the running Claude Code process in seconds, you do not have posture. You have a wiki.
...
3. `cd examples/java/agentic-dev-1021-pm-skill-enable && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-dev-1021-pm-skill-enable/enabled-set=...`
5. In the dashboard, change `enabled-set`. Keep the process up. No rebuild.
6. Point your Claude Code tool at the same leaf. Do not ship a new server binary to flip Skill enable set without restart.
...
If flipping **Skill enable set without restart** requires restarting Claude Code, you do not have a gate. You have a hope with a process ID.
...
How to try: `examples/java/agentic-dev-1021-pm-skill-enable` and `./gradlew test`.
