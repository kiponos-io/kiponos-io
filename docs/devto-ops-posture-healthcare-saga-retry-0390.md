---
title: "Saga Step Retry Budget Should Not Wait for a Redeploy — Healthcare Live Posture"
published: false
...
main_image: https://iili.io/Csz7QKg.jpg
---
...
Domain: routing, queue depth, triage. This essay maps hub key `sagaMaxAttempts` to **saga step retry budget** so the lesson stays concrete for healthcare operators.
...
```java
Folder policy = kiponos.path("examples", "ops-healthcare-saga-retry");
...
1. Name the hub path so humans find it under pressure (`examples/ops-healthcare-saga-retry/sagaMaxAttempts`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: saga step retry budget for healthcare — numbers humans already change verbally in war rooms.
