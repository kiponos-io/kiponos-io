---
title: "Production Debug Probes for Agents and Services — While They Run"
published: false
tags: java, python, kiponos, devops
description: "live verbose probe flag live across Java, Python, React, Angular and agents — no restart."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agent-debug-probe-live.md
main_image: https://iili.io/CSnvJKN.jpg
---

**The Aha:** live verbose probe flag is not a properties-file trophy. It is **mesh posture** — and posture that waits for four redeploys is already late.

The agent only misbehaved with production traffic. Restarting with DEBUG=1 made the bug vanish for an hour.

Someone said:

> Debug that requires a restart is not production debug. It is a different universe.

Domain: multi-SDK distributed systems, agents, MCP, observability, war-room truth. This essay maps hub key `verbose` under `examples/agent-debug-probe-live/` so the lesson stays concrete.

## The problem: ceremony between judgment and effect

You already know the right value for **live verbose probe flag**. The war room knows. The agent almost knows. What you do not have is a path from **mouth → every running peer** that is shorter than a release train and safer than pasting tokens into a SPA.

When `verbose` is frozen in YAML (or baked into a browser build), every incident becomes a process argument. When it lives in a hub with clamps, the argument ends and the work begins — for Java services, Python agents, React BFFs, and Angular admin hosts **at once**.
...
examples/
  agent-debug-probe-live/
    verbose: off   # live verbose probe flag
...
```text
examples / agent-debug-probe-live
  verbose = off
...
String posture = f.hasKey("verbose")
    ? f.get("verbose")
    : "off";
// honor posture on the hot path — no hub RTT
```

Runnable: [examples/java/agent-debug-probe-live](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agent-debug-probe-live)
...
# Read examples/agent-debug-probe-live/verbose live.
# Enabling a skill, tightening a budget, or pausing MCP writes
# must not require killing this process.
```

Companion: [examples/python/agent-debug-probe-live](https://github.com/kiponos-io/kiponos-io/tree/master/examples/python/agent-debug-probe-live)
...
await kip.connect();
// set/get under examples/agent-debug-probe-live — inject mirror to SPA; never embed Connect tokens in bundles
```
...
3. `cd examples/java/agent-debug-probe-live && cp kiponos.local.env.example kiponos.local.env` and fill tokens.  
4. `./gradlew test run` — print `verbose`.  
5. Flip the leaf on the dashboard; re-run or leave a long-lived peer connected — no rebuild.  
6. Optional: `examples/python/agent-debug-probe-live` agent peer; `@kiponos/react` / `@kiponos/angular` on Node.
...
**live verbose probe flag** belongs on the hub — not in four redeploys, not in a dead agent session, not in a browser secret. The multi-SDK mesh is here. Use it while the process still runs.
