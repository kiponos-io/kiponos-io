---
title: "Telecom: Turn per-tenant quota Into Incident Posture"
published: false
tags: java, telecom, sre, kiponos
description: "Live per-tenant quota via Kiponos Python SDK — telecom posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-telecom-tenant-quota-0529.md
main_image: ./devto-cover-ops-posture-telecom-tenant-quota-0529.jpg
---
...
Domain: QoS, rate classes, trunk limits. This essay maps hub key `quotaPerMin` to **per-tenant quota** so the lesson stays concrete for telecom operators.
...
```python
policy = kiponos.path("examples", "ops-telecom-tenant-quota")
...
1. Name the hub path so humans find it under pressure (`examples/ops-telecom-tenant-quota/quotaPerMin`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: per-tenant quota for telecom — numbers humans already change verbally in war rooms.
