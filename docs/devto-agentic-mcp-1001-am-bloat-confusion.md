...
description: "MCP host on senses wall: Bloat = schema-budget. Confusion = tools-allow.. Tool design post 2026: bloat loads unused defs; confusion picks wrong tools as reasoni"
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-mcp-1001-am-bloat-confusion.md
---

I have sat next to the senses wall at 11:41 while MCP host was *this close* to calling the wrong MCP tool.

Not a model failure. A **posture** failure.

AWS MCP tool-design: the two problems behind most failures. Fixes that add more description worsen bloat.

Someone on the floor said, out loud, **add a better tool description so it picks right**. MCP host had started earlier. The tool list was frozen in `config.toml` / `.mcp.json` / a skill file. The only “safe” move anyone trusted was:
...
That is the ceremony. **Tool design post 2026: bloat loads unused defs; confusion picks wrong tools as reasoning degrades**

**The Aha:** More prose in the schema is more bloat. Shrink the live set instead. Real-time nested state is now a **primitive** — as native as a hashmap — via [Kiponos.io](https://kiponos.io). The host stays up. The leaf moves. The next tool call already obeys.

## The problem: Bloat = schema-budget. Confusion = tools-allow.
...
Tool design post 2026: bloat loads unused defs; confusion picks wrong tools as reasoning degrades
...
Profile shape: `['my-app']['v1.0.0']['dev']['base']`. The leaf in this story is `schema-budget` under `examples/agentic-mcp-1001-am-bloat-confusion/schema-budget`.

## Architecture

```mermaid
flowchart LR
  wall[senses wall] -->|set schema-budget| hub[Kiponos hub]
...
```yaml
examples/agentic-mcp-1001-am-bloat-confusion:
  schema-budget: 4000
  schema-budget: "4000"
  result-cap: "2000"
  tool-search: "on"
...
Eight-plus keys when you pair `schema-budget` with sister dials (budget, mute, pause). They move together on the senses wall.
...
Folder gate = Kiponos.createForCurrentTeam()
    .getFolder("examples/agentic-mcp-1001-am-bloat-confusion");
...
try:
    posture = k.get("examples/agentic-mcp-1001-am-bloat-confusion/schema-budget", "live")
    if not str(posture):
        raise PermissionError("Bloat = schema-budget. Confusion = tools-allow. gated live — host not restarted")
finally:
    k.disconnect()
```
...
Runnable mesh: `https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-mcp-1001-am-bloat-confusion` — `./gradlew test run`.
...
| User says “no writes” mid-turn | Restart MCP host | Set `schema-budget`; next call already denies |
...
`schema-budget` rarely moves alone on the senses wall. Pair it with `schema-budget`, `result-cap`, or `execute-gate` so you do not fix Bloat = schema-budget. Confusion = tools-allow. by inventing a second incident.

## Why MCP host is the wrong restart target

MCP host is good at calling tools. It is not a control plane. Killing it to flip `schema-budget` teaches the on-call that judgment requires a process ID. The senses wall already disagrees.
...
3. `cd examples/java/agentic-mcp-1001-am-bloat-confusion && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-mcp-1001-am-bloat-confusion/schema-budget=...`
5. In the dashboard, change `schema-budget`. Keep MCP host up. No MCP reconnect.
6. Point the tool wrapper at the same leaf. Do not ship a new server binary to flip Bloat = schema-budget. Confusion = tools-allow..
...
If flipping **Bloat = schema-budget. Confusion = tools-allow.** requires restarting MCP host or reconnecting MCP, you do not have agentic control. You have a hope with a process ID.
...
How to try: `examples/java/agentic-mcp-1001-am-bloat-confusion` and `./gradlew test`.
