---
title: "The Ads War Room Already Knew logSamplePerThousand — The Jar Did Not"
published: false
tags: java, advertising, devops, kiponos
description: "Live log sample per thousand via Kiponos Java SDK — ads posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-ads-log-sample-0763.md
main_image: https://files.catbox.moe/nim9xp.jpg
---
...
SEV-2 started as a metric spike and became a ceremony debate: who can change logSamplePerThousand without a PR.

Domain: bid caps, frequency, pacing. This essay maps hub key `logSamplePerThousand` to **log sample per thousand** so the lesson stays concrete for ads operators.
...
```java
var policy = kiponos.path("examples", "ops-ads-log-sample");
String mode = policy.getString("degradeMode", "full");
int logSamplePerThousand = policy.getInt("logSamplePerThousand");
return router.decide(mode, logSamplePerThousand);
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-ads-log-sample/logSamplePerThousand`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: log sample per thousand for ads — numbers humans already change verbally in war rooms.
