...
description: "Claude Code on senses wall: Do not wait for every host to implement tool search. Your Grok Build / Cursor / Claude Code build may or may not defer. Production c"
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-mcp-0918-am-client-blame.md
---
...
David Soria Parra: context bloat criticism is often naive clients dumping 20–100 tools. Progressive discovery exists.

Someone on the floor said, out loud, **it's a client bug, not MCP**. Claude Code had started earlier. The tool list was frozen in `config.toml` / `.mcp.json` / a skill file. The only “safe” move anyone trusted was:
...
That is the ceremony. **Your Grok Build / Cursor / Claude Code build may or may not defer. Production cannot wait.**

**The Aha:** Even a perfect client needs a live roster. Kiponos is that roster when the binary is already running. Real-time nested state is now a **primitive** — as native as a hashmap — via [Kiponos.io](https://kiponos.io). The host stays up. The leaf moves. The next tool call already obeys.

## The problem: Do not wait for every host to implement tool search
...
Your Grok Build / Cursor / Claude Code build may or may not defer. Production cannot wait.
...
Profile shape: `['my-app']['v1.0.0']['dev']['base']`. The leaf in this story is `tool-search` under `examples/agentic-mcp-0918-am-client-blame/tool-search`.
...
```yaml
examples/agentic-mcp-0918-am-client-blame:
  tool-search: on
  schema-budget: "4000"
  result-cap: "2000"
  tool-search: "on"
...
Eight-plus keys when you pair `tool-search` with sister dials (budget, mute, pause). They move together on the senses wall.
...
Folder gate = Kiponos.createForCurrentTeam()
    .getFolder("examples/agentic-mcp-0918-am-client-blame");
...
try:
    posture = k.get("examples/agentic-mcp-0918-am-client-blame/tool-search", "live")
    if not str(posture):
        raise PermissionError("Do not wait for every host to implement tool search gated live — host not restarted")
finally:
    k.disconnect()
```
...
Runnable mesh: `https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-mcp-0918-am-client-blame` — `./gradlew test run`.
...
`tool-search` rarely moves alone on the senses wall. Pair it with `schema-budget`, `result-cap`, or `execute-gate` so you do not fix Do not wait for every host to implement tool search by inventing a second incident.

## Why Claude Code is the wrong restart target

Claude Code is good at calling tools. It is not a control plane. Killing it to flip `tool-search` teaches the on-call that judgment requires a process ID. The senses wall already disagrees.
...
3. `cd examples/java/agentic-mcp-0918-am-client-blame && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-mcp-0918-am-client-blame/tool-search=...`
5. In the dashboard, change `tool-search`. Keep Claude Code up. No MCP reconnect.
6. Point the tool wrapper at the same leaf. Do not ship a new server binary to flip Do not wait for every host to implement tool search.
...
If flipping **Do not wait for every host to implement tool search** requires restarting Claude Code or reconnecting MCP, you do not have agentic control. You have a hope with a process ID.
...
How to try: `examples/java/agentic-mcp-0918-am-client-blame` and `./gradlew test`.
