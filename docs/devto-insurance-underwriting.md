...
At 02:14 the war room already knew the right **underwriting score cutoff**. The jar still believed last week's YAML.

## The problem: ceremony between judgment and effect

When **underwriting score cutoff** is frozen in a deploy unit, every incident becomes a process argument. Hotfixes still mean CI + roll. Feature flags often mean a second control plane with its own delay.
...
```java
Folder policy = kiponos.path("examples", "insurance-underwriting");
int scoreCutoff = policy.getInt("scoreCutoff"); // local get — no hub RTT
if (scoreCutoff <= 0) {
    return failClosed();
}
return apply(scoreCutoff);
```

Ops (or automation) sets `scoreCutoff` in the hub. The **next** evaluation uses it. Same binary. Same tests for structure.
...
Dashboard / automation ──write──► Kiponos hub
                               ...
                               SDK in-process cache
                                      │ local get
                                      ▼
                               Hot path (underwriting score cutoff)
```
...
- Canonical source: [this article on GitHub](https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-insurance-underwriting.md)
...
Feature flags are often product gates. This essay is about **ops posture** on insurance underwriting: **underwriting score cutoff** — numbers war rooms already shout.
...
Treat `scoreCutoff` as a slice of error budget. Raise when the world is healthy; lower when dependencies are sick. Write the number that survives a bad day.
...
**Underwriting score cutoff** that requires a deploy is optimistic documentation.
