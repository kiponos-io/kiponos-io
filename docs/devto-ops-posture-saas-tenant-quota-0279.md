---
title: "Saas: Turn per-tenant quota Into Incident Posture"
published: false
...
main_image: ./devto-cover-ops-posture-saas-tenant-quota-0279.jpg
---
...
Domain: multi-tenant quotas, feature posture. This essay maps hub key `quotaPerMin` to **per-tenant quota** so the lesson stays concrete for saas operators.
...
```java
var policy = kiponos.path("examples", "ops-saas-tenant-quota");
...
1. Name the hub path so humans find it under pressure (`examples/ops-saas-tenant-quota/quotaPerMin`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: per-tenant quota for saas — numbers humans already change verbally in war rooms.
