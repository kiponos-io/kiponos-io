...
description: "Claude Code on shopping-admin wall: Protect the turn clock with two live ints. On-call math: minutes of outage vs tokens of schema"
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-mcp-1007-pm-turn-clock.md
---
...
Ops people feel this; agent people feel MCP. Same leaf.

Someone on the floor said, out loud, **the model needs those tool defs**. Claude Code had started earlier. The tool list was frozen in `config.toml` / `.mcp.json` / a skill file. The only “safe” move anyone trusted was:
...
That is the ceremony. **On-call math: minutes of outage vs tokens of schema**

**The Aha:** The model needs the *incident*. `schema-budget` + `max-tokens` protect the clock. Real-time nested state is now a **primitive** — as native as a hashmap — via [Kiponos.io](https://kiponos.io). The host stays up. The leaf moves. The next tool call already obeys.
...
On-call math: minutes of outage vs tokens of schema
...
Profile shape: `['my-app']['v1.0.0']['dev']['base']`. The leaf in this story is `max-tokens` under `examples/agentic-mcp-1007-pm-turn-clock/max-tokens`.
...
```yaml
examples/agentic-mcp-1007-pm-turn-clock:
  max-tokens: 8000
  schema-budget: "4000"
  result-cap: "2000"
  tool-search: "on"
...
Folder gate = Kiponos.createForCurrentTeam()
    .getFolder("examples/agentic-mcp-1007-pm-turn-clock");
...
try:
    posture = k.get("examples/agentic-mcp-1007-pm-turn-clock/max-tokens", "live")
    if not str(posture):
        raise PermissionError("Protect the turn clock with two live ints gated live — host not restarted")
finally:
    k.disconnect()
```
...
Runnable mesh: `https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-mcp-1007-pm-turn-clock` — `./gradlew test run`.
...
`max-tokens` rarely moves alone on the shopping-admin wall. Pair it with `schema-budget`, `result-cap`, or `execute-gate` so you do not fix Protect the turn clock with two live ints by inventing a second incident.
...
3. `cd examples/java/agentic-mcp-1007-pm-turn-clock && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-mcp-1007-pm-turn-clock/max-tokens=...`
5. In the dashboard, change `max-tokens`. Keep Claude Code up. No MCP reconnect.
6. Point the tool wrapper at the same leaf. Do not ship a new server binary to flip Protect the turn clock with two live ints.
...
If flipping **Protect the turn clock with two live ints** requires restarting Claude Code or reconnecting MCP, you do not have agentic control. You have a hope with a process ID.
...
How to try: `examples/java/agentic-mcp-1007-pm-turn-clock` and `./gradlew test`.
