---
title: "The Insurance War Room Already Knew degradeMode — The Jar Did Not"
published: false
tags: java, insurance, architecture, kiponos
description: "Live degradation mode via Kiponos Java SDK — insurance posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-insurance-degrade-mode-0628.md
main_image: ./devto-cover-ops-posture-insurance-degrade-mode-0628.jpg
---
...
Domain: underwriting, claims velocity. This essay maps hub key `degradeMode` to **degradation mode** so the lesson stays concrete for insurance operators.
...
```java
var policy = kiponos.path("examples", "ops-insurance-degrade-mode");
String mode = policy.getString("degradeMode", "full");
int degradeMode = policy.getInt("degradeMode");
return router.decide(mode, degradeMode);
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-insurance-degrade-mode/degradeMode`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: degradation mode for insurance — numbers humans already change verbally in war rooms.
