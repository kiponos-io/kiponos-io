---
title: "Live Trace Sample Per Thousand for Media — Mouth to Process in Seconds"
published: false
...
main_image: ./devto-cover-ops-posture-media-trace-sample-0162.jpg
---
...
Domain: bitrate, CDN, origin. This essay maps hub key `samplePerThousand` to **trace sample per thousand** so the lesson stays concrete for media operators.
...
```python
policy = kiponos.path("examples", "ops-media-trace-sample")
samplePerThousand = int(policy.get("samplePerThousand", 10))
if not limiter.try_acquire(samplePerThousand):
    return too_many()
return handle(req)
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-media-trace-sample/samplePerThousand`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: trace sample per thousand for media — numbers humans already change verbally in war rooms.
