---
title: "The Platform War Room Already Knew velocityCap — The Jar Did Not"
published: false
tags: java, platform, architecture, kiponos
description: "Live fraud velocity cap via Kiponos Java SDK — platform posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-platform-velocity-cap-1123.md
main_image: ./devto-cover-ops-posture-platform-velocity-cap-1123.jpg
---
...
The partner brownout did not wait for your change-management window. Your fraud velocity cap did — until it lived in a hub.

Domain: shared libraries, defaults. This essay maps hub key `velocityCap` to **fraud velocity cap** so the lesson stays concrete for platform operators.
...
```java
var policy = kiponos.path("examples", "ops-platform-velocity-cap");
String mode = policy.getString("degradeMode", "full");
int velocityCap = policy.getInt("velocityCap");
return router.decide(mode, velocityCap);
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-platform-velocity-cap/velocityCap`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: fraud velocity cap for platform — numbers humans already change verbally in war rooms.
