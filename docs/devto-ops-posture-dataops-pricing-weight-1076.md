---
title: "We Stopped Shipping Pricing / Promo Weight as a Constant (Dataops)"
published: false
...
main_image: https://files.catbox.moe/v8lgyi.jpg
---
...
Domain: batch, lag, checkpoint. This essay maps hub key `weight` to **pricing / promo weight** so the lesson stays concrete for dataops operators.
...
```python
policy = kiponos.path("examples", "ops-dataops-pricing-weight")
...
1. Name the hub path so humans find it under pressure (`examples/ops-dataops-pricing-weight/weight`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: pricing / promo weight for dataops — numbers humans already change verbally in war rooms.
