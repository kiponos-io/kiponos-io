---
title: "The Saas War Room Already Knew webhookMaxAttempts — The Jar Did Not"
published: false
...
Domain: multi-tenant quotas, feature posture. This essay maps hub key `webhookMaxAttempts` to **webhook retry budget** so the lesson stays concrete for saas operators.
...
```java
Folder policy = kiponos.path("examples", "ops-saas-webhook-retry");
...
1. Name the hub path so humans find it under pressure (`examples/ops-saas-webhook-retry/webhookMaxAttempts`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: webhook retry budget for saas — numbers humans already change verbally in war rooms.
