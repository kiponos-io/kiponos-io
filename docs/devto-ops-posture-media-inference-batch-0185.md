---
title: "Inference Batch Size Should Not Wait for a Redeploy — Media Live Posture"
published: false
tags: java, streaming, devops, kiponos
description: "Live inference batch size via Kiponos Python SDK — media posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-media-inference-batch-0185.md
main_image: ./devto-cover-ops-posture-media-inference-batch-0185.jpg
---
...
Domain: bitrate, CDN, origin. This essay maps hub key `batchSize` to **inference batch size** so the lesson stays concrete for media operators.
...
```python
policy = kiponos.path("examples", "ops-media-inference-batch")
batchSize = int(policy.get("batchSize", 16))
if not limiter.try_acquire(batchSize):
    return too_many()
return handle(req)
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-media-inference-batch/batchSize`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: inference batch size for media — numbers humans already change verbally in war rooms.
