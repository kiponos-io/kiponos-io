---
title: "Composite as a Super Pattern — Live Tree of Rules"
published: false
...
**The Aha:** Enable/disable branches of a rule tree live

Composite trees are great until a leaf must mute without rewriting the whole tree in a PR.
...
```yaml
patterns/composite/rules/
  # live knobs
  enabled-branches: geo,velocity
```

Hot path (local memory after connect — WebSocket deltas, no per-request hub RTT):

```java
Folder policy = kiponos.path('patterns', 'composite', 'rules');
// local gets — never remote on the money path
root.eval(ctx, enabledBranches);
```
...
Full tree: [https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/pattern-composite-live-rules](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/pattern-composite-live-rules)
...
Composite owns structure. Super Composite owns which branches speak.
...
*Runnable: [pattern-composite-live-rules](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/pattern-composite-live-rules) · [kiponos.io](https://kiponos.io)*
