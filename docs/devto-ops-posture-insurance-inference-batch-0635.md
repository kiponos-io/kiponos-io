---
title: "Inference Batch Size Should Not Wait for a Redeploy — Insurance Live Posture"
published: false
tags: java, insurance, architecture, kiponos
description: "Live inference batch size via Kiponos Java SDK — insurance posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-insurance-inference-batch-0635.md
main_image: ./devto-cover-ops-posture-insurance-inference-batch-0635.jpg
---
...
Domain: underwriting, claims velocity. This essay maps hub key `batchSize` to **inference batch size** so the lesson stays concrete for insurance operators.
...
```java
Folder policy = kiponos.path("examples", "ops-insurance-inference-batch");
int batchSize = Math.min(policy.getInt("batchSize"), HARD_MAX);
limiter.setLimit(batchSize); // next evaluation sees it
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-insurance-inference-batch/batchSize`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: inference batch size for insurance — numbers humans already change verbally in war rooms.
