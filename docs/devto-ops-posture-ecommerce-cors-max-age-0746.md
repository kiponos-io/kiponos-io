---
title: "We Stopped Shipping Cors Max-Age as a Constant (Ecommerce)"
published: false
...
Domain: cart, promo weights, stock. This essay maps hub key `corsMaxAgeSec` to **CORS max-age** so the lesson stays concrete for ecommerce operators.
...
```python
policy = kiponos.path("examples", "ops-ecommerce-cors-max-age")
...
1. Name the hub path so humans find it under pressure (`examples/ops-ecommerce-cors-max-age/corsMaxAgeSec`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: CORS max-age for ecommerce — numbers humans already change verbally in war rooms.
