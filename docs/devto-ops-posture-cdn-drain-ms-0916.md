---
title: "We Stopped Shipping Graceful Drain Window as a Constant (Cdn)"
published: false
...
main_image: https://iili.io/CSrZ1ff.jpg
---
...
Domain: edge rules, cache TTL, purge. This essay maps hub key `gracefulShutdownMs` to **graceful drain window** so the lesson stays concrete for cdn operators.
...
```python
policy = kiponos.path("examples", "ops-cdn-drain-ms")
...
1. Name the hub path so humans find it under pressure (`examples/ops-cdn-drain-ms/gracefulShutdownMs`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: graceful drain window for cdn — numbers humans already change verbally in war rooms.
