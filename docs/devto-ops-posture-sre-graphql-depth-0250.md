---
title: "Graphql Query Depth Should Not Wait for a Redeploy — Sre Live Posture"
published: false
...
Domain: canary, drain, error budget. This essay maps hub key `maxDepth` to **GraphQL query depth** so the lesson stays concrete for sre operators.
...
```java
var policy = kiponos.path("examples", "ops-sre-graphql-depth");
String mode = policy.getString("degradeMode", "full");
int maxDepth = policy.getInt("maxDepth");
return router.decide(mode, maxDepth);
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-sre-graphql-depth/maxDepth`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: GraphQL query depth for sre — numbers humans already change verbally in war rooms.
