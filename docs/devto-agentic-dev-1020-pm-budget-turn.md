---
main_image: https://iili.io/nHdh0xI.jpg
title: "Cursor Finished the Turn Blind — Token budget mid-session on the Senses Path"
published: false
tags: java, python, devops, ai, kiponos
description: "Cursor kept finishing the turn blind on senses wall. Token budget mid-session is a live hub leaf."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-dev-1020-pm-budget-turn.md
---
...
The host had started with `max-tokens` frozen in env / argv / a skill file. Someone on the floor said, out loud, **priority is a leaf, not a restart**. Cursor was still holding the old process. The only “safe” move anyone trusted was:
...
```text
examples/agentic-dev-1020-pm-budget-turn/max-tokens = 8000
```

Runnable proof: [`examples/java/agentic-dev-1020-pm-budget-turn`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-dev-1020-pm-budget-turn)
...
```yaml
examples/
  agentic-dev-1020-pm-budget-turn/
...
Folder gate = kip.getRootFolder()
        ...
        .folderOrCreate("agentic-dev-1020-pm-budget-turn");
...
try:
    posture = k.get("examples/agentic-dev-1020-pm-budget-turn/max-tokens", "8000")
    if str(posture) == "8000":
...
|-------|-----------------|--------------|
| A mid-turn interrupt the host treats as noise | Restart Cursor; lose the turn budget and the open investigation | Set `max-tokens` live; next Cursor tool call already obeys |
...
| Incident over, resume | Another Cursor restart | Set `max-tokens` back; session continues |
...
Cursor is good at calling tools. It is not a control plane. Killing it to flip `max-tokens` teaches the on-call that judgment requires a process ID. The senses wall already disagrees.
...
3. `cd examples/java/agentic-dev-1020-pm-budget-turn && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-dev-1020-pm-budget-turn/max-tokens=...`
5. In the dashboard, change `max-tokens`. Keep the process up. No rebuild.
6. Point your Cursor tool at the same leaf. Do not ship a new server binary to flip Token budget mid-session.
...
If flipping **Token budget mid-session** requires restarting Cursor, you do not have a gate. You have a hope with a process ID.
...
How to try: `examples/java/agentic-dev-1020-pm-budget-turn` and `./gradlew test`.
