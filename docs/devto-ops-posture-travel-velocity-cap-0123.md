---
title: "The Travel War Room Already Knew velocityCap — The Jar Did Not"
published: false
tags: java, travel, architecture, kiponos
description: "Live fraud velocity cap via Kiponos Java SDK — travel posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-travel-velocity-cap-0123.md
main_image: ./devto-cover-ops-posture-travel-velocity-cap-0123.jpg
---
...
Domain: inventory, overbooking, GDS. This essay maps hub key `velocityCap` to **fraud velocity cap** so the lesson stays concrete for travel operators.
...
```java
Folder policy = kiponos.path("examples", "ops-travel-velocity-cap");
int velocityCap = Math.min(policy.getInt("velocityCap"), HARD_MAX);
limiter.setLimit(velocityCap); // next evaluation sees it
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-travel-velocity-cap/velocityCap`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: fraud velocity cap for travel — numbers humans already change verbally in war rooms.
