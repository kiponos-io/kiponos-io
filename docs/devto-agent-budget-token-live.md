---
title: "Agent Token Budgets You Can Tighten Mid-Run"
published: false
...
Domain: multi-SDK distributed systems, agents, MCP, observability, war-room truth. This essay maps hub key `max-tokens-per-turn` under `examples/agent-budget-token-live/` so the lesson stays concrete.

## The problem: ceremony between judgment and effect

You already know the right value for **live max tokens per turn**. The war room knows. The agent almost knows. What you do not have is a path from **mouth → every running peer** that is shorter than a release train and safer than pasting tokens into a SPA.

When `max-tokens-per-turn` is frozen in YAML (or baked into a browser build), every incident becomes a process argument. When it lives in a hub with clamps, the argument ends and the work begins — for Java services, Python agents, React BFFs, and Angular admin hosts **at once**.
...
examples/
  agent-budget-token-live/
    max-tokens-per-turn: 4000   # live max tokens per turn
...
```text
examples / agent-budget-token-live
...
String posture = f.hasKey("max-tokens-per-turn")
    ? f.get("max-tokens-per-turn")
    : "4000";
// honor posture on the hot path — no hub RTT
```

Runnable: [examples/java/agent-budget-token-live](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agent-budget-token-live)
...
# Read examples/agent-budget-token-live/max-tokens-per-turn live.
# Enabling a skill, tightening a budget, or pausing MCP writes
# must not require killing this process.
```

Companion: [examples/python/agent-budget-token-live](https://github.com/kiponos-io/kiponos-io/tree/master/examples/python/agent-budget-token-live)
...
await kip.connect();
// set/get under examples/agent-budget-token-live — inject mirror to SPA; never embed Connect tokens in bundles
```
...
3. `cd examples/java/agent-budget-token-live && cp kiponos.local.env.example kiponos.local.env` and fill tokens.  
4. `./gradlew test run` — print `max-tokens-per-turn`.  
5. Flip the leaf on the dashboard; re-run or leave a long-lived peer connected — no rebuild.  
6. Optional: `examples/python/agent-budget-token-live` agent peer; `@kiponos/react` / `@kiponos/angular` on Node.
...
**live max tokens per turn** belongs on the hub — not in four redeploys, not in a dead agent session, not in a browser secret. The multi-SDK mesh is here. Use it while the process still runs.
