---
title: "The Logistics War Room Already Knew idempotencyWindowSec — The Jar Did Not"
published: false
...
main_image: https://iili.io/CLtLsNj.jpg
---
...
Domain: routing, SLA windows, fleet. This essay maps hub key `idempotencyWindowSec` to **idempotency window** so the lesson stays concrete for logistics operators.
...
```java
Folder policy = kiponos.path("examples", "ops-logistics-idempotency-win");
...
1. Name the hub path so humans find it under pressure (`examples/ops-logistics-idempotency-win/idempotencyWindowSec`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: idempotency window for logistics — numbers humans already change verbally in war rooms.
