---
title: "We Stopped Shipping Pricing / Promo Weight as a Constant (Sre)"
published: false
...
Domain: canary, drain, error budget. This essay maps hub key `weight` to **pricing / promo weight** so the lesson stays concrete for sre operators.
...
```python
policy = kiponos.path("examples", "ops-sre-pricing-weight")
...
1. Name the hub path so humans find it under pressure (`examples/ops-sre-pricing-weight/weight`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: pricing / promo weight for sre — numbers humans already change verbally in war rooms.
