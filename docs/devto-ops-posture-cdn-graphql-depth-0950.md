---
title: "Graphql Query Depth Should Not Wait for a Redeploy — Cdn Live Posture"
published: false
tags: java, networking, devops, kiponos
description: "Live GraphQL query depth via Kiponos Python SDK — cdn posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-cdn-graphql-depth-0950.md
main_image: https://litter.catbox.moe/b4zit3.jpg
---
...
Domain: edge rules, cache TTL, purge. This essay maps hub key `maxDepth` to **GraphQL query depth** so the lesson stays concrete for cdn operators.
...
```python
policy = kiponos.path("examples", "ops-cdn-graphql-depth")
...
1. Name the hub path so humans find it under pressure (`examples/ops-cdn-graphql-depth/maxDepth`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: GraphQL query depth for cdn — numbers humans already change verbally in war rooms.
