...
description: "Live incident pause flag on the Kiponos hub from a React server peer — createFromEnv, no SPA secrets, Java parity. Install @kiponos/react."
...
I have sat through a room that looked like this: agents still doing work while humans were trying to freeze the plane. Someone said “we just need to flip it.” Someone else opened a PR. CI ran. The window closed.
...
- Companion Java tree: `examples/java/react-sdk-incident-pause` (same hub moral)
...
Identity is **where the process runs**. React’s job is to be a first-class peer on the hub, not a bundler that smuggles secrets. Ship the peer once. Leave the bundle alone when the only thing that changed is **incident pause flag**.
