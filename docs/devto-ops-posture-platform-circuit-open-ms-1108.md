---
title: "The Platform War Room Already Knew waitOpenMs — The Jar Did Not"
published: false
tags: java, platform, architecture, kiponos
description: "Live circuit open duration via Kiponos Java SDK — platform posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-platform-circuit-open-ms-1108.md
main_image: ./devto-cover-ops-posture-platform-circuit-open-ms-1108.jpg
---
...
Domain: shared libraries, defaults. This essay maps hub key `waitOpenMs` to **circuit open duration** so the lesson stays concrete for platform operators.
...
```java
Folder policy = kiponos.path("examples", "ops-platform-circuit-open-ms");
int waitOpenMs = Math.min(policy.getInt("waitOpenMs"), HARD_MAX);
limiter.setLimit(waitOpenMs); // next evaluation sees it
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-platform-circuit-open-ms/waitOpenMs`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: circuit open duration for platform — numbers humans already change verbally in war rooms.
