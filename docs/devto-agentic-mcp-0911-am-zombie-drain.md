...
description: "MCP host on senses wall: Live drain/lifecycle for MCP children. Abnormal session end leaves zombies, memory climb, bound ports — no spec lifecycle hook"
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-mcp-0911-am-zombie-drain.md
---

I have sat next to the senses wall at 02:14 while MCP host was *this close* to calling the wrong MCP tool.

Not a model failure. A **posture** failure.

Teams write cron janitors. That is not a protocol. That is a missing live drain leaf.

Someone on the floor said, out loud, **kill -9 the leftover npx — again**. MCP host had started earlier. The tool list was frozen in `config.toml` / `.mcp.json` / a skill file. The only “safe” move anyone trusted was:
...
That is the ceremony. **Abnormal session end leaves zombies, memory climb, bound ports — no spec lifecycle hook**

**The Aha:** Set `mcp-drain=now`. The supervisor obeys. Do not SSH in to reap children. Real-time nested state is now a **primitive** — as native as a hashmap — via [Kiponos.io](https://kiponos.io). The host stays up. The leaf moves. The next tool call already obeys.

## The problem: Live drain/lifecycle for MCP children
...
Abnormal session end leaves zombies, memory climb, bound ports — no spec lifecycle hook
...
Profile shape: `['my-app']['v1.0.0']['dev']['base']`. The leaf in this story is `mcp-drain` under `examples/agentic-mcp-0911-am-zombie-drain/mcp-drain`.

## Architecture

```mermaid
flowchart LR
  wall[senses wall] -->|set mcp-drain| hub[Kiponos hub]
...
```yaml
examples/agentic-mcp-0911-am-zombie-drain:
  mcp-drain: off
  schema-budget: "4000"
  result-cap: "2000"
  tool-search: "on"
...
Eight-plus keys when you pair `mcp-drain` with sister dials (budget, mute, pause). They move together on the senses wall.
...
Folder gate = Kiponos.createForCurrentTeam()
    .getFolder("examples/agentic-mcp-0911-am-zombie-drain");
...
try:
    posture = k.get("examples/agentic-mcp-0911-am-zombie-drain/mcp-drain", "live")
    if not str(posture):
        raise PermissionError("Live drain/lifecycle for MCP children gated live — host not restarted")
finally:
    k.disconnect()
```
...
Runnable mesh: `https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-mcp-0911-am-zombie-drain` — `./gradlew test run`.
...
| User says “no writes” mid-turn | Restart MCP host | Set `mcp-drain`; next call already denies |
...
`mcp-drain` rarely moves alone on the senses wall. Pair it with `schema-budget`, `result-cap`, or `execute-gate` so you do not fix Live drain/lifecycle for MCP children by inventing a second incident.

## Why MCP host is the wrong restart target

MCP host is good at calling tools. It is not a control plane. Killing it to flip `mcp-drain` teaches the on-call that judgment requires a process ID. The senses wall already disagrees.
...
3. `cd examples/java/agentic-mcp-0911-am-zombie-drain && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-mcp-0911-am-zombie-drain/mcp-drain=...`
5. In the dashboard, change `mcp-drain`. Keep MCP host up. No MCP reconnect.
6. Point the tool wrapper at the same leaf. Do not ship a new server binary to flip Live drain/lifecycle for MCP children.
...
If flipping **Live drain/lifecycle for MCP children** requires restarting MCP host or reconnecting MCP, you do not have agentic control. You have a hope with a process ID.
...
How to try: `examples/java/agentic-mcp-0911-am-zombie-drain` and `./gradlew test`.
