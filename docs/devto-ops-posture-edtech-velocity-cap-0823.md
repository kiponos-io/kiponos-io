---
title: "The Edtech War Room Already Knew velocityCap — The Jar Did Not"
published: false
...
Domain: class size, exam windows. This essay maps hub key `velocityCap` to **fraud velocity cap** so the lesson stays concrete for edtech operators.
...
```java
Folder policy = kiponos.path("examples", "ops-edtech-velocity-cap");
...
1. Name the hub path so humans find it under pressure (`examples/ops-edtech-velocity-cap/velocityCap`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: fraud velocity cap for edtech — numbers humans already change verbally in war rooms.
