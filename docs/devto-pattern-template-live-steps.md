---
title: "Template Method as a Super Pattern — Live Step Flags"
published: false
...
**The Aha:** Hook steps on/off without rewriting the template

The skeleton is sacred. The optional hooks are not — fraud check, enrich, notify can toggle under load.
...
```yaml
patterns/template/pipeline/
  # live knobs
  steps: validate,enrich,fraud,persist,notify
```

Hot path (local memory after connect — WebSocket deltas, no per-request hub RTT):

```java
Folder policy = kiponos.path('patterns', 'template', 'pipeline');
// local gets — never remote on the money path
for (String s : steps) runStep(s, ctx);
```
...
Full tree: [https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/pattern-template-live-steps](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/pattern-template-live-steps)
...
Template Method owns order. Super Template lets ops mute a step.
...
*Runnable: [pattern-template-live-steps](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/pattern-template-live-steps) · [kiponos.io](https://kiponos.io)*
