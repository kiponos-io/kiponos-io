---
title: "We Stopped Shipping Graceful Drain Window as a Constant (Mobile)"
published: false
...
main_image: https://iili.io/nTtQgta.jpg
---
...
Domain: API timeouts, payload caps. This essay maps hub key `gracefulShutdownMs` to **graceful drain window** so the lesson stays concrete for mobile operators.
...
```python
policy = kiponos.path("examples", "ops-mobile-drain-ms")
...
1. Name the hub path so humans find it under pressure (`examples/ops-mobile-drain-ms/gracefulShutdownMs`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: graceful drain window for mobile — numbers humans already change verbally in war rooms.
