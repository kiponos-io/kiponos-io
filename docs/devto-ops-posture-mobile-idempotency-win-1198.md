---
title: "The Mobile War Room Already Knew idempotencyWindowSec — The Jar Did Not"
published: false
...
main_image: ./devto-cover-ops-posture-mobile-idempotency-win-1198.jpg
---
...
Domain: API timeouts, payload caps. This essay maps hub key `idempotencyWindowSec` to **idempotency window** so the lesson stays concrete for mobile operators.
...
```java
Folder policy = kiponos.path("examples", "ops-mobile-idempotency-win");
...
1. Name the hub path so humans find it under pressure (`examples/ops-mobile-idempotency-win/idempotencyWindowSec`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: idempotency window for mobile — numbers humans already change verbally in war rooms.
