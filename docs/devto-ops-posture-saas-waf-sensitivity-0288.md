---
title: "The Saas War Room Already Knew wafSensitivity — The Jar Did Not"
published: false
...
Domain: multi-tenant quotas, feature posture. This essay maps hub key `wafSensitivity` to **WAF sensitivity 1-5** so the lesson stays concrete for saas operators.
...
```java
var policy = kiponos.path("examples", "ops-saas-waf-sensitivity");
...
1. Name the hub path so humans find it under pressure (`examples/ops-saas-waf-sensitivity/wafSensitivity`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: WAF sensitivity 1-5 for saas — numbers humans already change verbally in war rooms.
