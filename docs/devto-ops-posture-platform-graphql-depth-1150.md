---
title: "Graphql Query Depth Should Not Wait for a Redeploy — Platform Live Posture"
published: false
...
main_image: ./devto-cover-ops-posture-platform-graphql-depth-1150.jpg
---
...
Domain: shared libraries, defaults. This essay maps hub key `maxDepth` to **GraphQL query depth** so the lesson stays concrete for platform operators.
...
```python
policy = kiponos.path("examples", "ops-platform-graphql-depth")
...
1. Name the hub path so humans find it under pressure (`examples/ops-platform-graphql-depth/maxDepth`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: GraphQL query depth for platform — numbers humans already change verbally in war rooms.
