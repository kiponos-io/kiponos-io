---
title: "The Sre War Room Already Knew bulkheadMax — The Jar Did Not"
published: false
tags: java, sre, devops, kiponos
description: "Live bulkhead concurrency via Kiponos Java SDK — sre posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-sre-bulkhead-max-0218.md
main_image: ./devto-cover-ops-posture-sre-bulkhead-max-0218.jpg
---
...
Domain: canary, drain, error budget. This essay maps hub key `bulkheadMax` to **bulkhead concurrency** so the lesson stays concrete for sre operators.
...
```java
Folder policy = kiponos.path("examples", "ops-sre-bulkhead-max");
int bulkheadMax = Math.min(policy.getInt("bulkheadMax"), HARD_MAX);
limiter.setLimit(bulkheadMax); // next evaluation sees it
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-sre-bulkhead-max/bulkheadMax`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: bulkhead concurrency for sre — numbers humans already change verbally in war rooms.
