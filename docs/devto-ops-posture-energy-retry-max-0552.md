---
title: "Live Retry Budget for Energy — Mouth to Process in Seconds"
published: false
...
Domain: grid dispatch, demand response. This essay maps hub key `maxAttempts` to **retry budget** so the lesson stays concrete for energy operators.
...
```python
policy = kiponos.path("examples", "ops-energy-retry-max")
...
1. Name the hub path so humans find it under pressure (`examples/ops-energy-retry-max/maxAttempts`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: retry budget for energy — numbers humans already change verbally in war rooms.
