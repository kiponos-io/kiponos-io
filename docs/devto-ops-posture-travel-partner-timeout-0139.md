---
title: "Travel: Turn partner API timeout Into Incident Posture"
published: false
...
Domain: inventory, overbooking, GDS. This essay maps hub key `partnerTimeoutMs` to **partner API timeout** so the lesson stays concrete for travel operators.
...
```python
policy = kiponos.path("examples", "ops-travel-partner-timeout")
...
1. Name the hub path so humans find it under pressure (`examples/ops-travel-partner-timeout/partnerTimeoutMs`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: partner API timeout for travel — numbers humans already change verbally in war rooms.
