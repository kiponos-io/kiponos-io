---
title: "The Sre War Room Already Knew waitOpenMs — The Jar Did Not"
published: false
...
main_image: https://iili.io/CtvqLdJ.jpg
---
...
Domain: canary, drain, error budget. This essay maps hub key `waitOpenMs` to **circuit open duration** so the lesson stays concrete for sre operators.
...
```python
policy = kiponos.path("examples", "ops-sre-circuit-open-ms")
...
1. Name the hub path so humans find it under pressure (`examples/ops-sre-circuit-open-ms/waitOpenMs`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: circuit open duration for sre — numbers humans already change verbally in war rooms.
