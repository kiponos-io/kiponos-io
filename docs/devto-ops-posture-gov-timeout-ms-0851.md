---
title: "We Stopped Shipping Outbound Call Timeout as a Constant (Gov)"
published: false
tags: java, architecture, devops, kiponos
description: "Live outbound call timeout via Kiponos Java SDK — gov posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-gov-timeout-ms-0851.md
main_image: ./devto-cover-ops-posture-gov-timeout-ms-0851.jpg
---
...
Domain: form windows, batch cutoffs. This essay maps hub key `timeoutMs` to **outbound call timeout** so the lesson stays concrete for gov operators.
...
```java
Folder policy = kiponos.path("examples", "ops-gov-timeout-ms");
...
1. Name the hub path so humans find it under pressure (`examples/ops-gov-timeout-ms/timeoutMs`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: outbound call timeout for gov — numbers humans already change verbally in war rooms.
