---
title: "The Iot War Room Already Knew wafSensitivity — The Jar Did Not"
published: false
tags: java, iot, devops, kiponos
description: "Live WAF sensitivity 1-5 via Kiponos Java SDK — iot posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-iot-waf-sensitivity-0488.md
main_image: ./devto-cover-ops-posture-iot-waf-sensitivity-0488.jpg
---
...
Domain: device firmware gates, telemetry sample. This essay maps hub key `wafSensitivity` to **WAF sensitivity 1-5** so the lesson stays concrete for iot operators.
...
```java
var policy = kiponos.path("examples", "ops-iot-waf-sensitivity");
String mode = policy.getString("degradeMode", "full");
int wafSensitivity = policy.getInt("wafSensitivity");
return router.decide(mode, wafSensitivity);
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-iot-waf-sensitivity/wafSensitivity`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: WAF sensitivity 1-5 for iot — numbers humans already change verbally in war rooms.
...
WAF sensitivity 1-5 that requires a deploy is optimistic documentation.
