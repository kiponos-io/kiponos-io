---
title: "We Stopped Shipping Slo Warn Burn Bps as a Constant (Insurance)"
published: false
tags: java, insurance, architecture, kiponos
description: "Live SLO warn burn bps via Kiponos Java SDK — insurance posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-insurance-slo-warn-bps-0631.md
main_image: ./devto-cover-ops-posture-insurance-slo-warn-bps-0631.jpg
---
...
SEV-2 started as a metric spike and became a ceremony debate: who can change errorBudgetWarnBps without a PR.

Domain: underwriting, claims velocity. This essay maps hub key `errorBudgetWarnBps` to **SLO warn burn bps** so the lesson stays concrete for insurance operators.
...
```java
Folder policy = kiponos.path("examples", "ops-insurance-slo-warn-bps");
int errorBudgetWarnBps = Math.min(policy.getInt("errorBudgetWarnBps"), HARD_MAX);
limiter.setLimit(errorBudgetWarnBps); // next evaluation sees it
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-insurance-slo-warn-bps/errorBudgetWarnBps`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: SLO warn burn bps for insurance — numbers humans already change verbally in war rooms.
