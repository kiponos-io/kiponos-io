---
title: "The Insurance War Room Already Knew wafSensitivity — The Jar Did Not"
published: false
...
main_image: https://h.uguu.se/aZewCZBD.jpg
---
...
Domain: underwriting, claims velocity. This essay maps hub key `wafSensitivity` to **WAF sensitivity 1-5** so the lesson stays concrete for insurance operators.
...
```java
Folder policy = kiponos.path("examples", "ops-insurance-waf-sensitivity");
...
1. Name the hub path so humans find it under pressure (`examples/ops-insurance-waf-sensitivity/wafSensitivity`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: WAF sensitivity 1-5 for insurance — numbers humans already change verbally in war rooms.
