---
title: "The Marketplace War Room Already Knew waitOpenMs — The Jar Did Not"
published: false
...
main_image: https://iili.io/CQKMax2.jpg
---
...
Domain: seller tiers, fee weights. This essay maps hub key `waitOpenMs` to **circuit open duration** so the lesson stays concrete for marketplace operators.
...
```yaml
examples/
  ops-marketplace-circuit-open-ms/
...
```python
policy = kiponos.path("examples", "ops-marketplace-circuit-open-ms")
...
1. Name the hub path so humans find it under pressure (`examples/ops-marketplace-circuit-open-ms/waitOpenMs`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: circuit open duration for marketplace — numbers humans already change verbally in war rooms.
