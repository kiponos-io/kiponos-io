---
title: "We Stopped Shipping Rag Top-K as a Constant (Sre)"
published: false
tags: java, sre, devops, kiponos
description: "Live RAG top-k via Kiponos Python SDK — sre posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-sre-rag-topk-0236.md
main_image: ./devto-cover-ops-posture-sre-rag-topk-0236.jpg
---
...
Domain: canary, drain, error budget. This essay maps hub key `topK` to **RAG top-k** so the lesson stays concrete for sre operators.
...
```python
policy = kiponos.path("examples", "ops-sre-rag-topk")
topK = min(int(policy.get("topK", 8)), HARD_MAX)
worker_pool.resize(topK)  # or admission semaphore
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-sre-rag-topk/topK`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: RAG top-k for sre — numbers humans already change verbally in war rooms.
