---
title: "Live Ui/Api Debounce for Sre — Mouth to Process in Seconds"
published: false
...
Domain: canary, drain, error budget. This essay maps hub key `debounceMs` to **UI/API debounce** so the lesson stays concrete for sre operators.
...
```python
policy = kiponos.path("examples", "ops-sre-debounce-ms")
...
1. Name the hub path so humans find it under pressure (`examples/ops-sre-debounce-ms/debounceMs`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: UI/API debounce for sre — numbers humans already change verbally in war rooms.
