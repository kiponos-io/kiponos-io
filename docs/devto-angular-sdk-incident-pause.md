...
description: "Live incident pause flag on the Kiponos hub from a Angular server peer — createFromEnv, no SPA secrets, Java parity. Install @kiponos/angular."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-angular-sdk-incident-pause.md
---

**The Aha:** Incident pause flag is a judgment call under pressure. If the only path is a SPA rebuild, you do not have a control plane — you have a ceremony.

I have sat through a room that looked like this: background workers ignoring a human freeze until someone shipped a flag. Someone said “we just need to flip it.” Someone else opened a PR. CI ran. The window closed.
...
- Companion Java tree: `examples/java/angular-sdk-incident-pause` (same hub moral)
...
await kip.path('incident').set('pause', '…');
...
String v = k.path("incident").get("pause", "…");
...
Flip `pause` (or your domain leaf) on the dashboard or from the Node peer. The Java print should follow the hub — not the last SPA deploy.
...
|------|--------------------------------|------------------------|
| Change incident pause flag | Ship SPA / service build | Dashboard or SDK `set()` |
...
1. Name the hub path out loud: `incident/pause`  
...
Identity is **where the process runs**. Angular’s job is to be a first-class peer on the hub, not a bundler that smuggles secrets. Ship the peer once. Leave the bundle alone when the only thing that changed is **incident pause flag**.
