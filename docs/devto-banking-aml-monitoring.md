...
When **AML rule threshold** is frozen in a deploy unit, every incident becomes a process argument. Hotfixes still mean CI + roll. Feature flags often mean a second control plane with its own delay.
...
```java
Folder policy = kiponos.path("examples", "banking-aml-monitoring");
int amlThreshold = policy.getInt("amlThreshold"); // local get — no hub RTT
if (amlThreshold <= 0) {
    return failClosed();
}
return apply(amlThreshold);
```

Ops (or automation) sets `amlThreshold` in the hub. The **next** evaluation uses it. Same binary. Same tests for structure.
...
- Canonical source: [this article on GitHub](https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-banking-aml-monitoring.md)
...
Feature flags are often product gates. This essay is about **ops posture** on AML monitoring: **AML rule threshold** — numbers war rooms already shout.
...
Treat `amlThreshold` as a slice of error budget. Raise when the world is healthy; lower when dependencies are sick. Write the number that survives a bad day.
...
**AML rule threshold** that requires a deploy is optimistic documentation.
