---
title: "We Stopped Shipping Outbound Call Timeout as a Constant (Dataops)"
published: false
...
Domain: batch, lag, checkpoint. This essay maps hub key `timeoutMs` to **outbound call timeout** so the lesson stays concrete for dataops operators.
...
```java
var policy = kiponos.path("examples", "ops-dataops-timeout-ms");
...
1. Name the hub path so humans find it under pressure (`examples/ops-dataops-timeout-ms/timeoutMs`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: outbound call timeout for dataops — numbers humans already change verbally in war rooms.
