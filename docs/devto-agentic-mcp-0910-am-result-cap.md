...
description: "Claude Code on admin-dashboard wall: Live cap on tool-result payload size. MCP Interviewer: top tool averaged 557,766 tokens; 16 tools exceed 128k"
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-mcp-0910-am-result-cap.md
---

I have sat next to the admin-dashboard wall at 00:40 while Claude Code was *this close* to calling the wrong MCP tool.

Not a model failure. A **posture** failure.

Tool results dumped raw into the prompt. Exact counts become strawberry-problems. Cost explodes.

Someone on the floor said, out loud, **stop pasting the whole Jira dump into the model**. Claude Code had started earlier. The tool list was frozen in `config.toml` / `.mcp.json` / a skill file. The only “safe” move anyone trusted was:
...
That is the ceremony. **MCP Interviewer: top tool averaged 557,766 tokens; 16 tools exceed 128k**

**The Aha:** `result-cap` is a hub int. Project/truncate in the tool. Do not restart to 'use a smaller server'. Real-time nested state is now a **primitive** — as native as a hashmap — via [Kiponos.io](https://kiponos.io). The host stays up. The leaf moves. The next tool call already obeys.

## The problem: Live cap on tool-result payload size
...
MCP Interviewer: top tool averaged 557,766 tokens; 16 tools exceed 128k
...
Profile shape: `['my-app']['v1.0.0']['dev']['base']`. The leaf in this story is `result-cap` under `examples/agentic-mcp-0910-am-result-cap/result-cap`.

## Architecture

```mermaid
flowchart LR
  wall[admin-dashboard wall] -->|set result-cap| hub[Kiponos hub]
...
```yaml
examples/agentic-mcp-0910-am-result-cap:
  result-cap: 4000
  schema-budget: "4000"
  result-cap: "2000"
  tool-search: "on"
...
Eight-plus keys when you pair `result-cap` with sister dials (budget, mute, pause). They move together on the admin-dashboard wall.
...
Folder gate = Kiponos.createForCurrentTeam()
    .getFolder("examples/agentic-mcp-0910-am-result-cap");
...
try:
    posture = k.get("examples/agentic-mcp-0910-am-result-cap/result-cap", "live")
    if not str(posture):
        raise PermissionError("Live cap on tool-result payload size gated live — host not restarted")
finally:
    k.disconnect()
```
...
Runnable mesh: `https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-mcp-0910-am-result-cap` — `./gradlew test run`.
...
| User says “no writes” mid-turn | Restart Claude Code | Set `result-cap`; next call already denies |
...
`result-cap` rarely moves alone on the admin-dashboard wall. Pair it with `schema-budget`, `result-cap`, or `execute-gate` so you do not fix Live cap on tool-result payload size by inventing a second incident.

## Why Claude Code is the wrong restart target

Claude Code is good at calling tools. It is not a control plane. Killing it to flip `result-cap` teaches the on-call that judgment requires a process ID. The admin-dashboard wall already disagrees.
...
3. `cd examples/java/agentic-mcp-0910-am-result-cap && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-mcp-0910-am-result-cap/result-cap=...`
5. In the dashboard, change `result-cap`. Keep Claude Code up. No MCP reconnect.
6. Point the tool wrapper at the same leaf. Do not ship a new server binary to flip Live cap on tool-result payload size.
...
If flipping **Live cap on tool-result payload size** requires restarting Claude Code or reconnecting MCP, you do not have agentic control. You have a hope with a process ID.
...
How to try: `examples/java/agentic-mcp-0910-am-result-cap` and `./gradlew test`.
