---
title: "The Travel War Room Already Knew logSamplePerThousand — The Jar Did Not"
published: false
tags: java, travel, architecture, kiponos
description: "Live log sample per thousand via Kiponos Java SDK — travel posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-travel-log-sample-0113.md
main_image: ./devto-cover-ops-posture-travel-log-sample-0113.jpg
---
...
Domain: inventory, overbooking, GDS. This essay maps hub key `logSamplePerThousand` to **log sample per thousand** so the lesson stays concrete for travel operators.
...
```java
Folder policy = kiponos.path("examples", "ops-travel-log-sample");
int logSamplePerThousand = policy.getInt("logSamplePerThousand");
if (logSamplePerThousand <= 0) {
    return failClosed(); // money paths
}
return apply(logSamplePerThousand);
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-travel-log-sample/logSamplePerThousand`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: log sample per thousand for travel — numbers humans already change verbally in war rooms.
