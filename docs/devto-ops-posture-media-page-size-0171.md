---
title: "We Stopped Shipping Pagination Page Size as a Constant (Media)"
published: false
...
main_image: https://iili.io/nx24cX4.jpg
---
...
Domain: bitrate, CDN, origin. This essay maps hub key `pageSize` to **pagination page size** so the lesson stays concrete for media operators.
...
```java
var policy = kiponos.path("examples", "ops-media-page-size");
...
1. Name the hub path so humans find it under pressure (`examples/ops-media-page-size/pageSize`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: pagination page size for media — numbers humans already change verbally in war rooms.
