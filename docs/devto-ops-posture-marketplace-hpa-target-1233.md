---
title: "The Marketplace War Room Already Knew hpaTarget — The Jar Did Not"
published: false
...
main_image: https://iili.io/CQKVCej.jpg
---
...
Domain: seller tiers, fee weights. This essay maps hub key `hpaTarget` to **HPA CPU target percent** so the lesson stays concrete for marketplace operators.
...
```python
policy = kiponos.path("examples", "ops-marketplace-hpa-target")
...
1. Name the hub path so humans find it under pressure (`examples/ops-marketplace-hpa-target/hpaTarget`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: HPA CPU target percent for marketplace — numbers humans already change verbally in war rooms.
