...
description: "Grok Build on travel-coordinator wall: Per-server schema budget live. GitHub MCP ~26k, Slack ~21k — before any user text"
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-mcp-0923-pm-lazy-schema.md
---
...
Stephanie Goodman / AgentPMT 2026. One server is already a tax. Three is an emergency.

Someone on the floor said, out loud, **we only use issues.search, why is the rest in context?**. Grok Build had started earlier. The tool list was frozen in `config.toml` / `.mcp.json` / a skill file. The only “safe” move anyone trusted was:
...
That is the ceremony. **GitHub MCP ~26k, Slack ~21k — before any user text**

**The Aha:** Budget the schemas. Expose search. Hide the other 34 until a leaf says so. Real-time nested state is now a **primitive** — as native as a hashmap — via [Kiponos.io](https://kiponos.io). The host stays up. The leaf moves. The next tool call already obeys.

## The problem: Per-server schema budget live
...
GitHub MCP ~26k, Slack ~21k — before any user text
...
Profile shape: `['my-app']['v1.0.0']['dev']['base']`. The leaf in this story is `schema-budget` under `examples/agentic-mcp-0923-pm-lazy-schema/schema-budget`.

## Architecture

```mermaid
flowchart LR
  wall[travel-coordinator wall] -->|set schema-budget| hub[Kiponos hub]
...
```yaml
examples/agentic-mcp-0923-pm-lazy-schema:
  schema-budget: 4000
  schema-budget: "4000"
  result-cap: "2000"
  tool-search: "on"
...
Eight-plus keys when you pair `schema-budget` with sister dials (budget, mute, pause). They move together on the travel-coordinator wall.
...
Folder gate = Kiponos.createForCurrentTeam()
    .getFolder("examples/agentic-mcp-0923-pm-lazy-schema");
...
try:
    posture = k.get("examples/agentic-mcp-0923-pm-lazy-schema/schema-budget", "live")
    if not str(posture):
        raise PermissionError("Per-server schema budget live gated live — host not restarted")
finally:
    k.disconnect()
```
...
Runnable mesh: `https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-mcp-0923-pm-lazy-schema` — `./gradlew test run`.
...
`schema-budget` rarely moves alone on the travel-coordinator wall. Pair it with `schema-budget`, `result-cap`, or `execute-gate` so you do not fix Per-server schema budget live by inventing a second incident.

## Why Grok Build is the wrong restart target

Grok Build is good at calling tools. It is not a control plane. Killing it to flip `schema-budget` teaches the on-call that judgment requires a process ID. The travel-coordinator wall already disagrees.
...
3. `cd examples/java/agentic-mcp-0923-pm-lazy-schema && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-mcp-0923-pm-lazy-schema/schema-budget=...`
5. In the dashboard, change `schema-budget`. Keep Grok Build up. No MCP reconnect.
6. Point the tool wrapper at the same leaf. Do not ship a new server binary to flip Per-server schema budget live.
...
If flipping **Per-server schema budget live** requires restarting Grok Build or reconnecting MCP, you do not have agentic control. You have a hope with a process ID.
...
How to try: `examples/java/agentic-mcp-0923-pm-lazy-schema` and `./gradlew test`.
