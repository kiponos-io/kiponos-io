---
main_image: https://iili.io/nIFJyts.jpg
...
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-overflow-20260919-senses.md
---

I have sat next to the senses wall at 11:41 while Cursor was *this close* to doing the wrong thing.

Not a model failure. A **posture** failure.

The host had started with `handoff` frozen in env / argv / a skill file. Someone on the floor said, out loud, **priority is a leaf, not a restart**. Cursor was still holding the old process. The only “safe” move anyone trusted was:
...
**The Aha:** agent handoff without restart as live posture on the senses wall is not a binary you reboot. It is a **function that should read live posture** from [Kiponos.io](https://kiponos.io) on every call. The host stays up. The leaf moves.

## The problem: agent handoff without restart as live posture on the senses wall lived in the process, not in the turn

Cursor is good at *calling* tools. It is not born with a **shared, instant, restart-free control plane**.

So teams hide agent handoff without restart as live posture on the senses wall in the only places agent frameworks actually ship:
...
| Hard-coded `if` on `handoff` | A release | The incident clock |

The senses wall already *knew*. Cursor did not, because it had started earlier.
...
| Restart Cursor — it is cheap | Cheap until 11:41 ate the turn budget and the open investigation |

## The Aha: local get, live write, host stays up

Kiponos holds a nested tree. Java and Python SDKs keep the latest values **in memory**, patched over WebSocket deltas. The hot path inside a Cursor tool is a **local get** — no HTTP RTT per senses lookup.

Hub leaf for this essay:

```text
examples/multi-agent-collab-bus/handoff = live
```

Runnable proof: [`examples/java/multi-agent-collab-bus`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/multi-agent-collab-bus)
...
examples/
  multi-agent-collab-bus/
    handoff: live          # agent handoff without restart as live posture on the senses wall
apps/
  senses/
    live:
      handoff: live
...
if (!gate.hasKey("handoff")) {
    gate.set("handoff", "live");
}
String posture = gate.get("handoff");
...
try:
    posture = k.get("examples/multi-agent-collab-bus/handoff", "live")
    if str(posture) == "live":
        raise PermissionError("agent handoff without restart as live posture on the senses wall gated live — host not restarted")
finally:
    k.disconnect()
```
...
|-------|-----------------|--------------|
| A mid-turn interrupt the host treats as noise | Restart Cursor; lose the turn budget and the open investigation | Set `handoff` live; next Cursor tool call already obeys |
| Peer host still on old handoff | Paste the value into the other chat | One hub leaf; both processes `get()` locally |
| senses wall shows the new posture | Cursor started earlier so it writes anyway | Dashboard and tool share the same memory tree |
| Incident over, resume | Another Cursor restart | Set `handoff` back; session continues |
...
- A dashboard edit is a **delta** of `handoff`, not a config-file reload.
...
|----------|------------|------------------------|
| Env file + Cursor reboot | Simple at 09:00 | The freeze is at 11:41 |
...
At 11:41 someone said, out loud: **priority is a leaf, not a restart**. That sentence is the whole product. If it cannot land in the running Cursor process in seconds, you do not have posture. You have a wiki.
...
3. `cd examples/java/multi-agent-collab-bus && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/multi-agent-collab-bus/handoff=...`
5. In the dashboard, change `handoff`. Keep the process up. No rebuild.
6. Point your Cursor tool at the same leaf. Do not ship a new server binary to flip agent handoff without restart as live posture on the senses wall.
...
If flipping **agent handoff without restart as live posture on the senses wall** requires restarting Cursor, you do not have a gate. You have a hope with a process ID.
...
How to try: `examples/java/multi-agent-collab-bus` and `./gradlew test`.
