---
title: "Graphql Query Depth Should Not Wait for a Redeploy — Energy Live Posture"
published: false
...
Domain: grid dispatch, demand response. This essay maps hub key `maxDepth` to **GraphQL query depth** so the lesson stays concrete for energy operators.
...
```java
Folder policy = kiponos.path("examples", "ops-energy-graphql-depth");
...
1. Name the hub path so humans find it under pressure (`examples/ops-energy-graphql-depth/maxDepth`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: GraphQL query depth for energy — numbers humans already change verbally in war rooms.
