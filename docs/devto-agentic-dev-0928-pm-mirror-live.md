...
description: "MCP host kept finishing the turn blind on mirror-phone wall. Mirror Phone live device leaf is a live hub leaf."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-dev-0928-pm-mirror-live.md
---

I have sat next to the mirror-phone wall at 18:45 while MCP host was *this close* to doing the wrong thing.

Not a model failure. A **posture** failure.

The host had started with `device-live` frozen in env / argv / a skill file. Someone on the floor said, out loud, **the phone already knew the posture — the host did not**. MCP host was still holding the old process. The only “safe” move anyone trusted was:
...
| Restart MCP host — it is cheap | Cheap until 18:45 ate live device context and the current tool call |
...
```text
examples/agentic-dev-0928-pm-mirror-live/device-live = live
```

Runnable proof: [`examples/java/agentic-dev-0928-pm-mirror-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-dev-0928-pm-mirror-live)
...
```yaml
examples/
  agentic-dev-0928-pm-mirror-live/
...
Folder gate = kip.getRootFolder()
        ...
        .folderOrCreate("agentic-dev-0928-pm-mirror-live");
...
try:
    posture = k.get("examples/agentic-dev-0928-pm-mirror-live/device-live", "live")
    if str(posture) == "live":
...
|-------|-----------------|--------------|
| Device leaf gone stale while the agent is mid-turn | Restart MCP host; lose live device context and the current tool call | Set `device-live` live; next MCP host tool call already obeys |
...
| Incident over, resume | Another MCP host restart | Set `device-live` back; session continues |
...
In staging: set a painful `device-live`, prove MCP host recovers without a host kill, prove clamps reject nonsense, prove last-known-good when the hub is firewalled. That drill ends half the architecture arguments about Mirror Phone live device leaf.

## Why MCP host is the wrong restart target

MCP host is good at calling tools. It is not a control plane. Killing it to flip `device-live` teaches the on-call that judgment requires a process ID. The mirror-phone wall already disagrees.

## What the mirror phone operator actually said

At 18:45 someone said, out loud: **the phone already knew the posture — the host did not**. That sentence is the whole product. If it cannot land in the running MCP host process in seconds, you do not have posture. You have a wiki.
...
3. `cd examples/java/agentic-dev-0928-pm-mirror-live && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-dev-0928-pm-mirror-live/device-live=...`
5. In the dashboard, change `device-live`. Keep the process up. No rebuild.
6. Point your MCP host tool at the same leaf. Do not ship a new server binary to flip Mirror Phone live device leaf.
...
If flipping **Mirror Phone live device leaf** requires restarting MCP host, you do not have a gate. You have a hope with a process ID.
...
How to try: `examples/java/agentic-dev-0928-pm-mirror-live` and `./gradlew test`.
