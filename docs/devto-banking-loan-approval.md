...
Retail banking does not wait for your change-management window. Your `approvalScore` did — until it lived in a hub.

## The problem: ceremony between judgment and effect

When **loan approval score** is frozen in a deploy unit, every incident becomes a process argument. Hotfixes still mean CI + roll. Feature flags often mean a second control plane with its own delay.
...
```java
Folder policy = kiponos.path("examples", "banking-loan-approval");
int approvalScore = policy.getInt("approvalScore"); // local get — no hub RTT
if (approvalScore <= 0) {
    return failClosed();
}
return apply(approvalScore);
```

Ops (or automation) sets `approvalScore` in the hub. The **next** evaluation uses it. Same binary. Same tests for structure.
...
- Canonical source: [this article on GitHub](https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-banking-loan-approval.md)
...
Feature flags are often product gates. This essay is about **ops posture** on retail banking: **loan approval score** — numbers war rooms already shout.
...
Treat `approvalScore` as a slice of error budget. Raise when the world is healthy; lower when dependencies are sick. Write the number that survives a bad day.
...
**Loan approval score** that requires a deploy is optimistic documentation.
