---
title: "The Healthcare War Room Already Knew hpaTarget — The Jar Did Not"
published: false
tags: java, healthcare, architecture, kiponos
description: "Live HPA CPU target percent via Kiponos Java SDK — healthcare posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-healthcare-hpa-target-0383.md
main_image: ./devto-cover-ops-posture-healthcare-hpa-target-0383.jpg
---
...
Domain: routing, queue depth, triage. This essay maps hub key `hpaTarget` to **HPA CPU target percent** so the lesson stays concrete for healthcare operators.
...
```java
Folder policy = kiponos.path("examples", "ops-healthcare-hpa-target");
int hpaTarget = policy.getInt("hpaTarget");
if (hpaTarget <= 0) {
    return failClosed(); // money paths
}
return apply(hpaTarget);
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-healthcare-hpa-target/hpaTarget`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: HPA CPU target percent for healthcare — numbers humans already change verbally in war rooms.
