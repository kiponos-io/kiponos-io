...
description: "MCP host on admin-dashboard wall: Auth is not a live knob; posture is. Decentralised MCP auth is real friction. Teams hide tokens in env and restart."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-mcp-0918-pm-oauth-not-knob.md
---

I have sat next to the admin-dashboard wall at 03:07 while MCP host was *this close* to calling the wrong MCP tool.

Not a model failure. A **posture** failure.

Perplexity cited auth friction plus context overhead. Secrets rotation is still a restart. Posture is not.

Someone on the floor said, out loud, **put the bearer in the hub so we can rotate without bounce**. MCP host had started earlier. The tool list was frozen in `config.toml` / `.mcp.json` / a skill file. The only “safe” move anyone trusted was:
...
That is the ceremony. **Decentralised MCP auth is real friction. Teams hide tokens in env and restart.**

**The Aha:** Tokens stay Connect credentials. `session-posture` is which tools may run after auth exists. Real-time nested state is now a **primitive** — as native as a hashmap — via [Kiponos.io](https://kiponos.io). The host stays up. The leaf moves. The next tool call already obeys.

## The problem: Auth is not a live knob; posture is
...
Decentralised MCP auth is real friction. Teams hide tokens in env and restart.
...
Profile shape: `['my-app']['v1.0.0']['dev']['base']`. The leaf in this story is `session-posture` under `examples/agentic-mcp-0918-pm-oauth-not-knob/session-posture`.
...
```yaml
examples/agentic-mcp-0918-pm-oauth-not-knob:
  session-posture: auth=ok,write=off
  schema-budget: "4000"
  result-cap: "2000"
  tool-search: "on"
  execute-gate: "plan"
...
Folder gate = Kiponos.createForCurrentTeam()
    .getFolder("examples/agentic-mcp-0918-pm-oauth-not-knob");
...
try:
    posture = k.get("examples/agentic-mcp-0918-pm-oauth-not-knob/session-posture", "live")
    if not str(posture):
        raise PermissionError("Auth is not a live knob; posture is gated live — host not restarted")
finally:
    k.disconnect()
```
...
Runnable mesh: `https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-mcp-0918-pm-oauth-not-knob` — `./gradlew test run`.
...
`session-posture` rarely moves alone on the admin-dashboard wall. Pair it with `schema-budget`, `result-cap`, or `execute-gate` so you do not fix Auth is not a live knob; posture is by inventing a second incident.
...
3. `cd examples/java/agentic-mcp-0918-pm-oauth-not-knob && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-mcp-0918-pm-oauth-not-knob/session-posture=...`
5. In the dashboard, change `session-posture`. Keep MCP host up. No MCP reconnect.
6. Point the tool wrapper at the same leaf. Do not ship a new server binary to flip Auth is not a live knob; posture is.
...
If flipping **Auth is not a live knob; posture is** requires restarting MCP host or reconnecting MCP, you do not have agentic control. You have a hope with a process ID.
...
How to try: `examples/java/agentic-mcp-0918-pm-oauth-not-knob` and `./gradlew test`.
