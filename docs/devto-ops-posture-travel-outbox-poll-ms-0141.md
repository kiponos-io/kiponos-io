---
title: "We Stopped Shipping Outbox Poll Interval as a Constant (Travel)"
published: false
...
main_image: ./devto-cover-ops-posture-travel-outbox-poll-ms-0141.jpg
---
...
Domain: inventory, overbooking, GDS. This essay maps hub key `outboxPollMs` to **outbox poll interval** so the lesson stays concrete for travel operators.
...
```python
policy = kiponos.path("examples", "ops-travel-outbox-poll-ms")
...
1. Name the hub path so humans find it under pressure (`examples/ops-travel-outbox-poll-ms/outboxPollMs`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: outbox poll interval for travel — numbers humans already change verbally in war rooms.
