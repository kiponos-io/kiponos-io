---
title: "We Stopped Shipping Outbox Poll Interval as a Constant (Cdn)"
published: false
tags: java, networking, devops, kiponos
description: "Live outbox poll interval via Kiponos Java SDK — cdn posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-cdn-outbox-poll-ms-0941.md
main_image: https://files.catbox.moe/to1fi2.jpg
---
...
Domain: edge rules, cache TTL, purge. This essay maps hub key `outboxPollMs` to **outbox poll interval** so the lesson stays concrete for cdn operators.
...
```java
Folder policy = kiponos.path("examples", "ops-cdn-outbox-poll-ms");
...
1. Name the hub path so humans find it under pressure (`examples/ops-cdn-outbox-poll-ms/outboxPollMs`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: outbox poll interval for cdn — numbers humans already change verbally in war rooms.
