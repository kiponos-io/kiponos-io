---
title: "Graphql Query Depth Should Not Wait for a Redeploy — Iot Live Posture"
published: false
tags: java, iot, devops, kiponos
description: "Live GraphQL query depth via Kiponos Python SDK — iot posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-iot-graphql-depth-0500.md
main_image: ./devto-cover-ops-posture-iot-graphql-depth-0500.jpg
---
...
Domain: device firmware gates, telemetry sample. This essay maps hub key `maxDepth` to **GraphQL query depth** so the lesson stays concrete for iot operators.
...
```python
policy = kiponos.path("examples", "ops-iot-graphql-depth")
maxDepth = int(policy.get("maxDepth", 8))
if not limiter.try_acquire(maxDepth):
    return too_many()
return handle(req)
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-iot-graphql-depth/maxDepth`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: GraphQL query depth for iot — numbers humans already change verbally in war rooms.

Kiponos makes that verbal decision **executable** without a second control plane tax on every request.

## Failure budget vs this dial

Treat `maxDepth` as a slice of **error budget**, not a comfort blanket. Raise it when the dependency is healthy; lower it when the dependency is already sick. Write the number that survives a bad day, not the number that flatters a sunny demo.

## Pair with a sister dial

`maxDepth` rarely moves alone. Pair with timeout/retry, canary share, or sampling so you do not fix one symptom by creating another.
