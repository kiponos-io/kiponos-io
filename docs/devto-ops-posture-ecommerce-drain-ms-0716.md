---
title: "We Stopped Shipping Graceful Drain Window as a Constant (Ecommerce)"
published: false
...
Domain: cart, promo weights, stock. This essay maps hub key `gracefulShutdownMs` to **graceful drain window** so the lesson stays concrete for ecommerce operators.
...
```python
policy = kiponos.path("examples", "ops-ecommerce-drain-ms")
gracefulShutdownMs = min(int(policy.get("gracefulShutdownMs", 15000)), HARD_MAX)
worker_pool.resize(gracefulShutdownMs)  # or admission semaphore
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-ecommerce-drain-ms/gracefulShutdownMs`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: graceful drain window for ecommerce — numbers humans already change verbally in war rooms.
