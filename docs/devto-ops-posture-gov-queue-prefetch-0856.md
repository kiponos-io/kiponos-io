---
title: "We Stopped Shipping Consumer Prefetch as a Constant (Gov)"
published: false
...
main_image: https://iili.io/n20Y8pj.jpg
---
...
Domain: form windows, batch cutoffs. This essay maps hub key `prefetch` to **consumer prefetch** so the lesson stays concrete for gov operators.
...
```java
Folder policy = kiponos.path("examples", "ops-gov-queue-prefetch");
...
1. Name the hub path so humans find it under pressure (`examples/ops-gov-queue-prefetch/prefetch`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: consumer prefetch for gov — numbers humans already change verbally in war rooms.
