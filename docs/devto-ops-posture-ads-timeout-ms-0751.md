---
title: "We Stopped Shipping Outbound Call Timeout as a Constant (Ads)"
published: false
...
Domain: bid caps, frequency, pacing. This essay maps hub key `timeoutMs` to **outbound call timeout** so the lesson stays concrete for ads operators.
...
```java
Folder policy = kiponos.path("examples", "ops-ads-timeout-ms");
...
1. Name the hub path so humans find it under pressure (`examples/ops-ads-timeout-ms/timeoutMs`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: outbound call timeout for ads — numbers humans already change verbally in war rooms.
