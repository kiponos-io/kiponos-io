---
title: "The Logistics War Room Already Knew hpaTarget — The Jar Did Not"
published: false
tags: java, logistics, devops, kiponos
description: "Live HPA CPU target percent via Kiponos Python SDK — logistics posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-logistics-hpa-target-0333.md
main_image: ./devto-cover-ops-posture-logistics-hpa-target-0333.jpg
---

**The Aha:** `hpaTarget` is not a property file trophy. It is **incident posture** — and posture that waits for a jar is already late.

The partner brownout did not wait for your change-management window. Your HPA CPU target percent did — until it lived in a hub.

Domain: routing, SLA windows, fleet. This essay maps hub key `hpaTarget` to **HPA CPU target percent** so the lesson stays concrete for logistics operators.
...
```python
policy = kiponos.path("examples", "ops-logistics-hpa-target")
...
1. Name the hub path so humans find it under pressure (`examples/ops-logistics-hpa-target/hpaTarget`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: HPA CPU target percent for logistics — numbers humans already change verbally in war rooms.
