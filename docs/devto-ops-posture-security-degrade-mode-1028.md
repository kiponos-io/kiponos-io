---
title: "The Security War Room Already Knew degradeMode — The Jar Did Not"
published: false
...
Domain: WAF, RPS, auth leeway. This essay maps hub key `degradeMode` to **degradation mode** so the lesson stays concrete for security operators.
...
```python
policy = kiponos.path("examples", "ops-security-degrade-mode")
...
1. Name the hub path so humans find it under pressure (`examples/ops-security-degrade-mode/degradeMode`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: degradation mode for security — numbers humans already change verbally in war rooms.
