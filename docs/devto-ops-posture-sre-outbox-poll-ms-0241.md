---
title: "We Stopped Shipping Outbox Poll Interval as a Constant (Sre)"
published: false
...
The partner brownout did not wait for your change-management window. Your outbox poll interval did — until it lived in a hub.

Domain: canary, drain, error budget. This essay maps hub key `outboxPollMs` to **outbox poll interval** so the lesson stays concrete for sre operators.
...
```python
policy = kiponos.path("examples", "ops-sre-outbox-poll-ms")
...
1. Name the hub path so humans find it under pressure (`examples/ops-sre-outbox-poll-ms/outboxPollMs`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: outbox poll interval for sre — numbers humans already change verbally in war rooms.
