---
title: "Live Retry Budget for Sre — Mouth to Process in Seconds"
published: false
...
main_image: ./devto-cover-ops-posture-sre-retry-max-0202.jpg
---
...
Domain: canary, drain, error budget. This essay maps hub key `maxAttempts` to **retry budget** so the lesson stays concrete for sre operators.
...
```java
var policy = kiponos.path("examples", "ops-sre-retry-max");
...
1. Name the hub path so humans find it under pressure (`examples/ops-sre-retry-max/maxAttempts`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: retry budget for sre — numbers humans already change verbally in war rooms.
