...
description: "Claude Code on shopping-admin wall: Token budget as a live leaf after schemas ate the sandwich. Budget lived in host argv. Schemas spent it before the user ques"
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-mcp-0921-pm-token-midturn.md
---

I have sat next to the shopping-admin wall at 00:40 while Claude Code was *this close* to calling the wrong MCP tool.

Not a model failure. A **posture** failure.

143k of 200k gone at t=0. Your '8k turn budget' is a lie unless schemas are live-thinned.

Someone on the floor said, out loud, **raise max-tokens and restart**. Claude Code had started earlier. The tool list was frozen in `config.toml` / `.mcp.json` / a skill file. The only “safe” move anyone trusted was:
...
That is the ceremony. **Budget lived in host argv. Schemas spent it before the user question.**

**The Aha:** Two leaves: `schema-budget` and `max-tokens`. Both `get()` on the hot path. Real-time nested state is now a **primitive** — as native as a hashmap — via [Kiponos.io](https://kiponos.io). The host stays up. The leaf moves. The next tool call already obeys.

## The problem: Token budget as a live leaf after schemas ate the sandwich
...
Budget lived in host argv. Schemas spent it before the user question.
...
Profile shape: `['my-app']['v1.0.0']['dev']['base']`. The leaf in this story is `max-tokens` under `examples/agentic-mcp-0921-pm-token-midturn/max-tokens`.

## Architecture

```mermaid
flowchart LR
  wall[shopping-admin wall] -->|set max-tokens| hub[Kiponos hub]
  hub -->|delta WS| gb[Claude Code process]
  gb -->|get max-tokens| tool[MCP / Skill / tool wrapper]
  tool -->|allow or deny| world[External system]
```

The model still calls tools. The wrapper reads `max-tokens` **before** the call. No schema dump policy in argv.

## Config tree

```yaml
examples/agentic-mcp-0921-pm-token-midturn:
  max-tokens: 8000
  schema-budget: "4000"
  result-cap: "2000"
  tool-search: "on"
...
Eight-plus keys when you pair `max-tokens` with sister dials (budget, mute, pause). They move together on the shopping-admin wall.
...
Folder gate = Kiponos.createForCurrentTeam()
    .getFolder("examples/agentic-mcp-0921-pm-token-midturn");
String roster = String.valueOf(gate.get("max-tokens"));
...
try:
    posture = k.get("examples/agentic-mcp-0921-pm-token-midturn/max-tokens", "live")
    if not str(posture):
        raise PermissionError("Token budget as a live leaf after schemas ate the sandwich gated live — host not restarted")
finally:
    k.disconnect()
```
...
Runnable mesh: `https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-mcp-0921-pm-token-midturn` — `./gradlew test run`.
...
| User says “no writes” mid-turn | Restart Claude Code | Set `max-tokens`; next call already denies |
...
- A dashboard edit is a **delta** of `max-tokens`, not a config.toml reload.
...
`max-tokens` rarely moves alone on the shopping-admin wall. Pair it with `schema-budget`, `result-cap`, or `execute-gate` so you do not fix Token budget as a live leaf after schemas ate the sandwich by inventing a second incident.
...
3. `cd examples/java/agentic-mcp-0921-pm-token-midturn && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-mcp-0921-pm-token-midturn/max-tokens=...`
5. In the dashboard, change `max-tokens`. Keep Claude Code up. No MCP reconnect.
6. Point the tool wrapper at the same leaf. Do not ship a new server binary to flip Token budget as a live leaf after schemas ate the sandwich.
...
If flipping **Token budget as a live leaf after schemas ate the sandwich** requires restarting Claude Code or reconnecting MCP, you do not have agentic control. You have a hope with a process ID.
...
How to try: `examples/java/agentic-mcp-0921-pm-token-midturn` and `./gradlew test`.
