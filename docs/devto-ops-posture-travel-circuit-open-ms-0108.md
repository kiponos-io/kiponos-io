---
title: "The Travel War Room Already Knew waitOpenMs — The Jar Did Not"
published: false
...
Domain: inventory, overbooking, GDS. This essay maps hub key `waitOpenMs` to **circuit open duration** so the lesson stays concrete for travel operators.
...
```python
policy = kiponos.path("examples", "ops-travel-circuit-open-ms")
...
1. Name the hub path so humans find it under pressure (`examples/ops-travel-circuit-open-ms/waitOpenMs`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: circuit open duration for travel — numbers humans already change verbally in war rooms.
