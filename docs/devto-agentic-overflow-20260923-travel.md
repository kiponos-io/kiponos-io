---
main_image: https://iili.io/nuiEi57.jpg
...
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-overflow-20260923-travel.md
---

I have sat next to the travel-coordinator wall at 03:07 while Cursor was *this close* to doing the wrong thing.
...
**The Aha:** MCP tool gates as live posture on the travel wall is not a binary you reboot. It is a **function that should read live posture** from [Kiponos.io](https://kiponos.io) on every call. The host stays up. The leaf moves.

## The problem: MCP tool gates as live posture on the travel wall lived in the process, not in the turn

Cursor is good at *calling* tools. It is not born with a **shared, instant, restart-free control plane**.

So teams hide MCP tool gates as live posture on the travel wall in the only places agent frameworks actually ship:
...
| Restart Cursor — it is cheap | Cheap until 03:07 ate the overbook thread the agent already paid for |
...
```text
examples/mcp-tool-gate-live/tools-allow = search,read
```

Runnable proof: [`examples/java/mcp-tool-gate-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/mcp-tool-gate-live)
...
examples/
  mcp-tool-gate-live/
    tools-allow: search,read          # MCP tool gates as live posture on the travel wall
apps/
  travel/
    live:
...
try:
    posture = k.get("examples/mcp-tool-gate-live/tools-allow", "search,read")
    if str(posture) == "search,read":
        raise PermissionError("MCP tool gates as live posture on the travel wall gated live — host not restarted")
finally:
    k.disconnect()
```
...
At 03:07 someone said, out loud: **mute writes on the noisy room — keep reads**. That sentence is the whole product. If it cannot land in the running Cursor process in seconds, you do not have posture. You have a wiki.
...
3. `cd examples/java/mcp-tool-gate-live && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/mcp-tool-gate-live/tools-allow=...`
5. In the dashboard, change `tools-allow`. Keep the process up. No rebuild.
6. Point your Cursor tool at the same leaf. Do not ship a new server binary to flip MCP tool gates as live posture on the travel wall.
...
If flipping **MCP tool gates as live posture on the travel wall** requires restarting Cursor, you do not have a gate. You have a hope with a process ID.
...
How to try: `examples/java/mcp-tool-gate-live` and `./gradlew test`.
