---
main_image: https://iili.io/Cy3Rias.jpg
...
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-dev-0918-am-agent-folder.md
---

I have sat next to the senses wall at 11:41 while Claude Code was *this close* to doing the wrong thing.
...
| Restart Claude Code — it is cheap | Cheap until 11:41 ate the turn budget and the open investigation |
...
```text
examples/agentic-dev-0918-am-agent-folder/owner-agent = live
```

Runnable proof: [`examples/java/agentic-dev-0918-am-agent-folder`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-dev-0918-am-agent-folder)
...
```yaml
examples/
  agentic-dev-0918-am-agent-folder/
...
Folder gate = kip.getRootFolder()
        ...
        .folderOrCreate("agentic-dev-0918-am-agent-folder");
...
try:
    posture = k.get("examples/agentic-dev-0918-am-agent-folder/owner-agent", "live")
    if str(posture) == "live":
...
|----------|------------|------------------------|
| Env file + Claude Code reboot | Simple at 09:00 | The freeze is at 11:41 |
...
In staging: set a painful `owner-agent`, prove Claude Code recovers without a host kill, prove clamps reject nonsense, prove last-known-good when the hub is firewalled. That drill ends half the architecture arguments about Agents choose a metadata folder.
...
At 11:41 someone said, out loud: **priority is a leaf, not a restart**. That sentence is the whole product. If it cannot land in the running Claude Code process in seconds, you do not have posture. You have a wiki.
...
3. `cd examples/java/agentic-dev-0918-am-agent-folder && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-dev-0918-am-agent-folder/owner-agent=...`
...
How to try: `examples/java/agentic-dev-0918-am-agent-folder` and `./gradlew test`.
