---
title: "Sre: Turn cache TTL seconds Into Incident Posture"
published: false
...
main_image: https://iili.io/Ctvq1R4.jpg
---
...
Domain: canary, drain, error budget. This essay maps hub key `ttlSec` to **cache TTL seconds** so the lesson stays concrete for sre operators.
...
```java
var policy = kiponos.path("examples", "ops-sre-cache-ttl-sec");
...
1. Name the hub path so humans find it under pressure (`examples/ops-sre-cache-ttl-sec/ttlSec`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: cache TTL seconds for sre — numbers humans already change verbally in war rooms.
