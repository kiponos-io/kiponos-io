---
main_image: https://iili.io/ndeKqiJ.jpg
...
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-dev-1028-pm-mirror-live.md
---

I have sat next to the mirror-phone wall at 11:41 while Cursor was *this close* to doing the wrong thing.
...
| Restart Cursor — it is cheap | Cheap until 11:41 ate live device context and the current tool call |
...
```text
examples/agentic-dev-1028-pm-mirror-live/device-live = live
```

Runnable proof: [`examples/java/agentic-dev-1028-pm-mirror-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-dev-1028-pm-mirror-live)
...
```yaml
examples/
  agentic-dev-1028-pm-mirror-live/
...
Folder gate = kip.getRootFolder()
        ...
        .folderOrCreate("agentic-dev-1028-pm-mirror-live");
...
try:
    posture = k.get("examples/agentic-dev-1028-pm-mirror-live/device-live", "live")
    if str(posture) == "live":
...
In staging: set a painful `device-live`, prove Cursor recovers without a host kill, prove clamps reject nonsense, prove last-known-good when the hub is firewalled. That drill ends half the architecture arguments about Mirror Phone live device leaf.
...
At 11:41 someone said, out loud: **the phone already knew the posture — the host did not**. That sentence is the whole product. If it cannot land in the running Cursor process in seconds, you do not have posture. You have a wiki.
...
3. `cd examples/java/agentic-dev-1028-pm-mirror-live && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-dev-1028-pm-mirror-live/device-live=...`
...
How to try: `examples/java/agentic-dev-1028-pm-mirror-live` and `./gradlew test`.
