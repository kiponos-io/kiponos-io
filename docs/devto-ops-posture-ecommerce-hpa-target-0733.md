---
title: "The Ecommerce War Room Already Knew hpaTarget — The Jar Did Not"
published: false
tags: java, ecommerce, devops, kiponos
description: "Live HPA CPU target percent via Kiponos Java SDK — ecommerce posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-ecommerce-hpa-target-0733.md
main_image: https://files.catbox.moe/bv8now.jpg
---

**The Aha:** `hpaTarget` is not a property file trophy. It is **incident posture** — and posture that waits for a jar is already late.

SEV-2 started as a metric spike and became a ceremony debate: who can change hpaTarget without a PR.

Domain: cart, promo weights, stock. This essay maps hub key `hpaTarget` to **HPA CPU target percent** so the lesson stays concrete for ecommerce operators.
...
```java
Folder policy = kiponos.path("examples", "ops-ecommerce-hpa-target");
...
1. Name the hub path so humans find it under pressure (`examples/ops-ecommerce-hpa-target/hpaTarget`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: HPA CPU target percent for ecommerce — numbers humans already change verbally in war rooms.
