---
title: "Command as a Super Pattern — Live Dispatch Map"
published: false
tags: java, designpatterns, architecture, devops
description: "Which command id runs is live config"
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-pattern-command-live-dispatch.md
main_image: ./devto-cover-pattern-command-live-dispatch.jpg
...
**The Aha:** Which command id runs is live config

Ops needs to disable a dangerous command or remap an alias without shipping a jar full of ifs.
...
patterns/command/dispatch/
  # live knobs
  enabled: yes; aliases: retry=requeue
```

Hot path (local memory after connect — WebSocket deltas, no per-request hub RTT):

```java
Folder policy = kiponos.path('patterns', 'command', 'dispatch');
// local gets — never remote on the money path
if (!enabled) throw new Disabled();
Command c = registry.get(resolveAlias(name));
c.execute(ctx);
```
...
Full tree: [https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/pattern-command-live-dispatch](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/pattern-command-live-dispatch)
...
Commands are code. Dispatch is judgment.
...
*Runnable: [pattern-command-live-dispatch](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/pattern-command-live-dispatch) · [kiponos.io](https://kiponos.io)*
