---
title: "Ads: Turn circuit trip count Into Incident Posture"
published: false
...
Domain: bid caps, frequency, pacing. This essay maps hub key `failureThreshold` to **circuit trip count** so the lesson stays concrete for ads operators.
...
```java
var policy = kiponos.path("examples", "ops-ads-circuit-failures");
...
1. Name the hub path so humans find it under pressure (`examples/ops-ads-circuit-failures/failureThreshold`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: circuit trip count for ads — numbers humans already change verbally in war rooms.
