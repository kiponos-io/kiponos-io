---
title: "Graphql Query Depth Should Not Wait for a Redeploy — Ads Live Posture"
published: false
tags: java, advertising, devops, kiponos
description: "Live GraphQL query depth via Kiponos Python SDK — ads posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-ads-graphql-depth-0800.md
main_image: https://files.catbox.moe/aatoed.jpg
---

**The Aha:** `maxDepth` is not a property file trophy. It is **incident posture** — and posture that waits for a jar is already late.

SEV-2 started as a metric spike and became a ceremony debate: who can change maxDepth without a PR.

Domain: bid caps, frequency, pacing. This essay maps hub key `maxDepth` to **GraphQL query depth** so the lesson stays concrete for ads operators.
...
```python
policy = kiponos.path("examples", "ops-ads-graphql-depth")
...
1. Name the hub path so humans find it under pressure (`examples/ops-ads-graphql-depth/maxDepth`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: GraphQL query depth for ads — numbers humans already change verbally in war rooms.
