---
title: "Media: Turn partner API timeout Into Incident Posture"
published: false
...
main_image: https://iili.io/nx24Gr7.jpg
---
...
Domain: bitrate, CDN, origin. This essay maps hub key `partnerTimeoutMs` to **partner API timeout** so the lesson stays concrete for media operators.
...
```java
Folder policy = kiponos.path("examples", "ops-media-partner-timeout");
...
1. Name the hub path so humans find it under pressure (`examples/ops-media-partner-timeout/partnerTimeoutMs`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: partner API timeout for media — numbers humans already change verbally in war rooms.
