---
title: "The Banking War Room Already Knew backoffMs — The Jar Did Not"
published: false
...
Domain: AML thresholds, transfer velocity. This essay maps hub key `backoffMs` to **retry backoff base** so the lesson stays concrete for banking operators.
...
```java
var policy = kiponos.path("examples", "ops-banking-backoff-ms");
...
1. Name the hub path so humans find it under pressure (`examples/ops-banking-backoff-ms/backoffMs`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: retry backoff base for banking — numbers humans already change verbally in war rooms.
