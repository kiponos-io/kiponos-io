---
title: "We Stopped Shipping Shadow Traffic Share as a Constant (Security)"
published: false
tags: java, security, devops, kiponos
description: "Live shadow traffic share via Kiponos Java SDK — security posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-security-shadow-percent-1011.md
main_image: ./devto-cover-ops-posture-security-shadow-percent-1011.jpg
---
...
SEV-2 started as a metric spike and became a ceremony debate: who can change shadowPercent without a PR.

Domain: WAF, RPS, auth leeway. This essay maps hub key `shadowPercent` to **shadow traffic share** so the lesson stays concrete for security operators.
...
```java
Folder policy = kiponos.path("examples", "ops-security-shadow-percent");
int shadowPercent = policy.getInt("shadowPercent");
if (shadowPercent <= 0) {
    return failClosed(); // money paths
}
return apply(shadowPercent);
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-security-shadow-percent/shadowPercent`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: shadow traffic share for security — numbers humans already change verbally in war rooms.
...
`shadowPercent` rarely moves alone. Pair with timeout/retry, canary share, or sampling so you do not fix one symptom by creating another.
