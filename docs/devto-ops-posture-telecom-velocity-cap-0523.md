---
title: "The Telecom War Room Already Knew velocityCap — The Jar Did Not"
published: false
...
main_image: https://iili.io/CD71LoN.jpg
---
...
Domain: QoS, rate classes, trunk limits. This essay maps hub key `velocityCap` to **fraud velocity cap** so the lesson stays concrete for telecom operators.
...
```python
policy = kiponos.path("examples", "ops-telecom-velocity-cap")
...
1. Name the hub path so humans find it under pressure (`examples/ops-telecom-velocity-cap/velocityCap`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: fraud velocity cap for telecom — numbers humans already change verbally in war rooms.
