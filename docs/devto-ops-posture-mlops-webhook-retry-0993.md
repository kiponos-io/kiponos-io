---
title: "The Mlops War Room Already Knew webhookMaxAttempts — The Jar Did Not"
published: false
...
main_image: https://iili.io/nImJ342.jpg
---
...
Domain: batch size, score thresholds. This essay maps hub key `webhookMaxAttempts` to **webhook retry budget** so the lesson stays concrete for mlops operators.
...
```java
Folder policy = kiponos.path("examples", "ops-mlops-webhook-retry");
...
1. Name the hub path so humans find it under pressure (`examples/ops-mlops-webhook-retry/webhookMaxAttempts`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: webhook retry budget for mlops — numbers humans already change verbally in war rooms.
