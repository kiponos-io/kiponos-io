...
Multi-tenant SaaS does not wait for your change-management window. Your `entitlement` did — until it lived in a hub.

## The problem: ceremony between judgment and effect

When **tenant entitlement** is frozen in a deploy unit, every incident becomes a process argument. Hotfixes still mean CI + roll. Feature flags often mean a second control plane with its own delay.
...
```java
Folder policy = kiponos.path("examples", "saas-multitenant");
int entitlement = policy.getInt("entitlement"); // local get — no hub RTT
if (entitlement <= 0) {
    return failClosed();
}
return apply(entitlement);
```

Ops (or automation) sets `entitlement` in the hub. The **next** evaluation uses it. Same binary. Same tests for structure.
...
- Canonical source: [this article on GitHub](https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-saas-multitenant.md)
...
Feature flags are often product gates. This essay is about **ops posture** on multi-tenant SaaS: **tenant entitlement** — numbers war rooms already shout.
...
Treat `entitlement` as a slice of error budget. Raise when the world is healthy; lower when dependencies are sick. Write the number that survives a bad day.
...
**Tenant entitlement** that requires a deploy is optimistic documentation.
