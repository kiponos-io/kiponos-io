---
title: "The Insurance War Room Already Knew logSamplePerThousand — The Jar Did Not"
published: false
tags: java, insurance, architecture, kiponos
description: "Live log sample per thousand via Kiponos Python SDK — insurance posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-insurance-log-sample-0613.md
main_image: ./devto-cover-ops-posture-insurance-log-sample-0613.jpg
---
...
Domain: underwriting, claims velocity. This essay maps hub key `logSamplePerThousand` to **log sample per thousand** so the lesson stays concrete for insurance operators.
...
```python
policy = kiponos.path("examples", "ops-insurance-log-sample")
...
1. Name the hub path so humans find it under pressure (`examples/ops-insurance-log-sample/logSamplePerThousand`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: log sample per thousand for insurance — numbers humans already change verbally in war rooms.
...
Treat `logSamplePerThousand` as a slice of **error budget**, not a comfort blanket. Raise it when the dependency is healthy; lower it when the dependency is already sick. Write the number that survives a bad day, not the number that flatters a sunny demo.

## Pair with a sister dial

`logSamplePerThousand` rarely moves alone. Pair with timeout/retry, canary share, or sampling so you do not fix one symptom by creating another.
