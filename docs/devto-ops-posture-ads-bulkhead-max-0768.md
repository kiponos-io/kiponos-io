---
title: "The Ads War Room Already Knew bulkheadMax — The Jar Did Not"
published: false
...
Domain: bid caps, frequency, pacing. This essay maps hub key `bulkheadMax` to **bulkhead concurrency** so the lesson stays concrete for ads operators.
...
```python
policy = kiponos.path("examples", "ops-ads-bulkhead-max")
...
1. Name the hub path so humans find it under pressure (`examples/ops-ads-bulkhead-max/bulkheadMax`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: bulkhead concurrency for ads — numbers humans already change verbally in war rooms.
