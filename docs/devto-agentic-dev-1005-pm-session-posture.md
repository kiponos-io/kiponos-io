---
main_image: https://iili.io/n9AWMba.jpg
...
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-dev-1005-pm-session-posture.md
---

I have sat next to the senses wall at 14:12 while Claude Code was *this close* to doing the wrong thing.
...
| Restart Claude Code — it is cheap | Cheap until 14:12 ate the turn budget and the open investigation |
...
```text
examples/agentic-dev-1005-pm-session-posture/session-posture = shared
```

Runnable proof: [`examples/java/agentic-dev-1005-pm-session-posture`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-dev-1005-pm-session-posture)
...
```yaml
examples/
  agentic-dev-1005-pm-session-posture/
...
Folder gate = kip.getRootFolder()
        ...
        .folderOrCreate("agentic-dev-1005-pm-session-posture");
...
try:
    posture = k.get("examples/agentic-dev-1005-pm-session-posture/session-posture", "shared")
...
Claude Code is good at calling tools. It is not a control plane. Killing it to flip `session-posture` teaches the on-call that judgment requires a process ID. The senses wall already disagrees.

## What the senses operator actually said

At 14:12 someone said, out loud: **priority is a leaf, not a restart**. That sentence is the whole product. If it cannot land in the running Claude Code process in seconds, you do not have posture. You have a wiki.
...
3. `cd examples/java/agentic-dev-1005-pm-session-posture && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-dev-1005-pm-session-posture/session-posture=...`
...
How to try: `examples/java/agentic-dev-1005-pm-session-posture` and `./gradlew test`.
