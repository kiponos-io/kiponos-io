---
title: "Ecommerce: Turn search scroll timeout Into Incident Posture"
published: false
...
Domain: cart, promo weights, stock. This essay maps hub key `scrollTimeoutSec` to **search scroll timeout** so the lesson stays concrete for ecommerce operators.
...
```java
Folder policy = kiponos.path("examples", "ops-ecommerce-scroll-timeout");
...
1. Name the hub path so humans find it under pressure (`examples/ops-ecommerce-scroll-timeout/scrollTimeoutSec`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: search scroll timeout for ecommerce — numbers humans already change verbally in war rooms.
