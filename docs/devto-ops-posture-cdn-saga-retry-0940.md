---
title: "Saga Step Retry Budget Should Not Wait for a Redeploy — Cdn Live Posture"
published: false
tags: java, networking, devops, kiponos
description: "Live saga step retry budget via Kiponos Python SDK — cdn posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-cdn-saga-retry-0940.md
main_image: https://files.catbox.moe/9fdnkf.jpg
---
...
Domain: edge rules, cache TTL, purge. This essay maps hub key `sagaMaxAttempts` to **saga step retry budget** so the lesson stays concrete for cdn operators.
...
```python
policy = kiponos.path("examples", "ops-cdn-saga-retry")
...
1. Name the hub path so humans find it under pressure (`examples/ops-cdn-saga-retry/sagaMaxAttempts`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: saga step retry budget for cdn — numbers humans already change verbally in war rooms.
