---
main_image: https://iili.io/CpuZIHJ.jpg
title: "Grok Build Finished the Turn Blind — Session posture shared across hosts on the Senses Path"
published: false
tags: java, python, devops, ai, kiponos
description: "Grok Build kept finishing the turn blind on senses wall. Session posture shared across hosts is a live hub leaf."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-dev-0905-pm-session-posture.md
---

I have sat next to the senses wall at 02:14 while Grok Build was *this close* to doing the wrong thing.

Not a model failure. A **posture** failure.

The host had started with `session-posture` frozen in env / argv / a skill file. Someone on the floor said, out loud, **priority is a leaf, not a restart**. Grok Build was still holding the old process. The only “safe” move anyone trusted was:
...
```text
examples/agentic-dev-0905-pm-session-posture/session-posture = shared
```

Runnable proof: [`examples/java/agentic-dev-0905-pm-session-posture`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-dev-0905-pm-session-posture)
...
```yaml
examples/
  agentic-dev-0905-pm-session-posture/
...
Folder gate = kip.getRootFolder()
        ...
        .folderOrCreate("agentic-dev-0905-pm-session-posture");
...
try:
    posture = k.get("examples/agentic-dev-0905-pm-session-posture/session-posture", "shared")
...
|-------|-----------------|--------------|
| A mid-turn interrupt the host treats as noise | Restart Grok Build; lose the turn budget and the open investigation | Set `session-posture` live; next Grok Build tool call already obeys |
...
| Incident over, resume | Another Grok Build restart | Set `session-posture` back; session continues |
...
In staging: set a painful `session-posture`, prove Grok Build recovers without a host kill, prove clamps reject nonsense, prove last-known-good when the hub is firewalled. That drill ends half the architecture arguments about Session posture shared across hosts.

## Why Grok Build is the wrong restart target

Grok Build is good at calling tools. It is not a control plane. Killing it to flip `session-posture` teaches the on-call that judgment requires a process ID. The senses wall already disagrees.

## What the senses operator actually said

At 02:14 someone said, out loud: **priority is a leaf, not a restart**. That sentence is the whole product. If it cannot land in the running Grok Build process in seconds, you do not have posture. You have a wiki.
...
3. `cd examples/java/agentic-dev-0905-pm-session-posture && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-dev-0905-pm-session-posture/session-posture=...`
5. In the dashboard, change `session-posture`. Keep the process up. No rebuild.
6. Point your Grok Build tool at the same leaf. Do not ship a new server binary to flip Session posture shared across hosts.
...
If flipping **Session posture shared across hosts** requires restarting Grok Build, you do not have a gate. You have a hope with a process ID.
...
How to try: `examples/java/agentic-dev-0905-pm-session-posture` and `./gradlew test`.
