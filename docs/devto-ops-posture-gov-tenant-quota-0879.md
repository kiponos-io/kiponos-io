---
title: "Gov: Turn per-tenant quota Into Incident Posture"
published: false
tags: java, architecture, devops, kiponos
description: "Live per-tenant quota via Kiponos Java SDK — gov posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-gov-tenant-quota-0879.md
main_image: ./devto-cover-ops-posture-gov-tenant-quota-0879.jpg
---
...
Domain: form windows, batch cutoffs. This essay maps hub key `quotaPerMin` to **per-tenant quota** so the lesson stays concrete for gov operators.
...
```java
Folder policy = kiponos.path("examples", "ops-gov-tenant-quota");
...
1. Name the hub path so humans find it under pressure (`examples/ops-gov-tenant-quota/quotaPerMin`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: per-tenant quota for gov — numbers humans already change verbally in war rooms.
