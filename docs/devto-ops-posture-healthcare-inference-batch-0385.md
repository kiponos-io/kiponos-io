---
title: "Inference Batch Size Should Not Wait for a Redeploy — Healthcare Live Posture"
published: false
tags: java, healthcare, architecture, kiponos
description: "Live inference batch size via Kiponos Java SDK — healthcare posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-healthcare-inference-batch-0385.md
main_image: ./devto-cover-ops-posture-healthcare-inference-batch-0385.jpg
---
...
Domain: routing, queue depth, triage. This essay maps hub key `batchSize` to **inference batch size** so the lesson stays concrete for healthcare operators.
...
```yaml
examples/
  ops-healthcare-inference-batch/
...
```java
var policy = kiponos.path("examples", "ops-healthcare-inference-batch");
...
1. Name the hub path so humans find it under pressure (`examples/ops-healthcare-inference-batch/batchSize`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: inference batch size for healthcare — numbers humans already change verbally in war rooms.
