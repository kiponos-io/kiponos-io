---
title: "Saga Step Retry Budget Should Not Wait for a Redeploy — Insurance Live Posture"
published: false
tags: java, insurance, architecture, kiponos
description: "Live saga step retry budget via Kiponos Java SDK — insurance posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-insurance-saga-retry-0640.md
main_image: ./devto-cover-ops-posture-insurance-saga-retry-0640.jpg
---
...
Domain: underwriting, claims velocity. This essay maps hub key `sagaMaxAttempts` to **saga step retry budget** so the lesson stays concrete for insurance operators.
...
```java
var policy = kiponos.path("examples", "ops-insurance-saga-retry");
String mode = policy.getString("degradeMode", "full");
int sagaMaxAttempts = policy.getInt("sagaMaxAttempts");
return router.decide(mode, sagaMaxAttempts);
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-insurance-saga-retry/sagaMaxAttempts`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: saga step retry budget for insurance — numbers humans already change verbally in war rooms.
