---
title: "We Stopped Shipping Consumer Prefetch as a Constant (Banking)"
published: false
...
Domain: AML thresholds, transfer velocity. This essay maps hub key `prefetch` to **consumer prefetch** so the lesson stays concrete for banking operators.
...
```python
policy = kiponos.path("examples", "ops-banking-queue-prefetch")
...
1. Name the hub path so humans find it under pressure (`examples/ops-banking-queue-prefetch/prefetch`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: consumer prefetch for banking — numbers humans already change verbally in war rooms.
