---
title: "Saga Step Retry Budget Should Not Wait for a Redeploy — Banking Live Posture"
published: false
tags: java, banking, security, kiponos
description: "Live saga step retry budget via Kiponos Python SDK — banking posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-banking-saga-retry-0690.md
main_image: https://litter.catbox.moe/vzgi3c.jpg
---
...
Domain: AML thresholds, transfer velocity. This essay maps hub key `sagaMaxAttempts` to **saga step retry budget** so the lesson stays concrete for banking operators.
...
```python
policy = kiponos.path("examples", "ops-banking-saga-retry")
sagaMaxAttempts = int(policy.get("sagaMaxAttempts", 4))
if not limiter.try_acquire(sagaMaxAttempts):
    return too_many()
return handle(req)
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-banking-saga-retry/sagaMaxAttempts`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: saga step retry budget for banking — numbers humans already change verbally in war rooms.
...
2. Move `sagaMaxAttempts` to the emergency value (documented floor/ceiling).  
...
saga step retry budget that requires a deploy is optimistic documentation.
