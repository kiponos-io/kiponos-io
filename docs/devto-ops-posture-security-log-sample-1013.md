---
title: "The Security War Room Already Knew logSamplePerThousand — The Jar Did Not"
published: false
...
Domain: WAF, RPS, auth leeway. This essay maps hub key `logSamplePerThousand` to **log sample per thousand** so the lesson stays concrete for security operators.
...
```java
Folder policy = kiponos.path("examples", "ops-security-log-sample");
...
1. Name the hub path so humans find it under pressure (`examples/ops-security-log-sample/logSamplePerThousand`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: log sample per thousand for security — numbers humans already change verbally in war rooms.
