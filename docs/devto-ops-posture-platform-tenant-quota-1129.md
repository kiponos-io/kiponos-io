---
title: "Platform: Turn per-tenant quota Into Incident Posture"
published: false
tags: java, platform, architecture, kiponos
description: "Live per-tenant quota via Kiponos Java SDK — platform posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-platform-tenant-quota-1129.md
main_image: ./devto-cover-ops-posture-platform-tenant-quota-1129.jpg
---
...
Domain: shared libraries, defaults. This essay maps hub key `quotaPerMin` to **per-tenant quota** so the lesson stays concrete for platform operators.
...
```java
Folder policy = kiponos.path("examples", "ops-platform-tenant-quota");
...
1. Name the hub path so humans find it under pressure (`examples/ops-platform-tenant-quota/quotaPerMin`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: per-tenant quota for platform — numbers humans already change verbally in war rooms.
