---
title: "The Saas War Room Already Knew backoffMs — The Jar Did Not"
published: false
...
main_image: https://iili.io/CtfSrhl.jpg
---
...
Domain: multi-tenant quotas, feature posture. This essay maps hub key `backoffMs` to **retry backoff base** so the lesson stays concrete for saas operators.
...
```python
policy = kiponos.path("examples", "ops-saas-backoff-ms")
...
1. Name the hub path so humans find it under pressure (`examples/ops-saas-backoff-ms/backoffMs`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: retry backoff base for saas — numbers humans already change verbally in war rooms.
