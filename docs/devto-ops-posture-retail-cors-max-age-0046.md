---
title: "We Stopped Shipping Cors Max-Age as a Constant (Retail)"
published: false
tags: java, retail, devops, kiponos
description: "Live CORS max-age via Kiponos Java SDK — retail posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-retail-cors-max-age-0046.md
main_image: ./devto-cover-ops-posture-retail-cors-max-age-0046.jpg
---
...
Domain: checkout, merchandising, inventory. This essay maps hub key `corsMaxAgeSec` to **CORS max-age** so the lesson stays concrete for retail operators.
...
```java
Folder policy = kiponos.path("examples", "ops-retail-cors-max-age");
int corsMaxAgeSec = policy.getInt("corsMaxAgeSec");
if (corsMaxAgeSec <= 0) {
    return failClosed(); // money paths
}
return apply(corsMaxAgeSec);
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-retail-cors-max-age/corsMaxAgeSec`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: CORS max-age for retail — numbers humans already change verbally in war rooms.
...
2. Move `corsMaxAgeSec` to the emergency value (documented floor/ceiling).  
