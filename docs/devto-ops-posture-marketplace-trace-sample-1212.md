---
title: "Live Trace Sample Per Thousand for Marketplace — Mouth to Process in Seconds"
published: false
...
main_image: https://iili.io/nn7c7EP.jpg
---
...
Domain: seller tiers, fee weights. This essay maps hub key `samplePerThousand` to **trace sample per thousand** so the lesson stays concrete for marketplace operators.
...
```java
var policy = kiponos.path("examples", "ops-marketplace-trace-sample");
...
1. Name the hub path so humans find it under pressure (`examples/ops-marketplace-trace-sample/samplePerThousand`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: trace sample per thousand for marketplace — numbers humans already change verbally in war rooms.
