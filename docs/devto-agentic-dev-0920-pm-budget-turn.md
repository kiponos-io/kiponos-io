---
main_image: https://iili.io/CywTh21.jpg
title: "MCP host Finished the Turn Blind — Token budget mid-session on the Senses Path"
published: false
tags: java, python, devops, ai, kiponos
description: "MCP host kept finishing the turn blind on senses wall. Token budget mid-session is a live hub leaf."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-dev-0920-pm-budget-turn.md
---
...
```text
examples/agentic-dev-0920-pm-budget-turn/max-tokens = 8000
```

Runnable proof: [`examples/java/agentic-dev-0920-pm-budget-turn`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-dev-0920-pm-budget-turn)
...
```yaml
examples/
  agentic-dev-0920-pm-budget-turn/
...
Folder gate = kip.getRootFolder()
        ...
        .folderOrCreate("agentic-dev-0920-pm-budget-turn");
...
try:
    posture = k.get("examples/agentic-dev-0920-pm-budget-turn/max-tokens", "8000")
    if str(posture) == "8000":
...
MCP host is good at calling tools. It is not a control plane. Killing it to flip `max-tokens` teaches the on-call that judgment requires a process ID. The senses wall already disagrees.
...
3. `cd examples/java/agentic-dev-0920-pm-budget-turn && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-dev-0920-pm-budget-turn/max-tokens=...`
...
How to try: `examples/java/agentic-dev-0920-pm-budget-turn` and `./gradlew test`.
