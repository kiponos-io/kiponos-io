...
description: "Live canary percent on the Kiponos hub from a Angular server peer — createFromEnv, no SPA secrets, Java parity. Install @kiponos/angular."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-angular-sdk-canary-percent.md
---

**The Aha:** Canary percent is a judgment call under pressure. If the only path is a SPA rebuild, you do not have a control plane — you have a ceremony.

I have sat through a room that looked like this: canary window dying while the Angular pipeline was still green. Someone said “we just need to flip it.” Someone else opened a PR. CI ran. The window closed.
...
- Companion Java tree: `examples/java/angular-sdk-canary-percent` (same hub moral)
...
await kip.connect();
await kip.ensurePath('canary');
await kip.path('canary').set('percent', '…');
...
String v = k.path("canary").get("percent", "…");
...
Flip `percent` (or your domain leaf) on the dashboard or from the Node peer. The Java print should follow the hub — not the last SPA deploy.
...
|------|--------------------------------|------------------------|
| Change canary percent | Ship SPA / service build | Dashboard or SDK `set()` |
...
1. Name the hub path out loud: `canary/percent`  
...
Identity is **where the process runs**. Angular’s job is to be a first-class peer on the hub, not a bundler that smuggles secrets. Ship the peer once. Leave the bundle alone when the only thing that changed is **canary percent**.
