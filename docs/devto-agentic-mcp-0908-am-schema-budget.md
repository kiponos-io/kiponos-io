...
description: "Grok Build on admin-dashboard wall: MCP tool schemas as a live budget, not a dump. GitHub + Playwright + IDE burned ~143k of a 200k window at session start"
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-mcp-0908-am-schema-budget.md
---

I have sat next to the admin-dashboard wall at 03:07 while Grok Build was *this close* to calling the wrong MCP tool.

Not a model failure. A **posture** failure.

AgentPMT 2026 measured three popular MCP servers at 72% of a 200k window before the user spoke.

Someone on the floor said, out loud, **that is not overhead. that is the budget gone**. Grok Build had started earlier. The tool list was frozen in `config.toml` / `.mcp.json` / a skill file. The only “safe” move anyone trusted was:
...
That is the ceremony. **GitHub + Playwright + IDE burned ~143k of a 200k window at session start**

**The Aha:** The allow-list of schemas is a live hub leaf. Shrink it mid-turn. Do not restart the MCP process. Real-time nested state is now a **primitive** — as native as a hashmap — via [Kiponos.io](https://kiponos.io). The host stays up. The leaf moves. The next tool call already obeys.

## The problem: MCP tool schemas as a live budget, not a dump
...
GitHub + Playwright + IDE burned ~143k of a 200k window at session start
...
Profile shape: `['my-app']['v1.0.0']['dev']['base']`. The leaf in this story is `schema-budget` under `examples/agentic-mcp-0908-am-schema-budget/schema-budget`.

## Architecture

```mermaid
flowchart LR
  wall[admin-dashboard wall] -->|set schema-budget| hub[Kiponos hub]
  hub -->|delta WS| gb[Grok Build process]
  gb -->|get schema-budget| tool[MCP / Skill / tool wrapper]
  tool -->|allow or deny| world[External system]
```

The model still calls tools. The wrapper reads `schema-budget` **before** the call. No schema dump policy in argv.

## Config tree

```yaml
examples/agentic-mcp-0908-am-schema-budget:
  schema-budget: 8000
  schema-budget: "4000"
  result-cap: "2000"
  tool-search: "on"
...
Eight-plus keys when you pair `schema-budget` with sister dials (budget, mute, pause). They move together on the admin-dashboard wall.
...
Folder gate = Kiponos.createForCurrentTeam()
    .getFolder("examples/agentic-mcp-0908-am-schema-budget");
String roster = String.valueOf(gate.get("schema-budget"));
...
try:
    posture = k.get("examples/agentic-mcp-0908-am-schema-budget/schema-budget", "live")
    if not str(posture):
        raise PermissionError("MCP tool schemas as a live budget, not a dump gated live — host not restarted")
finally:
    k.disconnect()
```
...
Runnable mesh: `https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-mcp-0908-am-schema-budget` — `./gradlew test run`.
...
| User says “no writes” mid-turn | Restart Grok Build | Set `schema-budget`; next call already denies |
...
- A dashboard edit is a **delta** of `schema-budget`, not a config.toml reload.
...
## Pair `schema-budget` with a sister dial

`schema-budget` rarely moves alone on the admin-dashboard wall. Pair it with `schema-budget`, `result-cap`, or `execute-gate` so you do not fix MCP tool schemas as a live budget, not a dump by inventing a second incident.

## Why Grok Build is the wrong restart target

Grok Build is good at calling tools. It is not a control plane. Killing it to flip `schema-budget` teaches the on-call that judgment requires a process ID. The admin-dashboard wall already disagrees.
...
3. `cd examples/java/agentic-mcp-0908-am-schema-budget && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-mcp-0908-am-schema-budget/schema-budget=...`
5. In the dashboard, change `schema-budget`. Keep Grok Build up. No MCP reconnect.
6. Point the tool wrapper at the same leaf. Do not ship a new server binary to flip MCP tool schemas as a live budget, not a dump.
...
If flipping **MCP tool schemas as a live budget, not a dump** requires restarting Grok Build or reconnecting MCP, you do not have agentic control. You have a hope with a process ID.
...
How to try: `examples/java/agentic-mcp-0908-am-schema-budget` and `./gradlew test`.
