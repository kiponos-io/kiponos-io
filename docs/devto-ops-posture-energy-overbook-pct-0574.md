---
title: "Energy: Turn overbooking percent Into Incident Posture"
published: false
...
Domain: grid dispatch, demand response. This essay maps hub key `overbookPercent` to **overbooking percent** so the lesson stays concrete for energy operators.
...
```java
Folder policy = kiponos.path("examples", "ops-energy-overbook-pct");
...
1. Name the hub path so humans find it under pressure (`examples/ops-energy-overbook-pct/overbookPercent`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: overbooking percent for energy — numbers humans already change verbally in war rooms.
