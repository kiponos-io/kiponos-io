---
title: "We Stopped Shipping Pagination Page Size as a Constant (Security)"
published: false
...
Domain: WAF, RPS, auth leeway. This essay maps hub key `pageSize` to **pagination page size** so the lesson stays concrete for security operators.
...
```java
Folder policy = kiponos.path("examples", "ops-security-page-size");
...
1. Name the hub path so humans find it under pressure (`examples/ops-security-page-size/pageSize`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: pagination page size for security — numbers humans already change verbally in war rooms.
