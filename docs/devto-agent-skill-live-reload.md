---
title: "Agent Skills That Reload Without Killing the Session"
published: false
tags: java, python, kiponos, devops
description: "live skill enable set live across Java, Python, React, Angular and agents — no restart."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agent-skill-live-reload.md
main_image: https://iili.io/CSnkscP.jpg
---

**The Aha:** live skill enable set is not a properties-file trophy. It is **mesh posture** — and posture that waits for four redeploys is already late.
...
Domain: multi-SDK distributed systems, agents, MCP, observability, war-room truth. This essay maps hub key `enabled-set` under `examples/agent-skill-live-reload/` so the lesson stays concrete.

## The problem: ceremony between judgment and effect

You already know the right value for **live skill enable set**. The war room knows. The agent almost knows. What you do not have is a path from **mouth → every running peer** that is shorter than a release train and safer than pasting tokens into a SPA.

When `enabled-set` is frozen in YAML (or baked into a browser build), every incident becomes a process argument. When it lives in a hub with clamps, the argument ends and the work begins — for Java services, Python agents, React BFFs, and Angular admin hosts **at once**.
...
examples/
  agent-skill-live-reload/
    enabled-set: research,notify   # live skill enable set
...
```text
examples / agent-skill-live-reload
...
String posture = f.hasKey("enabled-set")
    ? f.get("enabled-set")
    : "research,notify";
// honor posture on the hot path — no hub RTT
```

Runnable: [examples/java/agent-skill-live-reload](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agent-skill-live-reload)
...
# Read examples/agent-skill-live-reload/enabled-set live.
# Enabling a skill, tightening a budget, or pausing MCP writes
# must not require killing this process.
```

Companion: [examples/python/agent-skill-live-reload](https://github.com/kiponos-io/kiponos-io/tree/master/examples/python/agent-skill-live-reload)
...
await kip.connect();
// set/get under examples/agent-skill-live-reload — inject mirror to SPA; never embed Connect tokens in bundles
```
...
3. `cd examples/java/agent-skill-live-reload && cp kiponos.local.env.example kiponos.local.env` and fill tokens.  
4. `./gradlew test run` — print `enabled-set`.  
5. Flip the leaf on the dashboard; re-run or leave a long-lived peer connected — no rebuild.  
6. Optional: `examples/python/agent-skill-live-reload` agent peer; `@kiponos/react` / `@kiponos/angular` on Node.
...
**live skill enable set** belongs on the hub — not in four redeploys, not in a dead agent session, not in a browser secret. The multi-SDK mesh is here. Use it while the process still runs.
