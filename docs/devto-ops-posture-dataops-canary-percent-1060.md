---
title: "Canary Traffic Share Should Not Wait for a Redeploy — Dataops Live Posture"
published: false
...
Domain: batch, lag, checkpoint. This essay maps hub key `canaryPercent` to **canary traffic share** so the lesson stays concrete for dataops operators.
...
```java
Folder policy = kiponos.path("examples", "ops-dataops-canary-percent");
...
1. Name the hub path so humans find it under pressure (`examples/ops-dataops-canary-percent/canaryPercent`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: canary traffic share for dataops — numbers humans already change verbally in war rooms.
