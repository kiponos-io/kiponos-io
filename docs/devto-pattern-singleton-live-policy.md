---
title: "Singleton as a Super Pattern — Live Policy Object"
published: false
...
**The Aha:** One instance, live fields — not a god object with frozen YAML

Everyone hates singletons until they need one policy object. Make its fields live.
...
```yaml
patterns/singleton/policy/
  # live knobs
  max-connections: 50; mode: normal
```

Hot path (local memory after connect — WebSocket deltas, no per-request hub RTT):

```java
Folder policy = kiponos.path('patterns', 'singleton', 'policy');
// local gets — never remote on the money path
Policy p = Policy.instance(); // fields refreshed from hub
...
Full tree: [https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/pattern-singleton-live-policy](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/pattern-singleton-live-policy)
...
One instance is fine. Frozen fields are not.
...
*Runnable: [pattern-singleton-live-policy](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/pattern-singleton-live-policy) · [kiponos.io](https://kiponos.io)*
