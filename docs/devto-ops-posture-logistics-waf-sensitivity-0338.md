---
title: "The Logistics War Room Already Knew wafSensitivity — The Jar Did Not"
published: false
tags: java, logistics, devops, kiponos
description: "Live WAF sensitivity 1-5 via Kiponos Java SDK — logistics posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-logistics-waf-sensitivity-0338.md
main_image: ./devto-cover-ops-posture-logistics-waf-sensitivity-0338.jpg
---
...
Domain: routing, SLA windows, fleet. This essay maps hub key `wafSensitivity` to **WAF sensitivity 1-5** so the lesson stays concrete for logistics operators.
...
```java
Folder policy = kiponos.path("examples", "ops-logistics-waf-sensitivity");
int wafSensitivity = Math.min(policy.getInt("wafSensitivity"), HARD_MAX);
limiter.setLimit(wafSensitivity); // next evaluation sees it
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-logistics-waf-sensitivity/wafSensitivity`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: WAF sensitivity 1-5 for logistics — numbers humans already change verbally in war rooms.
