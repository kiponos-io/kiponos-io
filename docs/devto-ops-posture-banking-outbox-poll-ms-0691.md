---
title: "We Stopped Shipping Outbox Poll Interval as a Constant (Banking)"
published: false
...
Domain: AML thresholds, transfer velocity. This essay maps hub key `outboxPollMs` to **outbox poll interval** so the lesson stays concrete for banking operators.
...
```java
Folder policy = kiponos.path("examples", "ops-banking-outbox-poll-ms");
...
1. Name the hub path so humans find it under pressure (`examples/ops-banking-outbox-poll-ms/outboxPollMs`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: outbox poll interval for banking — numbers humans already change verbally in war rooms.
