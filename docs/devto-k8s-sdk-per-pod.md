...
description: "Every pod runs its own SDK with an in-memory config cache. Reads are local at any replica count — WebSocket deltas keep the fleet consistent without a central c"
...
**The Aha:** `timeoutMs` is not a property-file trophy. It is **incident posture** for K8s pods + SDK — and posture that waits for a jar is already late.

Redeploying to change `timeoutMs` mid-incident is how teams invent 3am folklore around K8s pods + SDK.

## The problem: ceremony between judgment and effect

When **per-pod live config** is frozen in a deploy unit, every incident becomes a process argument. Hotfixes still mean CI + roll. Feature flags often mean a second control plane with its own delay.
...
```java
Folder policy = kiponos.path("examples", "k8s-sdk-per-pod");
int timeoutMs = policy.getInt("timeoutMs"); // local get — no hub RTT
if (timeoutMs <= 0) {
    return failClosed();
}
...
Ops (or automation) sets `timeoutMs` in the hub. The **next** evaluation uses it. Same binary. Same tests for structure.
...
- Canonical source: [this article on GitHub](https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-k8s-sdk-per-pod.md)
...
Feature flags are often product gates. This essay is about **ops posture** on K8s pods + SDK: **per-pod live config** — numbers war rooms already shout.
...
Treat `timeoutMs` as a slice of error budget. Raise when the world is healthy; lower when dependencies are sick. Write the number that survives a bad day.
...
**Per-pod live config** that requires a deploy is optimistic documentation.
