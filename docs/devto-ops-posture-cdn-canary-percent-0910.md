---
title: "Canary Traffic Share Should Not Wait for a Redeploy — Cdn Live Posture"
published: false
...
main_image: https://iili.io/CSe4F8Q.jpg
---
...
Domain: edge rules, cache TTL, purge. This essay maps hub key `canaryPercent` to **canary traffic share** so the lesson stays concrete for cdn operators.
...
```java
Folder policy = kiponos.path("examples", "ops-cdn-canary-percent");
...
1. Name the hub path so humans find it under pressure (`examples/ops-cdn-canary-percent/canaryPercent`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: canary traffic share for cdn — numbers humans already change verbally in war rooms.
