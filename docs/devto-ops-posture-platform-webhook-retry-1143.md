---
title: "The Platform War Room Already Knew webhookMaxAttempts — The Jar Did Not"
published: false
tags: java, platform, architecture, kiponos
description: "Live webhook retry budget via Kiponos Java SDK — platform posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-platform-webhook-retry-1143.md
main_image: ./devto-cover-ops-posture-platform-webhook-retry-1143.jpg
---
...
Domain: shared libraries, defaults. This essay maps hub key `webhookMaxAttempts` to **webhook retry budget** so the lesson stays concrete for platform operators.
...
```java
Folder policy = kiponos.path("examples", "ops-platform-webhook-retry");
int webhookMaxAttempts = Math.min(policy.getInt("webhookMaxAttempts"), HARD_MAX);
limiter.setLimit(webhookMaxAttempts); // next evaluation sees it
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-platform-webhook-retry/webhookMaxAttempts`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: webhook retry budget for platform — numbers humans already change verbally in war rooms.
