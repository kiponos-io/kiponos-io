---
title: "Canary Traffic Share Should Not Wait for a Redeploy — Media Live Posture"
published: false
tags: java, streaming, devops, kiponos
description: "Live canary traffic share via Kiponos Java SDK — media posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-media-canary-percent-0160.md
main_image: ./devto-cover-ops-posture-media-canary-percent-0160.jpg
---
...
Domain: bitrate, CDN, origin. This essay maps hub key `canaryPercent` to **canary traffic share** so the lesson stays concrete for media operators.
...
```java
Folder policy = kiponos.path("examples", "ops-media-canary-percent");
...
1. Name the hub path so humans find it under pressure (`examples/ops-media-canary-percent/canaryPercent`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: canary traffic share for media — numbers humans already change verbally in war rooms.
...
canary traffic share that requires a deploy is optimistic documentation.
