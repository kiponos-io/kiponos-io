---
title: "Canary Traffic Share Should Not Wait for a Redeploy — Travel Live Posture"
published: false
tags: java, travel, architecture, kiponos
description: "Live canary traffic share via Kiponos Python SDK — travel posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-travel-canary-percent-0110.md
main_image: ./devto-cover-ops-posture-travel-canary-percent-0110.jpg
---
...
Domain: inventory, overbooking, GDS. This essay maps hub key `canaryPercent` to **canary traffic share** so the lesson stays concrete for travel operators.
...
```python
policy = kiponos.path("examples", "ops-travel-canary-percent")
canaryPercent = int(policy.get("canaryPercent", 5))
if not limiter.try_acquire(canaryPercent):
    return too_many()
return handle(req)
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-travel-canary-percent/canaryPercent`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: canary traffic share for travel — numbers humans already change verbally in war rooms.
