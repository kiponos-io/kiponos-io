---
title: "We Stopped Shipping Rag Top-K as a Constant (Mlops)"
published: false
tags: java, machinelearning, devops, kiponos
description: "Live RAG top-k via Kiponos Python SDK — mlops posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-mlops-rag-topk-0986.md
main_image: ./devto-cover-ops-posture-mlops-rag-topk-0986.jpg
---

**The Aha:** `topK` is not a property file trophy. It is **incident posture** — and posture that waits for a jar is already late.

The partner brownout did not wait for your change-management window. Your RAG top-k did — until it lived in a hub.

Domain: batch size, score thresholds. This essay maps hub key `topK` to **RAG top-k** so the lesson stays concrete for mlops operators.
...
```python
policy = kiponos.path("examples", "ops-mlops-rag-topk")
...
1. Name the hub path so humans find it under pressure (`examples/ops-mlops-rag-topk/topK`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: RAG top-k for mlops — numbers humans already change verbally in war rooms.
