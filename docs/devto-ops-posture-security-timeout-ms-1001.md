---
title: "We Stopped Shipping Outbound Call Timeout as a Constant (Security)"
published: false
...
main_image: https://iili.io/CtvfmKu.jpg
---
...
Domain: WAF, RPS, auth leeway. This essay maps hub key `timeoutMs` to **outbound call timeout** so the lesson stays concrete for security operators.
...
```python
policy = kiponos.path("examples", "ops-security-timeout-ms")
...
1. Name the hub path so humans find it under pressure (`examples/ops-security-timeout-ms/timeoutMs`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: outbound call timeout for security — numbers humans already change verbally in war rooms.
