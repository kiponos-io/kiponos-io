---
title: "Flyweight as a Super Pattern — Live Cache Budget"
published: false
tags: java, designpatterns, architecture, devops
description: "Shared instance pool size and TTL from the hub"
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-pattern-flyweight-live-cache.md
main_image: ./devto-cover-pattern-flyweight-live-cache.jpg
...
**The Aha:** Shared instance pool size and TTL from the hub

Flyweights save memory until the pool is wrong for peak. Budget is posture.
...
```yaml
patterns/flyweight/cache/
  # live knobs
  pool-size: 256; ttl-s: 60
```

Hot path (local memory after connect — WebSocket deltas, no per-request hub RTT):

```java
Folder policy = kiponos.path('patterns', 'flyweight', 'cache');
// local gets — never remote on the money path
pool.ensureCapacity(policy.getInt("pool-size"));
```
...
Full tree: [https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/pattern-flyweight-live-cache](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/pattern-flyweight-live-cache)
...
Sharing is design. Pool limits are ops.
...
*Runnable: [pattern-flyweight-live-cache](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/pattern-flyweight-live-cache) · [kiponos.io](https://kiponos.io)*
