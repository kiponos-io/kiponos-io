---
title: "Sre: Turn overbooking percent Into Incident Posture"
published: false
...
Domain: canary, drain, error budget. This essay maps hub key `overbookPercent` to **overbooking percent** so the lesson stays concrete for sre operators.
...
```python
policy = kiponos.path("examples", "ops-sre-overbook-pct")
...
1. Name the hub path so humans find it under pressure (`examples/ops-sre-overbook-pct/overbookPercent`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: overbooking percent for sre — numbers humans already change verbally in war rooms.
