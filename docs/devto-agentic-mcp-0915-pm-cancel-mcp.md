...
description: "Grok Build on senses wall: Live cancel token for agentic MCP work. MCP Tasks primitive is arriving; clients still SIGKILL the whole agent to stop one call"
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-mcp-0915-pm-cancel-mcp.md
---

I have sat next to the senses wall at 03:07 while Grok Build was *this close* to calling the wrong MCP tool.

Not a model failure. A **posture** failure.

2026 MCP roadmap: long-running Tasks. Until hosts honor a live cancel leaf, people kill PID 1 of the session.

Someone on the floor said, out loud, **just kill Grok Build, the scrape will die with it**. Grok Build had started earlier. The tool list was frozen in `config.toml` / `.mcp.json` / a skill file. The only “safe” move anyone trusted was:
...
That is the ceremony. **MCP Tasks primitive is arriving; clients still SIGKILL the whole agent to stop one call**

**The Aha:** Set `cancel-token=now`. The tool aborts. The turn can continue. Real-time nested state is now a **primitive** — as native as a hashmap — via [Kiponos.io](https://kiponos.io). The host stays up. The leaf moves. The next tool call already obeys.

## The problem: Live cancel token for agentic MCP work
...
MCP Tasks primitive is arriving; clients still SIGKILL the whole agent to stop one call
...
Profile shape: `['my-app']['v1.0.0']['dev']['base']`. The leaf in this story is `cancel-token` under `examples/agentic-mcp-0915-pm-cancel-mcp/cancel-token`.
...
```yaml
examples/agentic-mcp-0915-pm-cancel-mcp:
  cancel-token: off
  schema-budget: "4000"
  result-cap: "2000"
  tool-search: "on"
...
Eight-plus keys when you pair `cancel-token` with sister dials (budget, mute, pause). They move together on the senses wall.
...
Folder gate = Kiponos.createForCurrentTeam()
    .getFolder("examples/agentic-mcp-0915-pm-cancel-mcp");
String roster = String.valueOf(gate.get("cancel-token"));
...
try:
    posture = k.get("examples/agentic-mcp-0915-pm-cancel-mcp/cancel-token", "live")
    if not str(posture):
        raise PermissionError("Live cancel token for agentic MCP work gated live — host not restarted")
finally:
    k.disconnect()
```
...
Runnable mesh: `https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-mcp-0915-pm-cancel-mcp` — `./gradlew test run`.
...
| User says “no writes” mid-turn | Restart Grok Build | Set `cancel-token`; next call already denies |
...
- A dashboard edit is a **delta** of `cancel-token`, not a config.toml reload.
...
## Pair `cancel-token` with a sister dial

`cancel-token` rarely moves alone on the senses wall. Pair it with `schema-budget`, `result-cap`, or `execute-gate` so you do not fix Live cancel token for agentic MCP work by inventing a second incident.

## Why Grok Build is the wrong restart target

Grok Build is good at calling tools. It is not a control plane. Killing it to flip `cancel-token` teaches the on-call that judgment requires a process ID. The senses wall already disagrees.
...
3. `cd examples/java/agentic-mcp-0915-pm-cancel-mcp && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-mcp-0915-pm-cancel-mcp/cancel-token=...`
5. In the dashboard, change `cancel-token`. Keep Grok Build up. No MCP reconnect.
6. Point the tool wrapper at the same leaf. Do not ship a new server binary to flip Live cancel token for agentic MCP work.
...
If flipping **Live cancel token for agentic MCP work** requires restarting Grok Build or reconnecting MCP, you do not have agentic control. You have a hope with a process ID.
...
How to try: `examples/java/agentic-mcp-0915-pm-cancel-mcp` and `./gradlew test`.
