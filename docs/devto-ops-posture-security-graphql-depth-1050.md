---
title: "Graphql Query Depth Should Not Wait for a Redeploy — Security Live Posture"
published: false
...
Domain: WAF, RPS, auth leeway. This essay maps hub key `maxDepth` to **GraphQL query depth** so the lesson stays concrete for security operators.
...
```java
Folder policy = kiponos.path("examples", "ops-security-graphql-depth");
...
1. Name the hub path so humans find it under pressure (`examples/ops-security-graphql-depth/maxDepth`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: GraphQL query depth for security — numbers humans already change verbally in war rooms.
