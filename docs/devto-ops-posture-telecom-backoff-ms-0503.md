---
title: "The Telecom War Room Already Knew backoffMs — The Jar Did Not"
published: false
...
Domain: QoS, rate classes, trunk limits. This essay maps hub key `backoffMs` to **retry backoff base** so the lesson stays concrete for telecom operators.
...
```java
var policy = kiponos.path("examples", "ops-telecom-backoff-ms");
...
1. Name the hub path so humans find it under pressure (`examples/ops-telecom-backoff-ms/backoffMs`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: retry backoff base for telecom — numbers humans already change verbally in war rooms.
