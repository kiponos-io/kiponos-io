---
title: "Interpreter as a Super Pattern — Live Rule Source"
published: false
...
**The Aha:** Rule text in the hub, engine in the jar

Promos and risk rules change faster than release trains. Keep the interpreter; live the source.
...
```yaml
patterns/interpreter/rules/
  # live knobs
  source: amount > 100 and country != XX
```

Hot path (local memory after connect — WebSocket deltas, no per-request hub RTT):

```java
Folder policy = kiponos.path('patterns', 'interpreter', 'rules');
// local gets — never remote on the money path
boolean ok = interpreter.eval(policy.getString("source"), ctx);
```
...
Full tree: [https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/pattern-interpreter-live-rules](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/pattern-interpreter-live-rules)
...
Engines are code. Rule sources are posture — with audit and allowlists.
...
*Runnable: [pattern-interpreter-live-rules](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/pattern-interpreter-live-rules) · [kiponos.io](https://kiponos.io)*
