---
main_image: https://iili.io/nq4BAWx.jpg
...
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-overflow-20260914-shopping.md
---

I have sat next to the shopping-admin wall at 02:14 while Grok Build was *this close* to doing the wrong thing.

Not a model failure. A **posture** failure.

The host had started with `max-tokens` frozen in env / argv / a skill file. Someone on the floor said, out loud, **disable inventory writes — keep search**. Grok Build was still holding the old process. The only “safe” move anyone trusted was:
...
**The Aha:** token budgets mid-run as live posture on the shopping wall is not a binary you reboot. It is a **function that should read live posture** from [Kiponos.io](https://kiponos.io) on every call. The host stays up. The leaf moves.

## The problem: token budgets mid-run as live posture on the shopping wall lived in the process, not in the turn

Grok Build is good at *calling* tools. It is not born with a **shared, instant, restart-free control plane**.

So teams hide token budgets mid-run as live posture on the shopping wall in the only places agent frameworks actually ship:
...
| Restart Grok Build — it is cheap | Cheap until 02:14 ate cart forensics and the open SKU thread |
...
```text
examples/agent-budget-token-live/max-tokens = 8000
```

Runnable proof: [`examples/java/agent-budget-token-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agent-budget-token-live)
...
examples/
  agent-budget-token-live/
    max-tokens: 8000          # token budgets mid-run as live posture on the shopping wall
apps/
  shopping/
    live:
      max-tokens: 8000
...
try:
    posture = k.get("examples/agent-budget-token-live/max-tokens", "8000")
    if str(posture) == "8000":
        raise PermissionError("token budgets mid-run as live posture on the shopping wall gated live — host not restarted")
finally:
    k.disconnect()
```
...
|-------|-----------------|--------------|
| Flash-freeze on sku writes | Restart Grok Build; lose cart forensics and the open SKU thread | Set `max-tokens` live; next Grok Build tool call already obeys |
...
| Incident over, resume | Another Grok Build restart | Set `max-tokens` back; session continues |
...
At 02:14 someone said, out loud: **disable inventory writes — keep search**. That sentence is the whole product. If it cannot land in the running Grok Build process in seconds, you do not have posture. You have a wiki.
...
3. `cd examples/java/agent-budget-token-live && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agent-budget-token-live/max-tokens=...`
5. In the dashboard, change `max-tokens`. Keep the process up. No rebuild.
6. Point your Grok Build tool at the same leaf. Do not ship a new server binary to flip token budgets mid-run as live posture on the shopping wall.
...
If flipping **token budgets mid-run as live posture on the shopping wall** requires restarting Grok Build, you do not have a gate. You have a hope with a process ID.
...
How to try: `examples/java/agent-budget-token-live` and `./gradlew test`.
