---
title: "The Sre War Room Already Knew logSamplePerThousand — The Jar Did Not"
published: false
tags: java, sre, devops, kiponos
description: "Live log sample per thousand via Kiponos Java SDK — sre posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-sre-log-sample-0213.md
main_image: ./devto-cover-ops-posture-sre-log-sample-0213.jpg
---
...
Domain: canary, drain, error budget. This essay maps hub key `logSamplePerThousand` to **log sample per thousand** so the lesson stays concrete for sre operators.
...
```java
var policy = kiponos.path("examples", "ops-sre-log-sample");
...
1. Name the hub path so humans find it under pressure (`examples/ops-sre-log-sample/logSamplePerThousand`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: log sample per thousand for sre — numbers humans already change verbally in war rooms.
...
log sample per thousand that requires a deploy is optimistic documentation.
