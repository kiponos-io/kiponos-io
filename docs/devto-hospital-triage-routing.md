...
Redeploying to change `triageRoute` mid-incident is how teams invent 3am folklore around ED routing.

## The problem: ceremony between judgment and effect

When **triage route weights** is frozen in a deploy unit, every incident becomes a process argument. Hotfixes still mean CI + roll. Feature flags often mean a second control plane with its own delay.
...
```java
Folder policy = kiponos.path("examples", "hospital-triage-routing");
int triageRoute = policy.getInt("triageRoute"); // local get — no hub RTT
if (triageRoute <= 0) {
    return failClosed();
}
return apply(triageRoute);
```

Ops (or automation) sets `triageRoute` in the hub. The **next** evaluation uses it. Same binary. Same tests for structure.
...
- Canonical source: [this article on GitHub](https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-hospital-triage-routing.md)
...
Feature flags are often product gates. This essay is about **ops posture** on ED routing: **triage route weights** — numbers war rooms already shout.
...
Treat `triageRoute` as a slice of error budget. Raise when the world is healthy; lower when dependencies are sick. Write the number that survives a bad day.
...
**Triage route weights** that requires a deploy is optimistic documentation.
