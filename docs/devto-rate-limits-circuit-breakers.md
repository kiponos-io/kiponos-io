...
When **RPS + circuit threshold** is frozen in a deploy unit, every incident becomes a process argument. Hotfixes still mean CI + roll. Feature flags often mean a second control plane with its own delay.
...
```java
Folder policy = kiponos.path("examples", "rate-limits-circuit-breakers");
int rps = policy.getInt("rps"); // local get — no hub RTT
if (rps <= 0) {
    return failClosed();
}
return apply(rps);
...
Ops (or automation) sets `rps` in the hub. The **next** evaluation uses it. Same binary. Same tests for structure.
...
Dashboard / automation ──write──► Kiponos hub
                               ...
                               SDK in-process cache
                                      │ local get
                                      ▼
                               Hot path (RPS + circuit threshold)
```
...
- Canonical source: [this article on GitHub](https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-rate-limits-circuit-breakers.md)
...
Feature flags are often product gates. This essay is about **ops posture** on API protection: **RPS + circuit threshold** — numbers war rooms already shout.
...
Treat `rps` as a slice of error budget. Raise when the world is healthy; lower when dependencies are sick. Write the number that survives a bad day.
...
**RPS + circuit threshold** that requires a deploy is optimistic documentation.
