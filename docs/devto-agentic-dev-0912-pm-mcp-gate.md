---
main_image: https://iili.io/CpsiF71.jpg
...
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-dev-0912-pm-mcp-gate.md
---

I have sat next to the travel-coordinator wall at 02:14 while MCP host was *this close* to doing the wrong thing.
...
| Restart MCP host — it is cheap | Cheap until 02:14 ate the overbook thread the agent already paid for |
...
```text
examples/agentic-dev-0912-pm-mcp-gate/tools-allow = search,read
```

Runnable proof: [`examples/java/agentic-dev-0912-pm-mcp-gate`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-dev-0912-pm-mcp-gate)
...
Folder gate = kip.getRootFolder()
        ...
        .folderOrCreate("agentic-dev-0912-pm-mcp-gate");
...
try:
    posture = k.get("examples/agentic-dev-0912-pm-mcp-gate/tools-allow", "search,read")
...
MCP host is good at calling tools. It is not a control plane. Killing it to flip `tools-allow` teaches the on-call that judgment requires a process ID. The travel-coordinator wall already disagrees.

## What the travel operator actually said

At 02:14 someone said, out loud: **mute writes on the noisy room — keep reads**. That sentence is the whole product. If it cannot land in the running MCP host process in seconds, you do not have posture. You have a wiki.
...
3. `cd examples/java/agentic-dev-0912-pm-mcp-gate && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-dev-0912-pm-mcp-gate/tools-allow=...`
...
How to try: `examples/java/agentic-dev-0912-pm-mcp-gate` and `./gradlew test`.
