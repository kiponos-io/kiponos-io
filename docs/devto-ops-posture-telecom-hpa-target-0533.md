---
title: "The Telecom War Room Already Knew hpaTarget — The Jar Did Not"
published: false
...
Domain: QoS, rate classes, trunk limits. This essay maps hub key `hpaTarget` to **HPA CPU target percent** so the lesson stays concrete for telecom operators.
...
```java
var policy = kiponos.path("examples", "ops-telecom-hpa-target");
...
1. Name the hub path so humans find it under pressure (`examples/ops-telecom-hpa-target/hpaTarget`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: HPA CPU target percent for telecom — numbers humans already change verbally in war rooms.
