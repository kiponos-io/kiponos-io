---
title: "We Stopped Shipping Outbound Call Timeout as a Constant (Retail)"
published: false
...
main_image: ./devto-cover-ops-posture-retail-timeout-ms-0001.jpg
---
...
Domain: checkout, merchandising, inventory. This essay maps hub key `timeoutMs` to **outbound call timeout** so the lesson stays concrete for retail operators.
...
```java
var policy = kiponos.path("examples", "ops-retail-timeout-ms");
...
1. Name the hub path so humans find it under pressure (`examples/ops-retail-timeout-ms/timeoutMs`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: outbound call timeout for retail — numbers humans already change verbally in war rooms.
