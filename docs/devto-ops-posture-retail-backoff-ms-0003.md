---
title: "The Retail War Room Already Knew backoffMs — The Jar Did Not"
published: false
...
main_image: https://d.uguu.se/sNmqVgCT.jpg
---
...
Domain: checkout, merchandising, inventory. This essay maps hub key `backoffMs` to **retry backoff base** so the lesson stays concrete for retail operators.
...
```java
Folder policy = kiponos.path("examples", "ops-retail-backoff-ms");
...
1. Name the hub path so humans find it under pressure (`examples/ops-retail-backoff-ms/backoffMs`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: retry backoff base for retail — numbers humans already change verbally in war rooms.
