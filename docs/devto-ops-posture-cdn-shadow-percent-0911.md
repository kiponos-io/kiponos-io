---
title: "We Stopped Shipping Shadow Traffic Share as a Constant (Cdn)"
published: false
...
main_image: https://iili.io/CUEuvI4.jpg
---
...
Domain: edge rules, cache TTL, purge. This essay maps hub key `shadowPercent` to **shadow traffic share** so the lesson stays concrete for cdn operators.
...
```java
var policy = kiponos.path("examples", "ops-cdn-shadow-percent");
...
1. Name the hub path so humans find it under pressure (`examples/ops-cdn-shadow-percent/shadowPercent`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: shadow traffic share for cdn — numbers humans already change verbally in war rooms.
