---
main_image: https://iili.io/CyJUEGt.jpg
title: "Cursor Finished the Turn Blind — Sense priority mid-turn on the Admin Dashboard Path"
published: false
tags: java, python, devops, ai, kiponos
description: "Cursor kept finishing the turn blind on admin-dashboard wall. Sense priority mid-turn is a live hub leaf."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-dev-0917-am-sense-priority.md
---

I have sat next to the admin-dashboard wall at 03:07 while Cursor was *this close* to doing the wrong thing.

Not a model failure. A **posture** failure.

The host had started with `priority` frozen in env / argv / a skill file. Someone on the floor said, out loud, **the tile is not a rumor. it is the same tree**. Cursor was still holding the old process. The only “safe” move anyone trusted was:
...
| Restart Cursor — it is cheap | Cheap until 03:07 ate the operator's last ten minutes of diagnosis |
...
```text
examples/agentic-dev-0917-am-sense-priority/priority = live
```

Runnable proof: [`examples/java/agentic-dev-0917-am-sense-priority`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-dev-0917-am-sense-priority)
...
```yaml
examples/
  agentic-dev-0917-am-sense-priority/
...
Folder gate = kip.getRootFolder()
        ...
        .folderOrCreate("agentic-dev-0917-am-sense-priority");
...
try:
    posture = k.get("examples/agentic-dev-0917-am-sense-priority/priority", "live")
    if str(posture) == "live":
...
|-------|-----------------|--------------|
| Status tile green while the agent host started earlier | Restart Cursor; lose the operator's last ten minutes of diagnosis | Set `priority` live; next Cursor tool call already obeys |
...
| Incident over, resume | Another Cursor restart | Set `priority` back; session continues |
...
In staging: set a painful `priority`, prove Cursor recovers without a host kill, prove clamps reject nonsense, prove last-known-good when the hub is firewalled. That drill ends half the architecture arguments about Sense priority mid-turn.

## Why Cursor is the wrong restart target

Cursor is good at calling tools. It is not a control plane. Killing it to flip `priority` teaches the on-call that judgment requires a process ID. The admin-dashboard wall already disagrees.

## What the admin dashboard operator actually said

At 03:07 someone said, out loud: **the tile is not a rumor. it is the same tree**. That sentence is the whole product. If it cannot land in the running Cursor process in seconds, you do not have posture. You have a wiki.
...
3. `cd examples/java/agentic-dev-0917-am-sense-priority && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-dev-0917-am-sense-priority/priority=...`
5. In the dashboard, change `priority`. Keep the process up. No rebuild.
6. Point your Cursor tool at the same leaf. Do not ship a new server binary to flip Sense priority mid-turn.
...
If flipping **Sense priority mid-turn** requires restarting Cursor, you do not have a gate. You have a hope with a process ID.
...
How to try: `examples/java/agentic-dev-0917-am-sense-priority` and `./gradlew test`.
