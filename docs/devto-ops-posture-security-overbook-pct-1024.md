---
title: "Security: Turn overbooking percent Into Incident Posture"
published: false
tags: java, security, devops, kiponos
description: "Live overbooking percent via Kiponos Java SDK — security posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-security-overbook-pct-1024.md
main_image: ./devto-cover-ops-posture-security-overbook-pct-1024.jpg
---
...
Domain: WAF, RPS, auth leeway. This essay maps hub key `overbookPercent` to **overbooking percent** so the lesson stays concrete for security operators.
...
```java
Folder policy = kiponos.path("examples", "ops-security-overbook-pct");
int overbookPercent = policy.getInt("overbookPercent");
if (overbookPercent <= 0) {
    return failClosed(); // money paths
}
return apply(overbookPercent);
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-security-overbook-pct/overbookPercent`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: overbooking percent for security — numbers humans already change verbally in war rooms.
