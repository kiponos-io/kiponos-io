---
title: "We Stopped Shipping Graceful Drain Window as a Constant (Energy)"
published: false
tags: java, energy, architecture, kiponos
description: "Live graceful drain window via Kiponos Java SDK — energy posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-energy-drain-ms-0566.md
main_image: https://files.catbox.moe/nyfri5.jpg
---
...
Domain: grid dispatch, demand response. This essay maps hub key `gracefulShutdownMs` to **graceful drain window** so the lesson stays concrete for energy operators.
...
```java
Folder policy = kiponos.path("examples", "ops-energy-drain-ms");
...
1. Name the hub path so humans find it under pressure (`examples/ops-energy-drain-ms/gracefulShutdownMs`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: graceful drain window for energy — numbers humans already change verbally in war rooms.
