...
description: "MCP host on senses wall: Mid-turn interrupt as a live priority leaf. Probe lived in a file. The turn treated it as noise."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-mcp-0922-pm-sense-stale.md
---

I have sat next to the senses wall at 18:45 while MCP host was *this close* to calling the wrong MCP tool.

Not a model failure. A **posture** failure.

Agent tools stacks 2026: no memory layer for *this second's* priority. Files are not interrupts.

Someone on the floor said, out loud, **it will see the P1 on the next turn**. MCP host had started earlier. The tool list was frozen in `config.toml` / `.mcp.json` / a skill file. The only “safe” move anyone trusted was:
...
That is the ceremony. **Probe lived in a file. The turn treated it as noise.**

**The Aha:** `priority=P1` abort the dangerous MCP call now. Host stays up. Real-time nested state is now a **primitive** — as native as a hashmap — via [Kiponos.io](https://kiponos.io). The host stays up. The leaf moves. The next tool call already obeys.

## The problem: Mid-turn interrupt as a live priority leaf
...
Probe lived in a file. The turn treated it as noise.
...
Profile shape: `['my-app']['v1.0.0']['dev']['base']`. The leaf in this story is `priority` under `examples/agentic-mcp-0922-pm-sense-stale/priority`.

## Architecture

```mermaid
flowchart LR
  wall[senses wall] -->|set priority| hub[Kiponos hub]
  hub -->|delta WS| gb[MCP host process]
  gb -->|get priority| tool[MCP / Skill / tool wrapper]
  tool -->|allow or deny| world[External system]
```

The model still calls tools. The wrapper reads `priority` **before** the call. No schema dump policy in argv.

## Config tree

```yaml
examples/agentic-mcp-0922-pm-sense-stale:
  priority: P3
  schema-budget: "4000"
  result-cap: "2000"
  tool-search: "on"
...
Eight-plus keys when you pair `priority` with sister dials (budget, mute, pause). They move together on the senses wall.
...
Folder gate = Kiponos.createForCurrentTeam()
    .getFolder("examples/agentic-mcp-0922-pm-sense-stale");
String roster = String.valueOf(gate.get("priority"));
...
try:
    posture = k.get("examples/agentic-mcp-0922-pm-sense-stale/priority", "live")
    if not str(posture):
        raise PermissionError("Mid-turn interrupt as a live priority leaf gated live — host not restarted")
finally:
    k.disconnect()
```
...
Runnable mesh: `https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-mcp-0922-pm-sense-stale` — `./gradlew test run`.
...
| User says “no writes” mid-turn | Restart MCP host | Set `priority`; next call already denies |
...
- A dashboard edit is a **delta** of `priority`, not a config.toml reload.
...
`priority` rarely moves alone on the senses wall. Pair it with `schema-budget`, `result-cap`, or `execute-gate` so you do not fix Mid-turn interrupt as a live priority leaf by inventing a second incident.

## Why MCP host is the wrong restart target

MCP host is good at calling tools. It is not a control plane. Killing it to flip `priority` teaches the on-call that judgment requires a process ID. The senses wall already disagrees.
...
3. `cd examples/java/agentic-mcp-0922-pm-sense-stale && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-mcp-0922-pm-sense-stale/priority=...`
5. In the dashboard, change `priority`. Keep MCP host up. No MCP reconnect.
6. Point the tool wrapper at the same leaf. Do not ship a new server binary to flip Mid-turn interrupt as a live priority leaf.
...
If flipping **Mid-turn interrupt as a live priority leaf** requires restarting MCP host or reconnecting MCP, you do not have agentic control. You have a hope with a process ID.
...
How to try: `examples/java/agentic-mcp-0922-pm-sense-stale` and `./gradlew test`.
