---
title: "The Security War Room Already Knew backoffMs — The Jar Did Not"
published: false
...
Domain: WAF, RPS, auth leeway. This essay maps hub key `backoffMs` to **retry backoff base** so the lesson stays concrete for security operators.
...
```python
policy = kiponos.path("examples", "ops-security-backoff-ms")
...
1. Name the hub path so humans find it under pressure (`examples/ops-security-backoff-ms/backoffMs`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: retry backoff base for security — numbers humans already change verbally in war rooms.
