---
title: "Live Retry Budget for Security — Mouth to Process in Seconds"
published: false
...
Domain: WAF, RPS, auth leeway. This essay maps hub key `maxAttempts` to **retry budget** so the lesson stays concrete for security operators.
...
```python
policy = kiponos.path("examples", "ops-security-retry-max")
...
1. Name the hub path so humans find it under pressure (`examples/ops-security-retry-max/maxAttempts`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: retry budget for security — numbers humans already change verbally in war rooms.
