---
title: "We Stopped Shipping Rag Top-K as a Constant (Telecom)"
published: false
...
main_image: ./devto-cover-ops-posture-telecom-rag-topk-0536.jpg
---
...
Domain: QoS, rate classes, trunk limits. This essay maps hub key `topK` to **RAG top-k** so the lesson stays concrete for telecom operators.
...
```java
var policy = kiponos.path("examples", "ops-telecom-rag-topk");
...
1. Name the hub path so humans find it under pressure (`examples/ops-telecom-rag-topk/topK`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: RAG top-k for telecom — numbers humans already change verbally in war rooms.
