---
title: "We Stopped Shipping Shadow Traffic Share as a Constant (Mobile)"
published: false
tags: java, mobile, architecture, kiponos
description: "Live shadow traffic share via Kiponos Python SDK — mobile posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-mobile-shadow-percent-1161.md
main_image: ./devto-cover-ops-posture-mobile-shadow-percent-1161.jpg
---
...
Domain: API timeouts, payload caps. This essay maps hub key `shadowPercent` to **shadow traffic share** so the lesson stays concrete for mobile operators.
...
```python
policy = kiponos.path("examples", "ops-mobile-shadow-percent")
shadowPercent = min(int(policy.get("shadowPercent", 2)), HARD_MAX)
worker_pool.resize(shadowPercent)  # or admission semaphore
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-mobile-shadow-percent/shadowPercent`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: shadow traffic share for mobile — numbers humans already change verbally in war rooms.
