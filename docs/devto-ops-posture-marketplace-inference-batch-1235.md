---
title: "Inference Batch Size Should Not Wait for a Redeploy — Marketplace Live Posture"
published: false
tags: java, architecture, devops, kiponos
description: "Live inference batch size via Kiponos Python SDK — marketplace posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-marketplace-inference-batch-1235.md
main_image: ./devto-cover-ops-posture-marketplace-inference-batch-1235.jpg
---
...
Domain: seller tiers, fee weights. This essay maps hub key `batchSize` to **inference batch size** so the lesson stays concrete for marketplace operators.
...
```yaml
examples/
  ops-marketplace-inference-batch/
...
```python
policy = kiponos.path("examples", "ops-marketplace-inference-batch")
...
1. Name the hub path so humans find it under pressure (`examples/ops-marketplace-inference-batch/batchSize`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: inference batch size for marketplace — numbers humans already change verbally in war rooms.
