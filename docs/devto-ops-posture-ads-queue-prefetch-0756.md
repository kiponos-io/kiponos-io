---
title: "We Stopped Shipping Consumer Prefetch as a Constant (Ads)"
published: false
...
Domain: bid caps, frequency, pacing. This essay maps hub key `prefetch` to **consumer prefetch** so the lesson stays concrete for ads operators.
...
```python
policy = kiponos.path("examples", "ops-ads-queue-prefetch")
...
1. Name the hub path so humans find it under pressure (`examples/ops-ads-queue-prefetch/prefetch`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: consumer prefetch for ads — numbers humans already change verbally in war rooms.
