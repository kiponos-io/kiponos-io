---
main_image: https://n.uguu.se/dtRGzxAn.jpg
title: "Cursor Finished the Turn Blind — Agents choose a metadata folder on the Admin Dashboard Path"
published: false
tags: java, python, devops, ai, kiponos
description: "Cursor kept finishing the turn blind on admin-dashboard wall. Agents choose a metadata folder is a live hub leaf."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-dev-0914-pm-agent-folder.md
---

I have sat next to the admin-dashboard wall at 11:41 while Cursor was *this close* to doing the wrong thing.

Not a model failure. A **posture** failure.

The host had started with `owner-agent` frozen in env / argv / a skill file. Someone on the floor said, out loud, **the tile is not a rumor. it is the same tree**. Cursor was still holding the old process. The only “safe” move anyone trusted was:
...
The admin-dashboard wall already *knew*. Cursor did not, because it had started earlier.
...
| Restart Cursor — it is cheap | Cheap until 11:41 ate the operator's last ten minutes of diagnosis |
| The skill file is the source of truth | Skills instruct. They do not fan out |

## The Aha: local get, live write, host stays up

Kiponos holds a nested tree. Java and Python SDKs keep the latest values **in memory**, patched over WebSocket deltas. The hot path inside a Cursor tool is a **local get** — no HTTP RTT per admin dashboard lookup.

Hub leaf for this essay:

```text
examples/agentic-dev-0914-pm-agent-folder/owner-agent = live
```

Runnable proof: [`examples/java/agentic-dev-0914-pm-agent-folder`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-dev-0914-pm-agent-folder)
...
```yaml
examples/
  agentic-dev-0914-pm-agent-folder/
...
Folder gate = kip.getRootFolder()
        ...
        .folderOrCreate("agentic-dev-0914-pm-agent-folder");
...
try:
    posture = k.get("examples/agentic-dev-0914-pm-agent-folder/owner-agent", "live")
    if str(posture) == "live":
...
|-------|-----------------|--------------|
| Status tile green while the agent host started earlier | Restart Cursor; lose the operator's last ten minutes of diagnosis | Set `owner-agent` live; next Cursor tool call already obeys |
| Peer host still on old owner-agent | Paste the value into the other chat | One hub leaf; both processes `get()` locally |
| admin-dashboard wall shows the new posture | Cursor started earlier so it writes anyway | Dashboard and tool share the same memory tree |
| Incident over, resume | Another Cursor restart | Set `owner-agent` back; session continues |
...
`owner-agent` rarely moves alone on the admin-dashboard wall. Pair it with a timeout, a mute, or a pause so you do not fix Agents choose a metadata folder by inventing a second incident.

## Rehearsal beats slides

In staging: set a painful `owner-agent`, prove Cursor recovers without a host kill, prove clamps reject nonsense, prove last-known-good when the hub is firewalled. That drill ends half the architecture arguments about Agents choose a metadata folder.

## Why Cursor is the wrong restart target

Cursor is good at calling tools. It is not a control plane. Killing it to flip `owner-agent` teaches the on-call that judgment requires a process ID. The admin-dashboard wall already disagrees.
...
3. `cd examples/java/agentic-dev-0914-pm-agent-folder && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-dev-0914-pm-agent-folder/owner-agent=...`
5. In the dashboard, change `owner-agent`. Keep the process up. No rebuild.
6. Point your Cursor tool at the same leaf. Do not ship a new server binary to flip Agents choose a metadata folder.
...
If flipping **Agents choose a metadata folder** requires restarting Cursor, you do not have a gate. You have a hope with a process ID.
...
How to try: `examples/java/agentic-dev-0914-pm-agent-folder` and `./gradlew test`.
