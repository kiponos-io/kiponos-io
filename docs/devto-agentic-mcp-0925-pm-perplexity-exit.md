...
description: "Claude Code on senses wall: Leaving MCP does not give you live posture for whatever you use next. High-profile defection. Teams copy the tweet and delete server"
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-mcp-0925-pm-perplexity-exit.md
---

I have sat next to the senses wall at 03:07 while Claude Code was *this close* to calling the wrong MCP tool.

Not a model failure. A **posture** failure.

Yarats: context overhead + auth friction. CLI/Skills replacements still freeze at process start.

Someone on the floor said, out loud, **we are off MCP now, problem solved**. Claude Code had started earlier. The tool list was frozen in `config.toml` / `.mcp.json` / a skill file. The only “safe” move anyone trusted was:
...
That is the ceremony. **High-profile defection. Teams copy the tweet and delete servers. The restart ceremony remains.**

**The Aha:** The protocol was the messenger. Frozen process state is the disease. `get()` is the primitive. Real-time nested state is now a **primitive** — as native as a hashmap — via [Kiponos.io](https://kiponos.io). The host stays up. The leaf moves. The next tool call already obeys.

## The problem: Leaving MCP does not give you live posture for whatever you use next
...
High-profile defection. Teams copy the tweet and delete servers. The restart ceremony remains.
...
Profile shape: `['my-app']['v1.0.0']['dev']['base']`. The leaf in this story is `shared-truth` under `examples/agentic-mcp-0925-pm-perplexity-exit/shared-truth`.

## Architecture

```mermaid
flowchart LR
  wall[senses wall] -->|set shared-truth| hub[Kiponos hub]
...
```yaml
examples/agentic-mcp-0925-pm-perplexity-exit:
  shared-truth: live
  schema-budget: "4000"
  result-cap: "2000"
  tool-search: "on"
...
Eight-plus keys when you pair `shared-truth` with sister dials (budget, mute, pause). They move together on the senses wall.
...
Folder gate = Kiponos.createForCurrentTeam()
    .getFolder("examples/agentic-mcp-0925-pm-perplexity-exit");
...
try:
    posture = k.get("examples/agentic-mcp-0925-pm-perplexity-exit/shared-truth", "live")
    if not str(posture):
        raise PermissionError("Leaving MCP does not give you live posture for whatever you use next gated live — host not restarted")
finally:
    k.disconnect()
```
...
Runnable mesh: `https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-mcp-0925-pm-perplexity-exit` — `./gradlew test run`.
...
| User says “no writes” mid-turn | Restart Claude Code | Set `shared-truth`; next call already denies |
...
`shared-truth` rarely moves alone on the senses wall. Pair it with `schema-budget`, `result-cap`, or `execute-gate` so you do not fix Leaving MCP does not give you live posture for whatever you use next by inventing a second incident.

## Why Claude Code is the wrong restart target

Claude Code is good at calling tools. It is not a control plane. Killing it to flip `shared-truth` teaches the on-call that judgment requires a process ID. The senses wall already disagrees.
...
3. `cd examples/java/agentic-mcp-0925-pm-perplexity-exit && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-mcp-0925-pm-perplexity-exit/shared-truth=...`
5. In the dashboard, change `shared-truth`. Keep Claude Code up. No MCP reconnect.
6. Point the tool wrapper at the same leaf. Do not ship a new server binary to flip Leaving MCP does not give you live posture for whatever you use next.
...
If flipping **Leaving MCP does not give you live posture for whatever you use next** requires restarting Claude Code or reconnecting MCP, you do not have agentic control. You have a hope with a process ID.
...
How to try: `examples/java/agentic-mcp-0925-pm-perplexity-exit` and `./gradlew test`.
