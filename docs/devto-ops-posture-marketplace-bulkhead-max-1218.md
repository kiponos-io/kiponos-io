---
title: "The Marketplace War Room Already Knew bulkheadMax — The Jar Did Not"
published: false
tags: java, architecture, devops, kiponos
description: "Live bulkhead concurrency via Kiponos Python SDK — marketplace posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-marketplace-bulkhead-max-1218.md
main_image: ./devto-cover-ops-posture-marketplace-bulkhead-max-1218.jpg
---
...
Domain: seller tiers, fee weights. This essay maps hub key `bulkheadMax` to **bulkhead concurrency** so the lesson stays concrete for marketplace operators.
...
```python
policy = kiponos.path("examples", "ops-marketplace-bulkhead-max")
bulkheadMax = int(policy.get("bulkheadMax", 40))
if not limiter.try_acquire(bulkheadMax):
    return too_many()
return handle(req)
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-marketplace-bulkhead-max/bulkheadMax`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: bulkhead concurrency for marketplace — numbers humans already change verbally in war rooms.
