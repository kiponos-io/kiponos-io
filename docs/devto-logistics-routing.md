...
When **max detour km** is frozen in a deploy unit, every incident becomes a process argument. Hotfixes still mean CI + roll. Feature flags often mean a second control plane with its own delay.
...
```java
Folder policy = kiponos.path("examples", "logistics-routing");
int maxDetourKm = policy.getInt("maxDetourKm"); // local get — no hub RTT
if (maxDetourKm <= 0) {
    return failClosed();
}
return apply(maxDetourKm);
```

Ops (or automation) sets `maxDetourKm` in the hub. The **next** evaluation uses it. Same binary. Same tests for structure.
...
- Canonical source: [this article on GitHub](https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-logistics-routing.md)
...
Feature flags are often product gates. This essay is about **ops posture** on fleet routing: **max detour km** — numbers war rooms already shout.
...
Treat `maxDetourKm` as a slice of error budget. Raise when the world is healthy; lower when dependencies are sick. Write the number that survives a bad day.
...
**Max detour km** that requires a deploy is optimistic documentation.
