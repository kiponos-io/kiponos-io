...
description: "Claude Code on admin-dashboard wall: Namespaced tools, live csv. People allow 'github' and accidentally allow github__delete_repo"
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-mcp-0930-am-namespace-tools.md
---

I have sat next to the admin-dashboard wall at 03:07 while Claude Code was *this close* to calling the wrong MCP tool.

Not a model failure. A **posture** failure.

SpaceXAI docs: tools namespaced `<server>__<tool>`. Granularity is the product.

Someone on the floor said, out loud, **github is on, we need it**. Claude Code had started earlier. The tool list was frozen in `config.toml` / `.mcp.json` / a skill file. The only “safe” move anyone trusted was:
...
That is the ceremony. **People allow 'github' and accidentally allow github__delete_repo**

**The Aha:** Allow `github__search_issues`, not the server. Leaf is csv of real names. Real-time nested state is now a **primitive** — as native as a hashmap — via [Kiponos.io](https://kiponos.io). The host stays up. The leaf moves. The next tool call already obeys.

## The problem: Namespaced tools, live csv
...
People allow 'github' and accidentally allow github__delete_repo
...
Profile shape: `['my-app']['v1.0.0']['dev']['base']`. The leaf in this story is `tools-allow` under `examples/agentic-mcp-0930-am-namespace-tools/tools-allow`.

## Architecture

```mermaid
flowchart LR
  wall[admin-dashboard wall] -->|set tools-allow| hub[Kiponos hub]
...
```yaml
examples/agentic-mcp-0930-am-namespace-tools:
...
Folder gate = Kiponos.createForCurrentTeam()
    .getFolder("examples/agentic-mcp-0930-am-namespace-tools");
...
try:
    posture = k.get("examples/agentic-mcp-0930-am-namespace-tools/tools-allow", "live")
    if not str(posture):
        raise PermissionError("Namespaced tools, live csv gated live — host not restarted")
finally:
    k.disconnect()
```
...
Runnable mesh: `https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-mcp-0930-am-namespace-tools` — `./gradlew test run`.
...
`tools-allow` rarely moves alone on the admin-dashboard wall. Pair it with `schema-budget`, `result-cap`, or `execute-gate` so you do not fix Namespaced tools, live csv by inventing a second incident.

## Why Claude Code is the wrong restart target

Claude Code is good at calling tools. It is not a control plane. Killing it to flip `tools-allow` teaches the on-call that judgment requires a process ID. The admin-dashboard wall already disagrees.
...
3. `cd examples/java/agentic-mcp-0930-am-namespace-tools && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-mcp-0930-am-namespace-tools/tools-allow=...`
5. In the dashboard, change `tools-allow`. Keep Claude Code up. No MCP reconnect.
6. Point the tool wrapper at the same leaf. Do not ship a new server binary to flip Namespaced tools, live csv.
...
If flipping **Namespaced tools, live csv** requires restarting Claude Code or reconnecting MCP, you do not have agentic control. You have a hope with a process ID.
...
How to try: `examples/java/agentic-mcp-0930-am-namespace-tools` and `./gradlew test`.
