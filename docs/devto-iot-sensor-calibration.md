...
When **sensor threshold** is frozen in a deploy unit, every incident becomes a process argument. Hotfixes still mean CI + roll. Feature flags often mean a second control plane with its own delay.
...
```java
Folder policy = kiponos.path("examples", "iot-sensor-calibration");
int threshold = policy.getInt("threshold"); // local get — no hub RTT
if (threshold <= 0) {
    return failClosed();
}
...
Ops (or automation) sets `threshold` in the hub. The **next** evaluation uses it. Same binary. Same tests for structure.
...
- Canonical source: [this article on GitHub](https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-iot-sensor-calibration.md)
...
Feature flags are often product gates. This essay is about **ops posture** on edge calibration: **sensor threshold** — numbers war rooms already shout.
...
Treat `threshold` as a slice of error budget. Raise when the world is healthy; lower when dependencies are sick. Write the number that survives a bad day.
...
**Sensor threshold** that requires a deploy is optimistic documentation.
