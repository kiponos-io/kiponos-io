---
title: "We Stopped Shipping Rag Top-K as a Constant (Insurance)"
published: false
...
main_image: ./devto-cover-ops-posture-insurance-rag-topk-0636.jpg
---
...
Domain: underwriting, claims velocity. This essay maps hub key `topK` to **RAG top-k** so the lesson stays concrete for insurance operators.
...
```python
policy = kiponos.path("examples", "ops-insurance-rag-topk")
...
1. Name the hub path so humans find it under pressure (`examples/ops-insurance-rag-topk/topK`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: RAG top-k for insurance — numbers humans already change verbally in war rooms.
