---
title: "The Edtech War Room Already Knew hpaTarget — The Jar Did Not"
published: false
...
Domain: class size, exam windows. This essay maps hub key `hpaTarget` to **HPA CPU target percent** so the lesson stays concrete for edtech operators.
...
```python
policy = kiponos.path("examples", "ops-edtech-hpa-target")
...
1. Name the hub path so humans find it under pressure (`examples/ops-edtech-hpa-target/hpaTarget`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: HPA CPU target percent for edtech — numbers humans already change verbally in war rooms.
