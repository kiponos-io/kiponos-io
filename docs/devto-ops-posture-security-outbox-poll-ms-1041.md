---
title: "We Stopped Shipping Outbox Poll Interval as a Constant (Security)"
published: false
tags: java, security, devops, kiponos
description: "Live outbox poll interval via Kiponos Java SDK — security posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-security-outbox-poll-ms-1041.md
main_image: ./devto-cover-ops-posture-security-outbox-poll-ms-1041.jpg
---
...
Domain: WAF, RPS, auth leeway. This essay maps hub key `outboxPollMs` to **outbox poll interval** so the lesson stays concrete for security operators.
...
```java
Folder policy = kiponos.path("examples", "ops-security-outbox-poll-ms");
int outboxPollMs = Math.min(policy.getInt("outboxPollMs"), HARD_MAX);
limiter.setLimit(outboxPollMs); // next evaluation sees it
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-security-outbox-poll-ms/outboxPollMs`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: outbox poll interval for security — numbers humans already change verbally in war rooms.
