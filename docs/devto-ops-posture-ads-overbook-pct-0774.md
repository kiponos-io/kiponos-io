---
title: "Ads: Turn overbooking percent Into Incident Posture"
published: false
tags: java, advertising, devops, kiponos
description: "Live overbooking percent via Kiponos Java SDK — ads posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-ads-overbook-pct-0774.md
main_image: https://files.catbox.moe/f65p78.jpg
---
...
SEV-2 started as a metric spike and became a ceremony debate: who can change overbookPercent without a PR.

Domain: bid caps, frequency, pacing. This essay maps hub key `overbookPercent` to **overbooking percent** so the lesson stays concrete for ads operators.
...
```java
var policy = kiponos.path("examples", "ops-ads-overbook-pct");
String mode = policy.getString("degradeMode", "full");
int overbookPercent = policy.getInt("overbookPercent");
return router.decide(mode, overbookPercent);
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-ads-overbook-pct/overbookPercent`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: overbooking percent for ads — numbers humans already change verbally in war rooms.

Kiponos makes that verbal decision **executable** without a second control plane tax on every request.

## Pair with a sister dial

`overbookPercent` rarely moves alone. Pair with timeout/retry, canary share, or sampling so you do not fix one symptom by creating another.

## Failure budget vs this dial

Treat `overbookPercent` as a slice of **error budget**, not a comfort blanket. Raise it when the dependency is healthy; lower it when the dependency is already sick. Write the number that survives a bad day, not the number that flatters a sunny demo.
