---
title: "The Cdn War Room Already Knew hpaTarget — The Jar Did Not"
published: false
...
main_image: https://iili.io/CU9jcdv.jpg
---
...
Domain: edge rules, cache TTL, purge. This essay maps hub key `hpaTarget` to **HPA CPU target percent** so the lesson stays concrete for cdn operators.
...
```java
Folder policy = kiponos.path("examples", "ops-cdn-hpa-target");
...
1. Name the hub path so humans find it under pressure (`examples/ops-cdn-hpa-target/hpaTarget`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: HPA CPU target percent for cdn — numbers humans already change verbally in war rooms.
