---
title: "We Stopped Shipping Outbox Poll Interval as a Constant (Logistics)"
published: false
tags: java, logistics, devops, kiponos
description: "Live outbox poll interval via Kiponos Java SDK — logistics posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-logistics-outbox-poll-ms-0341.md
main_image: ./devto-cover-ops-posture-logistics-outbox-poll-ms-0341.jpg
---
...
Domain: routing, SLA windows, fleet. This essay maps hub key `outboxPollMs` to **outbox poll interval** so the lesson stays concrete for logistics operators.
...
```java
var policy = kiponos.path("examples", "ops-logistics-outbox-poll-ms");
String mode = policy.getString("degradeMode", "full");
int outboxPollMs = policy.getInt("outboxPollMs");
return router.decide(mode, outboxPollMs);
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-logistics-outbox-poll-ms/outboxPollMs`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: outbox poll interval for logistics — numbers humans already change verbally in war rooms.
