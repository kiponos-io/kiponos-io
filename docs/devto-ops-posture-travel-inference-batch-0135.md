---
title: "Inference Batch Size Should Not Wait for a Redeploy — Travel Live Posture"
published: false
...
Domain: inventory, overbooking, GDS. This essay maps hub key `batchSize` to **inference batch size** so the lesson stays concrete for travel operators.
...
```python
policy = kiponos.path("examples", "ops-travel-inference-batch")
...
1. Name the hub path so humans find it under pressure (`examples/ops-travel-inference-batch/batchSize`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: inference batch size for travel — numbers humans already change verbally in war rooms.
