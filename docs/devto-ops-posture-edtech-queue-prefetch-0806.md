---
title: "We Stopped Shipping Consumer Prefetch as a Constant (Edtech)"
published: false
...
Domain: class size, exam windows. This essay maps hub key `prefetch` to **consumer prefetch** so the lesson stays concrete for edtech operators.
...
```java
var policy = kiponos.path("examples", "ops-edtech-queue-prefetch");
...
1. Name the hub path so humans find it under pressure (`examples/ops-edtech-queue-prefetch/prefetch`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: consumer prefetch for edtech — numbers humans already change verbally in war rooms.
