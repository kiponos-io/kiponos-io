---
title: "Retail: Turn per-tenant quota Into Incident Posture"
published: false
tags: java, retail, devops, kiponos
description: "Live per-tenant quota via Kiponos Java SDK — retail posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-retail-tenant-quota-0029.md
main_image: ./devto-cover-ops-posture-retail-tenant-quota-0029.jpg
---
...
Domain: checkout, merchandising, inventory. This essay maps hub key `quotaPerMin` to **per-tenant quota** so the lesson stays concrete for retail operators.
...
```java
Folder policy = kiponos.path("examples", "ops-retail-tenant-quota");
...
1. Name the hub path so humans find it under pressure (`examples/ops-retail-tenant-quota/quotaPerMin`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: per-tenant quota for retail — numbers humans already change verbally in war rooms.
