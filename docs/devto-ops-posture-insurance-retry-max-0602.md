---
title: "Live Retry Budget for Insurance — Mouth to Process in Seconds"
published: false
tags: java, insurance, architecture, kiponos
description: "Live retry budget via Kiponos Java SDK — insurance posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-insurance-retry-max-0602.md
main_image: ./devto-cover-ops-posture-insurance-retry-max-0602.jpg
---
...
Domain: underwriting, claims velocity. This essay maps hub key `maxAttempts` to **retry budget** so the lesson stays concrete for insurance operators.
...
```java
Folder policy = kiponos.path("examples", "ops-insurance-retry-max");
...
1. Name the hub path so humans find it under pressure (`examples/ops-insurance-retry-max/maxAttempts`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: retry budget for insurance — numbers humans already change verbally in war rooms.
