...
description: "Cursor on admin-dashboard wall: Live shrink beats a bigger window. RAG-MCP: accuracy collapsed as tool count rose; 1M windows do not fix selection"
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-mcp-0916-pm-selection-drop.md
---
...
More tools, paradoxically, means worse tool use. That is architecture, not model IQ.

Someone on the floor said, out loud, **give it more context, it will pick the right tool**. Cursor had started earlier. The tool list was frozen in `config.toml` / `.mcp.json` / a skill file. The only “safe” move anyone trusted was:
...
That is the ceremony. **RAG-MCP: accuracy collapsed as tool count rose; 1M windows do not fix selection**

**The Aha:** Accuracy is the size of the live set. Cut `tools-allow`. Keep the 200k for the problem. Real-time nested state is now a **primitive** — as native as a hashmap — via [Kiponos.io](https://kiponos.io). The host stays up. The leaf moves. The next tool call already obeys.

## The problem: Live shrink beats a bigger window
...
RAG-MCP: accuracy collapsed as tool count rose; 1M windows do not fix selection
...
Profile shape: `['my-app']['v1.0.0']['dev']['base']`. The leaf in this story is `tools-allow` under `examples/agentic-mcp-0916-pm-selection-drop/tools-allow`.
...
```yaml
examples/agentic-mcp-0916-pm-selection-drop:
...
Folder gate = Kiponos.createForCurrentTeam()
    .getFolder("examples/agentic-mcp-0916-pm-selection-drop");
...
try:
    posture = k.get("examples/agentic-mcp-0916-pm-selection-drop/tools-allow", "live")
    if not str(posture):
        raise PermissionError("Live shrink beats a bigger window gated live — host not restarted")
finally:
    k.disconnect()
```
...
Runnable mesh: `https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-mcp-0916-pm-selection-drop` — `./gradlew test run`.
...
`tools-allow` rarely moves alone on the admin-dashboard wall. Pair it with `schema-budget`, `result-cap`, or `execute-gate` so you do not fix Live shrink beats a bigger window by inventing a second incident.
...
3. `cd examples/java/agentic-mcp-0916-pm-selection-drop && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-mcp-0916-pm-selection-drop/tools-allow=...`
5. In the dashboard, change `tools-allow`. Keep Cursor up. No MCP reconnect.
6. Point the tool wrapper at the same leaf. Do not ship a new server binary to flip Live shrink beats a bigger window.
...
If flipping **Live shrink beats a bigger window** requires restarting Cursor or reconnecting MCP, you do not have agentic control. You have a hope with a process ID.
...
How to try: `examples/java/agentic-mcp-0916-pm-selection-drop` and `./gradlew test`.
