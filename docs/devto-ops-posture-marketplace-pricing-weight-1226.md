---
title: "We Stopped Shipping Pricing / Promo Weight as a Constant (Marketplace)"
published: false
tags: java, architecture, devops, kiponos
description: "Live pricing / promo weight via Kiponos Java SDK — marketplace posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-marketplace-pricing-weight-1226.md
main_image: ./devto-cover-ops-posture-marketplace-pricing-weight-1226.jpg
---
...
Domain: seller tiers, fee weights. This essay maps hub key `weight` to **pricing / promo weight** so the lesson stays concrete for marketplace operators.
...
```yaml
examples/
  ops-marketplace-pricing-weight/
...
```java
Folder policy = kiponos.path("examples", "ops-marketplace-pricing-weight");
...
1. Name the hub path so humans find it under pressure (`examples/ops-marketplace-pricing-weight/weight`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: pricing / promo weight for marketplace — numbers humans already change verbally in war rooms.
...
`weight` rarely moves alone. Pair with timeout/retry, canary share, or sampling so you do not fix one symptom by creating another.
...
pricing / promo weight that requires a deploy is optimistic documentation.
