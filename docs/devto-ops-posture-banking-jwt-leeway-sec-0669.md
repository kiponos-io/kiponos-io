---
title: "Banking: Turn JWT clock skew leeway Into Incident Posture"
published: false
...
Domain: AML thresholds, transfer velocity. This essay maps hub key `leewaySec` to **JWT clock skew leeway** so the lesson stays concrete for banking operators.
...
```python
policy = kiponos.path("examples", "ops-banking-jwt-leeway-sec")
...
1. Name the hub path so humans find it under pressure (`examples/ops-banking-jwt-leeway-sec/leewaySec`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: JWT clock skew leeway for banking — numbers humans already change verbally in war rooms.
