---
title: "We Stopped Shipping Graceful Drain Window as a Constant (Saas)"
published: false
tags: java, saas, architecture, kiponos
description: "Live graceful drain window via Kiponos Python SDK — saas posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-saas-drain-ms-0266.md
main_image: ./devto-cover-ops-posture-saas-drain-ms-0266.jpg
---
...
Domain: multi-tenant quotas, feature posture. This essay maps hub key `gracefulShutdownMs` to **graceful drain window** so the lesson stays concrete for saas operators.
...
```python
policy = kiponos.path("examples", "ops-saas-drain-ms")
...
1. Name the hub path so humans find it under pressure (`examples/ops-saas-drain-ms/gracefulShutdownMs`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: graceful drain window for saas — numbers humans already change verbally in war rooms.
