---
main_image: https://iili.io/n9AXz1p.jpg
...
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-dev-1007-pm-shopping-pause.md
---

I have sat next to the travel-coordinator wall at 00:40 while Grok Build was *this close* to doing the wrong thing.
...
```text
examples/agentic-dev-1007-pm-shopping-pause/incident-pause = paused
```

Runnable proof: [`examples/java/agentic-dev-1007-pm-shopping-pause`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-dev-1007-pm-shopping-pause)
...
```yaml
examples/
  agentic-dev-1007-pm-shopping-pause/
...
Folder gate = kip.getRootFolder()
        ...
        .folderOrCreate("agentic-dev-1007-pm-shopping-pause");
...
try:
    posture = k.get("examples/agentic-dev-1007-pm-shopping-pause/incident-pause", "paused")
...
In staging: set a painful `incident-pause`, prove Grok Build recovers without a host kill, prove clamps reject nonsense, prove last-known-good when the hub is firewalled. That drill ends half the architecture arguments about Shopping incident pause as a leaf.
...
At 00:40 someone said, out loud: **mute writes on the noisy room — keep reads**. That sentence is the whole product. If it cannot land in the running Grok Build process in seconds, you do not have posture. You have a wiki.
...
3. `cd examples/java/agentic-dev-1007-pm-shopping-pause && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-dev-1007-pm-shopping-pause/incident-pause=...`
...
How to try: `examples/java/agentic-dev-1007-pm-shopping-pause` and `./gradlew test`.
