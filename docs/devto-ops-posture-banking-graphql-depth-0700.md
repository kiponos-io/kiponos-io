---
title: "Graphql Query Depth Should Not Wait for a Redeploy — Banking Live Posture"
published: false
tags: java, banking, security, kiponos
description: "Live GraphQL query depth via Kiponos Java SDK — banking posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-banking-graphql-depth-0700.md
main_image: https://files.catbox.moe/7gw17n.jpg
---
...
Domain: AML thresholds, transfer velocity. This essay maps hub key `maxDepth` to **GraphQL query depth** so the lesson stays concrete for banking operators.
...
```java
Folder policy = kiponos.path("examples", "ops-banking-graphql-depth");
int maxDepth = policy.getInt("maxDepth");
if (maxDepth <= 0) {
    return failClosed(); // money paths
}
return apply(maxDepth);
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-banking-graphql-depth/maxDepth`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: GraphQL query depth for banking — numbers humans already change verbally in war rooms.
...
2. Move `maxDepth` to the emergency value (documented floor/ceiling).  
