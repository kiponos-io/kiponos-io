---
title: "The Gaming War Room Already Knew idempotencyWindowSec — The Jar Did Not"
published: false
...
Domain: matchmaking, session caps, tick rate. This essay maps hub key `idempotencyWindowSec` to **idempotency window** so the lesson stays concrete for gaming operators.
...
```java
var policy = kiponos.path("examples", "ops-gaming-idempotency-win");
...
1. Name the hub path so humans find it under pressure (`examples/ops-gaming-idempotency-win/idempotencyWindowSec`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: idempotency window for gaming — numbers humans already change verbally in war rooms.
