---
title: "We Stopped Shipping Cors Max-Age as a Constant (Mlops)"
published: false
...
main_image: https://iili.io/nIFffjt.jpg
---
...
Domain: batch size, score thresholds. This essay maps hub key `corsMaxAgeSec` to **CORS max-age** so the lesson stays concrete for mlops operators.
...
```java
var policy = kiponos.path("examples", "ops-mlops-cors-max-age");
String mode = policy.getString("degradeMode", "full");
int corsMaxAgeSec = policy.getInt("corsMaxAgeSec");
return router.decide(mode, corsMaxAgeSec);
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-mlops-cors-max-age/corsMaxAgeSec`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: CORS max-age for mlops — numbers humans already change verbally in war rooms.
