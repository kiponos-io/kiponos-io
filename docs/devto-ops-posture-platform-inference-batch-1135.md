---
title: "Inference Batch Size Should Not Wait for a Redeploy — Platform Live Posture"
published: false
...
main_image: https://iili.io/CZOCrZl.jpg
---
...
Domain: shared libraries, defaults. This essay maps hub key `batchSize` to **inference batch size** so the lesson stays concrete for platform operators.
...
```java
var policy = kiponos.path("examples", "ops-platform-inference-batch");
...
1. Name the hub path so humans find it under pressure (`examples/ops-platform-inference-batch/batchSize`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: inference batch size for platform — numbers humans already change verbally in war rooms.
