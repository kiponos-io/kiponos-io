---
title: "We Stopped Shipping Graceful Drain Window as a Constant (Retail)"
published: false
...
main_image: https://iili.io/nRf4OIn.jpg
---
...
Domain: checkout, merchandising, inventory. This essay maps hub key `gracefulShutdownMs` to **graceful drain window** so the lesson stays concrete for retail operators.
...
```java
Folder policy = kiponos.path("examples", "ops-retail-drain-ms");
...
1. Name the hub path so humans find it under pressure (`examples/ops-retail-drain-ms/gracefulShutdownMs`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: graceful drain window for retail — numbers humans already change verbally in war rooms.
