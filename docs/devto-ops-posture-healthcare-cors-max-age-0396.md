---
title: "We Stopped Shipping Cors Max-Age as a Constant (Healthcare)"
published: false
tags: java, healthcare, architecture, kiponos
description: "Live CORS max-age via Kiponos Java SDK — healthcare posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-healthcare-cors-max-age-0396.md
main_image: ./devto-cover-ops-posture-healthcare-cors-max-age-0396.jpg
---
...
The partner brownout did not wait for your change-management window. Your CORS max-age did — until it lived in a hub.

Domain: routing, queue depth, triage. This essay maps hub key `corsMaxAgeSec` to **CORS max-age** so the lesson stays concrete for healthcare operators.
...
```java
Folder policy = kiponos.path("examples", "ops-healthcare-cors-max-age");
...
1. Name the hub path so humans find it under pressure (`examples/ops-healthcare-cors-max-age/corsMaxAgeSec`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: CORS max-age for healthcare — numbers humans already change verbally in war rooms.
