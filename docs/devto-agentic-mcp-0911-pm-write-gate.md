...
description: "Grok Build on shopping-admin wall: Write tools gated live without MCP reconnect. The only 'safe' move was restart the host so argv no longer listed write"
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-mcp-0911-pm-write-gate.md
---

I have sat next to the shopping-admin wall at 11:41 while Grok Build was *this close* to calling the wrong MCP tool.

Not a model failure. A **posture** failure.

Same ceremony as a 3am kill-switch — except the switch is which MCP tools exist.

Someone on the floor said, out loud, **take write off the server until checkout is calm**. Grok Build had started earlier. The tool list was frozen in `config.toml` / `.mcp.json` / a skill file. The only “safe” move anyone trusted was:
...
That is the ceremony. **The only 'safe' move was restart the host so argv no longer listed write**

**The Aha:** `tools-allow` without `write`. Next tool call already refuses. Session continues. Real-time nested state is now a **primitive** — as native as a hashmap — via [Kiponos.io](https://kiponos.io). The host stays up. The leaf moves. The next tool call already obeys.

## The problem: Write tools gated live without MCP reconnect
...
The only 'safe' move was restart the host so argv no longer listed write
...
Profile shape: `['my-app']['v1.0.0']['dev']['base']`. The leaf in this story is `tools-allow` under `examples/agentic-mcp-0911-pm-write-gate/tools-allow`.

## Architecture

![Architecture diagram](https://iili.io/n3CPaKg.png)
...
```yaml
examples/agentic-mcp-0911-pm-write-gate:
...
Folder gate = Kiponos.createForCurrentTeam()
    .getFolder("examples/agentic-mcp-0911-pm-write-gate");
...
try:
    posture = k.get("examples/agentic-mcp-0911-pm-write-gate/tools-allow", "live")
    if not str(posture):
        raise PermissionError("Write tools gated live without MCP reconnect gated live — host not restarted")
finally:
    k.disconnect()
```
...
Runnable mesh: `https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-mcp-0911-pm-write-gate` — `./gradlew test run`.
...
`tools-allow` rarely moves alone on the shopping-admin wall. Pair it with `schema-budget`, `result-cap`, or `execute-gate` so you do not fix Write tools gated live without MCP reconnect by inventing a second incident.
...
3. `cd examples/java/agentic-mcp-0911-pm-write-gate && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-mcp-0911-pm-write-gate/tools-allow=...`
5. In the dashboard, change `tools-allow`. Keep Grok Build up. No MCP reconnect.
6. Point the tool wrapper at the same leaf. Do not ship a new server binary to flip Write tools gated live without MCP reconnect.
...
If flipping **Write tools gated live without MCP reconnect** requires restarting Grok Build or reconnecting MCP, you do not have agentic control. You have a hope with a process ID.
...
How to try: `examples/java/agentic-mcp-0911-pm-write-gate` and `./gradlew test`.
