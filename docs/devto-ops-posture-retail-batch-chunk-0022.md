---
title: "Live Batch Chunk Size for Retail — Mouth to Process in Seconds"
published: false
...
main_image: https://d.uguu.se/FrvqXofR.jpg
---
...
Domain: checkout, merchandising, inventory. This essay maps hub key `chunkSize` to **batch chunk size** so the lesson stays concrete for retail operators.
...
```java
Folder policy = kiponos.path("examples", "ops-retail-batch-chunk");
...
1. Name the hub path so humans find it under pressure (`examples/ops-retail-batch-chunk/chunkSize`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: batch chunk size for retail — numbers humans already change verbally in war rooms.
