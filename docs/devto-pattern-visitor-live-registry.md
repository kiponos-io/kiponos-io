---
title: "Visitor as a Super Pattern — Live Visitor Registry"
published: false
...
**The Aha:** Which visitors run over the tree is hub policy

Audit, metrics, and transform visitors should not all be hardwired for every deploy.
...
```yaml
patterns/visitor/registry/
  # live knobs
  visitors: metrics,audit
```

Hot path (local memory after connect — WebSocket deltas, no per-request hub RTT):

```java
Folder policy = kiponos.path('patterns', 'visitor', 'registry');
// local gets — never remote on the money path
for (String v : visitors) node.accept(registry.get(v));
```
...
Full tree: [https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/pattern-visitor-live-registry](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/pattern-visitor-live-registry)
...
Visitors are code. Which ones walk tonight is live.
...
*Runnable: [pattern-visitor-live-registry](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/pattern-visitor-live-registry) · [kiponos.io](https://kiponos.io)*
