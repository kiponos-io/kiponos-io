...
description: "Live theme string on the Kiponos hub from a Angular server peer — createFromEnv, no SPA secrets, Java parity. Install @kiponos/angular."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-angular-sdk-live-theme.md
---

**The Aha:** Theme string is a judgment call under pressure. If the only path is a SPA rebuild, you do not have a control plane — you have a ceremony.

I have sat through a room that looked like this: design lead waiting for an Angular rebuild to flip dark mode. Someone said “we just need to flip it.” Someone else opened a PR. CI ran. The window closed.
...
- Companion Java tree: `examples/java/angular-sdk-live-theme` (same hub moral)
...
Flip `theme` (or your domain leaf) on the dashboard or from the Node peer. The Java print should follow the hub — not the last SPA deploy.
...
|------|--------------------------------|------------------------|
| Change theme string | Ship SPA / service build | Dashboard or SDK `set()` |
...
1. Name the hub path out loud: `ui/theme`  
...
Identity is **where the process runs**. Angular’s job is to be a first-class peer on the hub, not a bundler that smuggles secrets. Ship the peer once. Leave the bundle alone when the only thing that changed is **theme string**.
