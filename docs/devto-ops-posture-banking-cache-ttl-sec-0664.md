---
title: "Banking: Turn cache TTL seconds Into Incident Posture"
published: false
...
Domain: AML thresholds, transfer velocity. This essay maps hub key `ttlSec` to **cache TTL seconds** so the lesson stays concrete for banking operators.
...
```java
Folder policy = kiponos.path("examples", "ops-banking-cache-ttl-sec");
...
1. Name the hub path so humans find it under pressure (`examples/ops-banking-cache-ttl-sec/ttlSec`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: cache TTL seconds for banking — numbers humans already change verbally in war rooms.
