---
title: "Live Trace Sample Per Thousand for Mobile — Mouth to Process in Seconds"
published: false
...
main_image: https://iili.io/nu1C3pp.jpg
---
...
Domain: API timeouts, payload caps. This essay maps hub key `samplePerThousand` to **trace sample per thousand** so the lesson stays concrete for mobile operators.
...
```java
Folder policy = kiponos.path("examples", "ops-mobile-trace-sample");
...
1. Name the hub path so humans find it under pressure (`examples/ops-mobile-trace-sample/samplePerThousand`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: trace sample per thousand for mobile — numbers humans already change verbally in war rooms.
