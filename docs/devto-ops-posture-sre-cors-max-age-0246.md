---
title: "We Stopped Shipping Cors Max-Age as a Constant (Sre)"
published: false
tags: java, sre, devops, kiponos
description: "Live CORS max-age via Kiponos Python SDK — sre posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-sre-cors-max-age-0246.md
main_image: ./devto-cover-ops-posture-sre-cors-max-age-0246.jpg
---
...
Domain: canary, drain, error budget. This essay maps hub key `corsMaxAgeSec` to **CORS max-age** so the lesson stays concrete for sre operators.
...
```python
policy = kiponos.path("examples", "ops-sre-cors-max-age")
...
1. Name the hub path so humans find it under pressure (`examples/ops-sre-cors-max-age/corsMaxAgeSec`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: CORS max-age for sre — numbers humans already change verbally in war rooms.
