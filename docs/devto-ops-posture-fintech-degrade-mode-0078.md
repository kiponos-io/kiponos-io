---
title: "The Fintech War Room Already Knew degradeMode — The Jar Did Not"
published: false
...
Domain: payments, fraud, ledger. This essay maps hub key `degradeMode` to **degradation mode** so the lesson stays concrete for fintech operators.
...
```java
Folder policy = kiponos.path("examples", "ops-fintech-degrade-mode");
...
1. Name the hub path so humans find it under pressure (`examples/ops-fintech-degrade-mode/degradeMode`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: degradation mode for fintech — numbers humans already change verbally in war rooms.
