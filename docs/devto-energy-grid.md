...
Grid load dispatch does not wait for your change-management window. Your `dispatchLimitMw` did — until it lived in a hub.

## The problem: ceremony between judgment and effect

When **dispatch limit MW** is frozen in a deploy unit, every incident becomes a process argument. Hotfixes still mean CI + roll. Feature flags often mean a second control plane with its own delay.
...
examples/
  energy-grid/
    dispatchLimitMw: <live>
```

```java
Folder policy = kiponos.path("examples", "energy-grid");
int dispatchLimitMw = policy.getInt("dispatchLimitMw"); // local get — no hub RTT
if (dispatchLimitMw <= 0) {
    return failClosed();
}
return apply(dispatchLimitMw);
```

Ops (or automation) sets `dispatchLimitMw` in the hub. The **next** evaluation uses it. Same binary. Same tests for structure.
...
- Canonical source: [this article on GitHub](https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-energy-grid.md)
...
Feature flags are often product gates. This essay is about **ops posture** on grid load dispatch: **dispatch limit MW** — numbers war rooms already shout.
...
Treat `dispatchLimitMw` as a slice of error budget. Raise when the world is healthy; lower when dependencies are sick. Write the number that survives a bad day.
...
**Dispatch limit MW** that requires a deploy is optimistic documentation.
