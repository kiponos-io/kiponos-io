---
title: "We Stopped Shipping Cors Max-Age as a Constant (Energy)"
published: false
tags: java, energy, architecture, kiponos
description: "Live CORS max-age via Kiponos Python SDK — energy posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-energy-cors-max-age-0596.md
main_image: https://files.catbox.moe/in7zq9.jpg
---
...
Domain: grid dispatch, demand response. This essay maps hub key `corsMaxAgeSec` to **CORS max-age** so the lesson stays concrete for energy operators.
...
```python
policy = kiponos.path("examples", "ops-energy-cors-max-age")
corsMaxAgeSec = min(int(policy.get("corsMaxAgeSec", 600)), HARD_MAX)
worker_pool.resize(corsMaxAgeSec)  # or admission semaphore
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-energy-cors-max-age/corsMaxAgeSec`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: CORS max-age for energy — numbers humans already change verbally in war rooms.
