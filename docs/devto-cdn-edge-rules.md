...
Redeploying to change `cacheTtlSec` mid-incident is how teams invent 3am folklore around CDN edge rules.

## The problem: ceremony between judgment and effect

When **edge cache TTL** is frozen in a deploy unit, every incident becomes a process argument. Hotfixes still mean CI + roll. Feature flags often mean a second control plane with its own delay.
...
```java
Folder policy = kiponos.path("examples", "cdn-edge-rules");
int cacheTtlSec = policy.getInt("cacheTtlSec"); // local get — no hub RTT
if (cacheTtlSec <= 0) {
    return failClosed();
}
return apply(cacheTtlSec);
```

Ops (or automation) sets `cacheTtlSec` in the hub. The **next** evaluation uses it. Same binary. Same tests for structure.
...
- Canonical source: [this article on GitHub](https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-cdn-edge-rules.md)
...
Feature flags are often product gates. This essay is about **ops posture** on CDN edge rules: **edge cache TTL** — numbers war rooms already shout.
...
Treat `cacheTtlSec` as a slice of error budget. Raise when the world is healthy; lower when dependencies are sick. Write the number that survives a bad day.
...
**Edge cache TTL** that requires a deploy is optimistic documentation.
