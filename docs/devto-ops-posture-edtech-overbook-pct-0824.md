---
title: "Edtech: Turn overbooking percent Into Incident Posture"
published: false
...
Domain: class size, exam windows. This essay maps hub key `overbookPercent` to **overbooking percent** so the lesson stays concrete for edtech operators.
...
```python
policy = kiponos.path("examples", "ops-edtech-overbook-pct")
...
1. Name the hub path so humans find it under pressure (`examples/ops-edtech-overbook-pct/overbookPercent`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: overbooking percent for edtech — numbers humans already change verbally in war rooms.
