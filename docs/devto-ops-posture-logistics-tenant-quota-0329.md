---
title: "Logistics: Turn per-tenant quota Into Incident Posture"
published: false
...
main_image: ./devto-cover-ops-posture-logistics-tenant-quota-0329.jpg
---
...
SEV-2 started as a metric spike and became a ceremony debate: who can change quotaPerMin without a PR.

Domain: routing, SLA windows, fleet. This essay maps hub key `quotaPerMin` to **per-tenant quota** so the lesson stays concrete for logistics operators.
...
```java
Folder policy = kiponos.path("examples", "ops-logistics-tenant-quota");
...
1. Name the hub path so humans find it under pressure (`examples/ops-logistics-tenant-quota/quotaPerMin`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: per-tenant quota for logistics — numbers humans already change verbally in war rooms.
