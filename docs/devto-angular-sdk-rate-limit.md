...
description: "Live API RPS ceiling on the Kiponos hub from a Angular server peer — createFromEnv, no SPA secrets, Java parity. Install @kiponos/angular."
...
I have sat through a room that looked like this: API gateway YAML that only moved with the next Angular service jar. Someone said “we just need to flip it.” Someone else opened a PR. CI ran. The window closed.
...
- Companion Java tree: `examples/java/angular-sdk-rate-limit` (same hub moral)
...
Identity is **where the process runs**. Angular’s job is to be a first-class peer on the hub, not a bundler that smuggles secrets. Ship the peer once. Leave the bundle alone when the only thing that changed is **API RPS ceiling**.
