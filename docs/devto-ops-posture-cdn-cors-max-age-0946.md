---
title: "We Stopped Shipping Cors Max-Age as a Constant (Cdn)"
published: false
...
main_image: https://files.catbox.moe/d5g30v.jpg
---
...
Domain: edge rules, cache TTL, purge. This essay maps hub key `corsMaxAgeSec` to **CORS max-age** so the lesson stays concrete for cdn operators.
...
```python
policy = kiponos.path("examples", "ops-cdn-cors-max-age")
...
1. Name the hub path so humans find it under pressure (`examples/ops-cdn-cors-max-age/corsMaxAgeSec`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: CORS max-age for cdn — numbers humans already change verbally in war rooms.
