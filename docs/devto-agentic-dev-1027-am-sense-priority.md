---
main_image: https://iili.io/ndedBv1.jpg
...
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-dev-1027-am-sense-priority.md
---

I have sat next to the admin-dashboard wall at 21:03 while Cursor was *this close* to doing the wrong thing.
...
| Restart Cursor — it is cheap | Cheap until 21:03 ate the operator's last ten minutes of diagnosis |
...
```text
examples/agentic-dev-1027-am-sense-priority/priority = live
```

Runnable proof: [`examples/java/agentic-dev-1027-am-sense-priority`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-dev-1027-am-sense-priority)
...
```yaml
examples/
  agentic-dev-1027-am-sense-priority/
...
Folder gate = kip.getRootFolder()
        ...
        .folderOrCreate("agentic-dev-1027-am-sense-priority");
...
try:
    posture = k.get("examples/agentic-dev-1027-am-sense-priority/priority", "live")
    if str(posture) == "live":
...
At 21:03 someone said, out loud: **the tile is not a rumor. it is the same tree**. That sentence is the whole product. If it cannot land in the running Cursor process in seconds, you do not have posture. You have a wiki.

## Pair `priority` with a sister dial

`priority` rarely moves alone on the admin-dashboard wall. Pair it with a timeout, a mute, or a pause so you do not fix Sense priority mid-turn by inventing a second incident.
...
3. `cd examples/java/agentic-dev-1027-am-sense-priority && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-dev-1027-am-sense-priority/priority=...`
...
How to try: `examples/java/agentic-dev-1027-am-sense-priority` and `./gradlew test`.
