---
title: "Live Batch Chunk Size for Dataops — Mouth to Process in Seconds"
published: false
...
Domain: batch, lag, checkpoint. This essay maps hub key `chunkSize` to **batch chunk size** so the lesson stays concrete for dataops operators.
...
```java
Folder policy = kiponos.path("examples", "ops-dataops-batch-chunk");
...
1. Name the hub path so humans find it under pressure (`examples/ops-dataops-batch-chunk/chunkSize`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: batch chunk size for dataops — numbers humans already change verbally in war rooms.
