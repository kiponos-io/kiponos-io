---
title: "We Stopped Shipping Slo Warn Burn Bps as a Constant (Retail)"
published: false
...
main_image: ./devto-cover-ops-posture-retail-slo-warn-bps-0031.jpg
---
...
Domain: checkout, merchandising, inventory. This essay maps hub key `errorBudgetWarnBps` to **SLO warn burn bps** so the lesson stays concrete for retail operators.
...
```java
Folder policy = kiponos.path("examples", "ops-retail-slo-warn-bps");
...
1. Name the hub path so humans find it under pressure (`examples/ops-retail-slo-warn-bps/errorBudgetWarnBps`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: SLO warn burn bps for retail — numbers humans already change verbally in war rooms.
