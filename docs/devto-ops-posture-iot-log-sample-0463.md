---
title: "The Iot War Room Already Knew logSamplePerThousand — The Jar Did Not"
published: false
...
main_image: https://iili.io/CLVVAAu.jpg
---
...
Domain: device firmware gates, telemetry sample. This essay maps hub key `logSamplePerThousand` to **log sample per thousand** so the lesson stays concrete for iot operators.
...
```java
var policy = kiponos.path("examples", "ops-iot-log-sample");
...
1. Name the hub path so humans find it under pressure (`examples/ops-iot-log-sample/logSamplePerThousand`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: log sample per thousand for iot — numbers humans already change verbally in war rooms.
