---
title: "The Platform War Room Already Knew logSamplePerThousand — The Jar Did Not"
published: false
tags: java, platform, architecture, kiponos
description: "Live log sample per thousand via Kiponos Java SDK — platform posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-platform-log-sample-1113.md
main_image: ./devto-cover-ops-posture-platform-log-sample-1113.jpg
---
...
Domain: shared libraries, defaults. This essay maps hub key `logSamplePerThousand` to **log sample per thousand** so the lesson stays concrete for platform operators.
...
```java
Folder policy = kiponos.path("examples", "ops-platform-log-sample");
int logSamplePerThousand = Math.min(policy.getInt("logSamplePerThousand"), HARD_MAX);
limiter.setLimit(logSamplePerThousand); // next evaluation sees it
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-platform-log-sample/logSamplePerThousand`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: log sample per thousand for platform — numbers humans already change verbally in war rooms.
