---
title: "We Stopped Shipping Outbound Call Timeout as a Constant (Insurance)"
published: false
tags: java, insurance, architecture, kiponos
description: "Live outbound call timeout via Kiponos Java SDK — insurance posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-insurance-timeout-ms-0601.md
main_image: ./devto-cover-ops-posture-insurance-timeout-ms-0601.jpg
---
...
Domain: underwriting, claims velocity. This essay maps hub key `timeoutMs` to **outbound call timeout** so the lesson stays concrete for insurance operators.
...
```java
var policy = kiponos.path("examples", "ops-insurance-timeout-ms");
String mode = policy.getString("degradeMode", "full");
int timeoutMs = policy.getInt("timeoutMs");
return router.decide(mode, timeoutMs);
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-insurance-timeout-ms/timeoutMs`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: outbound call timeout for insurance — numbers humans already change verbally in war rooms.

Kiponos makes that verbal decision **executable** without a second control plane tax on every request.

## Failure budget vs this dial

Treat `timeoutMs` as a slice of **error budget**, not a comfort blanket. Raise it when the dependency is healthy; lower it when the dependency is already sick. Write the number that survives a bad day, not the number that flatters a sunny demo.
