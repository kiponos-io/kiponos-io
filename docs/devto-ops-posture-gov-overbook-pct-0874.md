---
title: "Gov: Turn overbooking percent Into Incident Posture"
published: false
...
main_image: ./devto-cover-ops-posture-gov-overbook-pct-0874.jpg
---
...
Domain: form windows, batch cutoffs. This essay maps hub key `overbookPercent` to **overbooking percent** so the lesson stays concrete for gov operators.
...
```python
policy = kiponos.path("examples", "ops-gov-overbook-pct")
overbookPercent = min(int(policy.get("overbookPercent", 8)), HARD_MAX)
worker_pool.resize(overbookPercent)  # or admission semaphore
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-gov-overbook-pct/overbookPercent`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: overbooking percent for gov — numbers humans already change verbally in war rooms.
