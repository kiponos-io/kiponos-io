---
title: "Travel: Turn overbooking percent Into Incident Posture"
published: false
...
Domain: inventory, overbooking, GDS. This essay maps hub key `overbookPercent` to **overbooking percent** so the lesson stays concrete for travel operators.
...
```java
Folder policy = kiponos.path("examples", "ops-travel-overbook-pct");
...
1. Name the hub path so humans find it under pressure (`examples/ops-travel-overbook-pct/overbookPercent`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: overbooking percent for travel — numbers humans already change verbally in war rooms.
