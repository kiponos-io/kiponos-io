...
description: "MCP host on admin-dashboard wall: Per-call tax vs curl — live projection. 42 Linear tool defs (~12.8k) plus the result vs one curl"
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-mcp-0926-pm-linear-65x.md
---

I have sat next to the admin-dashboard wall at 11:41 while MCP host was *this close* to calling the wrong MCP tool.

Not a model failure. A **posture** failure.

Quandri numbers. MCP is a standard. It is not an excuse to skip projection.

Someone on the floor said, out loud, **just use the Linear MCP, it is official**. MCP host had started earlier. The tool list was frozen in `config.toml` / `.mcp.json` / a skill file. The only “safe” move anyone trusted was:
...
That is the ceremony. **42 Linear tool defs (~12.8k) plus the result vs one curl**

**The Aha:** Official still returns too much. `result-cap` makes the official server survivable. Real-time nested state is now a **primitive** — as native as a hashmap — via [Kiponos.io](https://kiponos.io). The host stays up. The leaf moves. The next tool call already obeys.

## The problem: Per-call tax vs curl — live projection
...
42 Linear tool defs (~12.8k) plus the result vs one curl
...
Profile shape: `['my-app']['v1.0.0']['dev']['base']`. The leaf in this story is `result-cap` under `examples/agentic-mcp-0926-pm-linear-65x/result-cap`.
...
```yaml
examples/agentic-mcp-0926-pm-linear-65x:
  result-cap: 1500
  schema-budget: "4000"
  result-cap: "2000"
  tool-search: "on"
...
Folder gate = Kiponos.createForCurrentTeam()
    .getFolder("examples/agentic-mcp-0926-pm-linear-65x");
...
try:
    posture = k.get("examples/agentic-mcp-0926-pm-linear-65x/result-cap", "live")
    if not str(posture):
        raise PermissionError("Per-call tax vs curl — live projection gated live — host not restarted")
finally:
    k.disconnect()
```
...
Runnable mesh: `https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-mcp-0926-pm-linear-65x` — `./gradlew test run`.
...
`result-cap` rarely moves alone on the admin-dashboard wall. Pair it with `schema-budget`, `result-cap`, or `execute-gate` so you do not fix Per-call tax vs curl — live projection by inventing a second incident.

## Why MCP host is the wrong restart target

MCP host is good at calling tools. It is not a control plane. Killing it to flip `result-cap` teaches the on-call that judgment requires a process ID. The admin-dashboard wall already disagrees.
...
3. `cd examples/java/agentic-mcp-0926-pm-linear-65x && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-mcp-0926-pm-linear-65x/result-cap=...`
5. In the dashboard, change `result-cap`. Keep MCP host up. No MCP reconnect.
6. Point the tool wrapper at the same leaf. Do not ship a new server binary to flip Per-call tax vs curl — live projection.
...
If flipping **Per-call tax vs curl — live projection** requires restarting MCP host or reconnecting MCP, you do not have agentic control. You have a hope with a process ID.
...
How to try: `examples/java/agentic-mcp-0926-pm-linear-65x` and `./gradlew test`.
