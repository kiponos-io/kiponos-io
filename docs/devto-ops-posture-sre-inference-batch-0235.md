---
title: "Inference Batch Size Should Not Wait for a Redeploy — Sre Live Posture"
published: false
...
Domain: canary, drain, error budget. This essay maps hub key `batchSize` to **inference batch size** so the lesson stays concrete for sre operators.
...
```java
Folder policy = kiponos.path("examples", "ops-sre-inference-batch");
...
1. Name the hub path so humans find it under pressure (`examples/ops-sre-inference-batch/batchSize`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: inference batch size for sre — numbers humans already change verbally in war rooms.
