---
title: "We Stopped Shipping Pagination Page Size as a Constant (Cdn)"
published: false
...
Domain: edge rules, cache TTL, purge. This essay maps hub key `pageSize` to **pagination page size** so the lesson stays concrete for cdn operators.
...
```java
Folder policy = kiponos.path("examples", "ops-cdn-page-size");
...
1. Name the hub path so humans find it under pressure (`examples/ops-cdn-page-size/pageSize`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: pagination page size for cdn — numbers humans already change verbally in war rooms.
