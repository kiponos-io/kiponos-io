---
title: "The Fintech War Room Already Knew logSamplePerThousand — The Jar Did Not"
published: false
...
Domain: payments, fraud, ledger. This essay maps hub key `logSamplePerThousand` to **log sample per thousand** so the lesson stays concrete for fintech operators.
...
```java
Folder policy = kiponos.path("examples", "ops-fintech-log-sample");
...
1. Name the hub path so humans find it under pressure (`examples/ops-fintech-log-sample/logSamplePerThousand`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: log sample per thousand for fintech — numbers humans already change verbally in war rooms.
