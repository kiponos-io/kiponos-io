---
main_image: https://iili.io/CyUdV8g.jpg
...
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-dev-0929-pm-admin-wall.md
---

I have sat next to the admin-dashboard wall at 22:19 while Grok Build was *this close* to doing the wrong thing.
...
| Restart Grok Build — it is cheap | Cheap until 22:19 ate the operator's last ten minutes of diagnosis |
...
```text
examples/agentic-dev-0929-pm-admin-wall/wall-focus = live
```

Runnable proof: [`examples/java/agentic-dev-0929-pm-admin-wall`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-dev-0929-pm-admin-wall)
...
```yaml
examples/
  agentic-dev-0929-pm-admin-wall/
...
Folder gate = kip.getRootFolder()
        ...
        .folderOrCreate("agentic-dev-0929-pm-admin-wall");
...
try:
    posture = k.get("examples/agentic-dev-0929-pm-admin-wall/wall-focus", "live")
    if str(posture) == "live":
...
At 22:19 someone said, out loud: **the tile is not a rumor. it is the same tree**. That sentence is the whole product. If it cannot land in the running Grok Build process in seconds, you do not have posture. You have a wiki.
...
3. `cd examples/java/agentic-dev-0929-pm-admin-wall && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-dev-0929-pm-admin-wall/wall-focus=...`
...
How to try: `examples/java/agentic-dev-0929-pm-admin-wall` and `./gradlew test`.
