...
description: "Cursor on shopping-admin wall: Live retry cap when tools poison the window. Fail → retry → more schema+result tokens → context rot → fail"
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-mcp-0917-am-doom-loop.md
---

I have sat next to the shopping-admin wall at 00:40 while Cursor was *this close* to calling the wrong MCP tool.

Not a model failure. A **posture** failure.

Developers named it the MCP doom loop. Cost and quality both get worse the more you 'let it try'.

Someone on the floor said, out loud, **one more retry, it will find the right tool**. Cursor had started earlier. The tool list was frozen in `config.toml` / `.mcp.json` / a skill file. The only “safe” move anyone trusted was:
...
That is the ceremony. **Fail → retry → more schema+result tokens → context rot → fail**

**The Aha:** `retry-max=1` live. Stop feeding the fire. Do not restart to clear the loop — clamp it. Real-time nested state is now a **primitive** — as native as a hashmap — via [Kiponos.io](https://kiponos.io). The host stays up. The leaf moves. The next tool call already obeys.

## The problem: Live retry cap when tools poison the window
...
Fail → retry → more schema+result tokens → context rot → fail
...
Profile shape: `['my-app']['v1.0.0']['dev']['base']`. The leaf in this story is `retry-max` under `examples/agentic-mcp-0917-am-doom-loop/retry-max`.
...
```yaml
examples/agentic-mcp-0917-am-doom-loop:
  retry-max: 2
  schema-budget: "4000"
  result-cap: "2000"
  tool-search: "on"
...
Eight-plus keys when you pair `retry-max` with sister dials (budget, mute, pause). They move together on the shopping-admin wall.
...
Folder gate = Kiponos.createForCurrentTeam()
    .getFolder("examples/agentic-mcp-0917-am-doom-loop");
String roster = String.valueOf(gate.get("retry-max"));
...
try:
    posture = k.get("examples/agentic-mcp-0917-am-doom-loop/retry-max", "live")
    if not str(posture):
        raise PermissionError("Live retry cap when tools poison the window gated live — host not restarted")
finally:
    k.disconnect()
```
...
Runnable mesh: `https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-mcp-0917-am-doom-loop` — `./gradlew test run`.
...
| User says “no writes” mid-turn | Restart Cursor | Set `retry-max`; next call already denies |
...
- A dashboard edit is a **delta** of `retry-max`, not a config.toml reload.
...
## Pair `retry-max` with a sister dial

`retry-max` rarely moves alone on the shopping-admin wall. Pair it with `schema-budget`, `result-cap`, or `execute-gate` so you do not fix Live retry cap when tools poison the window by inventing a second incident.

## Why Cursor is the wrong restart target

Cursor is good at calling tools. It is not a control plane. Killing it to flip `retry-max` teaches the on-call that judgment requires a process ID. The shopping-admin wall already disagrees.
...
3. `cd examples/java/agentic-mcp-0917-am-doom-loop && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-mcp-0917-am-doom-loop/retry-max=...`
5. In the dashboard, change `retry-max`. Keep Cursor up. No MCP reconnect.
6. Point the tool wrapper at the same leaf. Do not ship a new server binary to flip Live retry cap when tools poison the window.
...
If flipping **Live retry cap when tools poison the window** requires restarting Cursor or reconnecting MCP, you do not have agentic control. You have a hope with a process ID.
...
How to try: `examples/java/agentic-mcp-0917-am-doom-loop` and `./gradlew test`.
