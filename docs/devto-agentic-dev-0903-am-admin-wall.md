---
main_image: https://iili.io/CpKg7lj.jpg
title: "MCP host Finished the Turn Blind — Admin wall tile without SPA tokens on the Senses Path"
published: false
tags: java, python, devops, ai, kiponos
description: "MCP host kept finishing the turn blind on senses wall. Admin wall tile without SPA tokens is a live hub leaf."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-dev-0903-am-admin-wall.md
---

I have sat next to the senses wall at 02:14 while MCP host was *this close* to doing the wrong thing.
...
| Restart MCP host — it is cheap | Cheap until 02:14 ate the turn budget and the open investigation |
...
```text
examples/agentic-dev-0903-am-admin-wall/wall-focus = live
```

Runnable proof: [`examples/java/agentic-dev-0903-am-admin-wall`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-dev-0903-am-admin-wall)
...
```yaml
examples/
  agentic-dev-0903-am-admin-wall/
...
Folder gate = kip.getRootFolder()
        ...
        .folderOrCreate("agentic-dev-0903-am-admin-wall");
...
try:
    posture = k.get("examples/agentic-dev-0903-am-admin-wall/wall-focus", "live")
    if str(posture) == "live":
...
|----------|------------|------------------------|
| Env file + MCP host reboot | Simple at 09:00 | The freeze is at 02:14 |
...
At 02:14 someone said, out loud: **priority is a leaf, not a restart**. That sentence is the whole product. If it cannot land in the running MCP host process in seconds, you do not have posture. You have a wiki.
...
3. `cd examples/java/agentic-dev-0903-am-admin-wall && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-dev-0903-am-admin-wall/wall-focus=...`
5. In the dashboard, change `wall-focus`. Keep the process up. No rebuild.
6. Point your MCP host tool at the same leaf. Do not ship a new server binary to flip Admin wall tile without SPA tokens.
...
How to try: `examples/java/agentic-dev-0903-am-admin-wall` and `./gradlew test`.
