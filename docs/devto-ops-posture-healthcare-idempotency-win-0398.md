---
title: "The Healthcare War Room Already Knew idempotencyWindowSec — The Jar Did Not"
published: false
...
main_image: https://iili.io/Csz22kv.jpg
---
...
Domain: routing, queue depth, triage. This essay maps hub key `idempotencyWindowSec` to **idempotency window** so the lesson stays concrete for healthcare operators.
...
```yaml
examples/
  ops-healthcare-idempotency-win/
...
```java
Folder policy = kiponos.path("examples", "ops-healthcare-idempotency-win");
...
1. Name the hub path so humans find it under pressure (`examples/ops-healthcare-idempotency-win/idempotencyWindowSec`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: idempotency window for healthcare — numbers humans already change verbally in war rooms.
