---
title: "We Stopped Shipping Outbox Poll Interval as a Constant (Insurance)"
published: false
tags: java, insurance, architecture, kiponos
description: "Live outbox poll interval via Kiponos Java SDK — insurance posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-insurance-outbox-poll-ms-0641.md
main_image: ./devto-cover-ops-posture-insurance-outbox-poll-ms-0641.jpg
---
...
Domain: underwriting, claims velocity. This essay maps hub key `outboxPollMs` to **outbox poll interval** so the lesson stays concrete for insurance operators.
...
```java
Folder policy = kiponos.path("examples", "ops-insurance-outbox-poll-ms");
int outboxPollMs = policy.getInt("outboxPollMs");
if (outboxPollMs <= 0) {
    return failClosed(); // money paths
}
return apply(outboxPollMs);
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-insurance-outbox-poll-ms/outboxPollMs`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: outbox poll interval for insurance — numbers humans already change verbally in war rooms.
...
outbox poll interval that requires a deploy is optimistic documentation.
