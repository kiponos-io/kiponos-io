---
title: "Mlops: Turn per-tenant quota Into Incident Posture"
published: false
tags: java, machinelearning, devops, kiponos
description: "Live per-tenant quota via Kiponos Java SDK — mlops posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-mlops-tenant-quota-0979.md
main_image: ./devto-cover-ops-posture-mlops-tenant-quota-0979.jpg
---
...
Domain: batch size, score thresholds. This essay maps hub key `quotaPerMin` to **per-tenant quota** so the lesson stays concrete for mlops operators.
...
```java
Folder policy = kiponos.path("examples", "ops-mlops-tenant-quota");
...
1. Name the hub path so humans find it under pressure (`examples/ops-mlops-tenant-quota/quotaPerMin`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: per-tenant quota for mlops — numbers humans already change verbally in war rooms.
