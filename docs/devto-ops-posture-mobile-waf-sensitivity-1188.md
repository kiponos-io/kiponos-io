---
title: "The Mobile War Room Already Knew wafSensitivity — The Jar Did Not"
published: false
...
main_image: https://iili.io/nu1CnGn.jpg
---
...
Domain: API timeouts, payload caps. This essay maps hub key `wafSensitivity` to **WAF sensitivity 1-5** so the lesson stays concrete for mobile operators.
...
```java
Folder policy = kiponos.path("examples", "ops-mobile-waf-sensitivity");
...
1. Name the hub path so humans find it under pressure (`examples/ops-mobile-waf-sensitivity/wafSensitivity`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: WAF sensitivity 1-5 for mobile — numbers humans already change verbally in war rooms.
