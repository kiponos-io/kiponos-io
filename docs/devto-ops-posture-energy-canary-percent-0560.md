---
title: "Canary Traffic Share Should Not Wait for a Redeploy — Energy Live Posture"
published: false
...
Domain: grid dispatch, demand response. This essay maps hub key `canaryPercent` to **canary traffic share** so the lesson stays concrete for energy operators.
...
```python
policy = kiponos.path("examples", "ops-energy-canary-percent")
...
1. Name the hub path so humans find it under pressure (`examples/ops-energy-canary-percent/canaryPercent`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: canary traffic share for energy — numbers humans already change verbally in war rooms.
