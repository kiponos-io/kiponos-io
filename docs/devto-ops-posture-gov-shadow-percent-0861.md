---
title: "We Stopped Shipping Shadow Traffic Share as a Constant (Gov)"
published: false
tags: java, architecture, devops, kiponos
description: "Live shadow traffic share via Kiponos Python SDK — gov posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-gov-shadow-percent-0861.md
main_image: ./devto-cover-ops-posture-gov-shadow-percent-0861.jpg
---
...
Domain: form windows, batch cutoffs. This essay maps hub key `shadowPercent` to **shadow traffic share** so the lesson stays concrete for gov operators.
...
```python
policy = kiponos.path("examples", "ops-gov-shadow-percent")
shadowPercent = int(policy.get("shadowPercent", 2))
if not limiter.try_acquire(shadowPercent):
    return too_many()
return handle(req)
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-gov-shadow-percent/shadowPercent`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: shadow traffic share for gov — numbers humans already change verbally in war rooms.
