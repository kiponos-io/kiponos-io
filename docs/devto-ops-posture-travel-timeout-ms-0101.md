---
title: "We Stopped Shipping Outbound Call Timeout as a Constant (Travel)"
published: false
tags: java, travel, architecture, kiponos
description: "Live outbound call timeout via Kiponos Python SDK — travel posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-travel-timeout-ms-0101.md
main_image: ./devto-cover-ops-posture-travel-timeout-ms-0101.jpg
---
...
Domain: inventory, overbooking, GDS. This essay maps hub key `timeoutMs` to **outbound call timeout** so the lesson stays concrete for travel operators.
...
```python
policy = kiponos.path("examples", "ops-travel-timeout-ms")
timeoutMs = int(policy.get("timeoutMs", 3000))
if not limiter.try_acquire(timeoutMs):
    return too_many()
return handle(req)
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-travel-timeout-ms/timeoutMs`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: outbound call timeout for travel — numbers humans already change verbally in war rooms.
