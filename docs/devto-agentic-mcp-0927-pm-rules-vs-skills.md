...
description: "Grok Build on mirror-phone wall: Docs compete. The hub decides what is on.. 2026: skills vs rules vs retrieved context. Teams duplicate policy in all three."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-mcp-0927-pm-rules-vs-skills.md
---

I have sat next to the mirror-phone wall at 21:03 while Grok Build was *this close* to calling the wrong MCP tool.

Not a model failure. A **posture** failure.

What belongs where is a real essay. None of them update a running Grok Build without a reread/restart.

Someone on the floor said, out loud, **put it in AGENTS.md, the agent will see it**. Grok Build had started earlier. The tool list was frozen in `config.toml` / `.mcp.json` / a skill file. The only “safe” move anyone trusted was:
...
That is the ceremony. **2026: skills vs rules vs retrieved context. Teams duplicate policy in all three.**

**The Aha:** Markdown is for humans and the next boot. `enabled-set` is for this tool call. Real-time nested state is now a **primitive** — as native as a hashmap — via [Kiponos.io](https://kiponos.io). The host stays up. The leaf moves. The next tool call already obeys.

## The problem: Docs compete. The hub decides what is on.
...
2026: skills vs rules vs retrieved context. Teams duplicate policy in all three.
...
Profile shape: `['my-app']['v1.0.0']['dev']['base']`. The leaf in this story is `enabled-set` under `examples/agentic-mcp-0927-pm-rules-vs-skills/enabled-set`.
...
```yaml
examples/agentic-mcp-0927-pm-rules-vs-skills:
...
Folder gate = Kiponos.createForCurrentTeam()
    .getFolder("examples/agentic-mcp-0927-pm-rules-vs-skills");
...
try:
    posture = k.get("examples/agentic-mcp-0927-pm-rules-vs-skills/enabled-set", "live")
    if not str(posture):
        raise PermissionError("Docs compete. The hub decides what is on. gated live — host not restarted")
finally:
    k.disconnect()
```
...
Runnable mesh: `https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-mcp-0927-pm-rules-vs-skills` — `./gradlew test run`.
...
`enabled-set` rarely moves alone on the mirror-phone wall. Pair it with `schema-budget`, `result-cap`, or `execute-gate` so you do not fix Docs compete. The hub decides what is on. by inventing a second incident.

## Why Grok Build is the wrong restart target

Grok Build is good at calling tools. It is not a control plane. Killing it to flip `enabled-set` teaches the on-call that judgment requires a process ID. The mirror-phone wall already disagrees.
...
3. `cd examples/java/agentic-mcp-0927-pm-rules-vs-skills && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-mcp-0927-pm-rules-vs-skills/enabled-set=...`
5. In the dashboard, change `enabled-set`. Keep Grok Build up. No MCP reconnect.
6. Point the tool wrapper at the same leaf. Do not ship a new server binary to flip Docs compete. The hub decides what is on..
...
If flipping **Docs compete. The hub decides what is on.** requires restarting Grok Build or reconnecting MCP, you do not have agentic control. You have a hope with a process ID.
...
How to try: `examples/java/agentic-mcp-0927-pm-rules-vs-skills` and `./gradlew test`.
