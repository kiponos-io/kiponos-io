---
title: "Dataops: Turn cache TTL seconds Into Incident Posture"
published: false
...
Domain: batch, lag, checkpoint. This essay maps hub key `ttlSec` to **cache TTL seconds** so the lesson stays concrete for dataops operators.
...
```java
var policy = kiponos.path("examples", "ops-dataops-cache-ttl-sec");
...
1. Name the hub path so humans find it under pressure (`examples/ops-dataops-cache-ttl-sec/ttlSec`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: cache TTL seconds for dataops — numbers humans already change verbally in war rooms.
