---
main_image: https://iili.io/CyJgL5G.jpg
title: "Cursor Finished the Turn Blind — Admin wall tile without SPA tokens on the Senses Path"
published: false
tags: java, python, devops, ai, kiponos
description: "Cursor kept finishing the turn blind on senses wall. Admin wall tile without SPA tokens is a live hub leaf."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-dev-0913-am-admin-wall.md
---
...
The host had started with `wall-focus` frozen in env / argv / a skill file. Someone on the floor said, out loud, **priority is a leaf, not a restart**. Cursor was still holding the old process. The only “safe” move anyone trusted was:
...
**The Aha:** Admin wall tile without SPA tokens is not a binary you reboot. It is a **function that should read live posture** from [Kiponos.io](https://kiponos.io) on every call. The host stays up. The leaf moves.
...
| Hard-coded `if` on `wall-focus` | A release | The incident clock |
...
| Paste the new wall-focus into chat | Two agents, two pastes, two lies |
...
```text
examples/agentic-dev-0913-am-admin-wall/wall-focus = live
```

Runnable proof: [`examples/java/agentic-dev-0913-am-admin-wall`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-dev-0913-am-admin-wall)
...
Folder gate = kip.getRootFolder()
        ...
        .folderOrCreate("agentic-dev-0913-am-admin-wall");
...
try:
    posture = k.get("examples/agentic-dev-0913-am-admin-wall/wall-focus", "live")
    if str(posture) == "live":
...
|-------|-----------------|--------------|
| A mid-turn interrupt the host treats as noise | Restart Cursor; lose the turn budget and the open investigation | Set `wall-focus` live; next Cursor tool call already obeys |
...
| Incident over, resume | Another Cursor restart | Set `wall-focus` back; session continues |
...
- A dashboard edit is a **delta** of `wall-focus`, not a config-file reload.
...
## Pair `wall-focus` with a sister dial

`wall-focus` rarely moves alone on the senses wall. Pair it with a timeout, a mute, or a pause so you do not fix Admin wall tile without SPA tokens by inventing a second incident.

## Rehearsal beats slides

In staging: set a painful `wall-focus`, prove Cursor recovers without a host kill, prove clamps reject nonsense, prove last-known-good when the hub is firewalled. That drill ends half the architecture arguments about Admin wall tile without SPA tokens.

## Why Cursor is the wrong restart target

Cursor is good at calling tools. It is not a control plane. Killing it to flip `wall-focus` teaches the on-call that judgment requires a process ID. The senses wall already disagrees.
...
3. `cd examples/java/agentic-dev-0913-am-admin-wall && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-dev-0913-am-admin-wall/wall-focus=...`
5. In the dashboard, change `wall-focus`. Keep the process up. No rebuild.
6. Point your Cursor tool at the same leaf. Do not ship a new server binary to flip Admin wall tile without SPA tokens.
...
If flipping **Admin wall tile without SPA tokens** requires restarting Cursor, you do not have a gate. You have a hope with a process ID.
...
How to try: `examples/java/agentic-dev-0913-am-admin-wall` and `./gradlew test`.
