---
title: "We Stopped Shipping Shadow Traffic Share as a Constant (Banking)"
published: false
tags: java, banking, security, kiponos
description: "Live shadow traffic share via Kiponos Java SDK — banking posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-banking-shadow-percent-0661.md
main_image: https://litter.catbox.moe/rk1cz0.jpg
---
...
Domain: AML thresholds, transfer velocity. This essay maps hub key `shadowPercent` to **shadow traffic share** so the lesson stays concrete for banking operators.
...
```java
Folder policy = kiponos.path("examples", "ops-banking-shadow-percent");
int shadowPercent = Math.min(policy.getInt("shadowPercent"), HARD_MAX);
limiter.setLimit(shadowPercent); // next evaluation sees it
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-banking-shadow-percent/shadowPercent`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: shadow traffic share for banking — numbers humans already change verbally in war rooms.
