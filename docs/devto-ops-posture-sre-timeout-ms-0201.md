---
title: "We Stopped Shipping Outbound Call Timeout as a Constant (Sre)"
published: false
tags: java, sre, devops, kiponos
description: "Live outbound call timeout via Kiponos Python SDK — sre posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-sre-timeout-ms-0201.md
main_image: ./devto-cover-ops-posture-sre-timeout-ms-0201.jpg
---
...
Domain: canary, drain, error budget. This essay maps hub key `timeoutMs` to **outbound call timeout** so the lesson stays concrete for sre operators.
...
```python
policy = kiponos.path("examples", "ops-sre-timeout-ms")
...
1. Name the hub path so humans find it under pressure (`examples/ops-sre-timeout-ms/timeoutMs`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: outbound call timeout for sre — numbers humans already change verbally in war rooms.
