---
title: "We Stopped Shipping Outbound Call Timeout as a Constant (Energy)"
published: false
...
Domain: grid dispatch, demand response. This essay maps hub key `timeoutMs` to **outbound call timeout** so the lesson stays concrete for energy operators.
...
```python
policy = kiponos.path("examples", "ops-energy-timeout-ms")
...
1. Name the hub path so humans find it under pressure (`examples/ops-energy-timeout-ms/timeoutMs`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: outbound call timeout for energy — numbers humans already change verbally in war rooms.
