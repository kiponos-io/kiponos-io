---
title: "Dataops: Turn search scroll timeout Into Incident Posture"
published: false
...
Domain: batch, lag, checkpoint. This essay maps hub key `scrollTimeoutSec` to **search scroll timeout** so the lesson stays concrete for dataops operators.
...
```java
Folder policy = kiponos.path("examples", "ops-dataops-scroll-timeout");
...
1. Name the hub path so humans find it under pressure (`examples/ops-dataops-scroll-timeout/scrollTimeoutSec`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: search scroll timeout for dataops — numbers humans already change verbally in war rooms.
