---
title: "The Ads War Room Already Knew degradeMode — The Jar Did Not"
published: false
tags: java, advertising, devops, kiponos
description: "Live degradation mode via Kiponos Java SDK — ads posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-ads-degrade-mode-0778.md
main_image: https://files.catbox.moe/l9t4sf.jpg
---
...
Domain: bid caps, frequency, pacing. This essay maps hub key `degradeMode` to **degradation mode** so the lesson stays concrete for ads operators.
...
```java
Folder policy = kiponos.path("examples", "ops-ads-degrade-mode");
int degradeMode = Math.min(policy.getInt("degradeMode"), HARD_MAX);
limiter.setLimit(degradeMode); // next evaluation sees it
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-ads-degrade-mode/degradeMode`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: degradation mode for ads — numbers humans already change verbally in war rooms.
