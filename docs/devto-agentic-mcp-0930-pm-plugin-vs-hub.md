...
description: "MCP host on mirror-phone wall: Plugins/skills marketplaces are distribution. The hub is runtime.. Marketplace install ≠ live enable for this session"
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-mcp-0930-pm-plugin-vs-hub.md
---

I have sat next to the mirror-phone wall at 00:40 while MCP host was *this close* to calling the wrong MCP tool.

Not a model failure. A **posture** failure.

Plugins copy into `~/.grok/skills`. That is disk. Disk is the next boot.

Someone on the floor said, out loud, **the plugin is installed, why is it off?**. MCP host had started earlier. The tool list was frozen in `config.toml` / `.mcp.json` / a skill file. The only “safe” move anyone trusted was:
...
That is the ceremony. **Marketplace install ≠ live enable for this session**

**The Aha:** Installed is git. Enabled is `enabled-set`. Two clocks. Real-time nested state is now a **primitive** — as native as a hashmap — via [Kiponos.io](https://kiponos.io). The host stays up. The leaf moves. The next tool call already obeys.

## The problem: Plugins/skills marketplaces are distribution. The hub is runtime.
...
Marketplace install ≠ live enable for this session
...
Profile shape: `['my-app']['v1.0.0']['dev']['base']`. The leaf in this story is `enabled-set` under `examples/agentic-mcp-0930-pm-plugin-vs-hub/enabled-set`.

## Architecture

```mermaid
flowchart LR
  wall[mirror-phone wall] -->|set enabled-set| hub[Kiponos hub]
...
```yaml
examples/agentic-mcp-0930-pm-plugin-vs-hub:
...
Eight-plus keys when you pair `enabled-set` with sister dials (budget, mute, pause). They move together on the mirror-phone wall.
...
Folder gate = Kiponos.createForCurrentTeam()
    .getFolder("examples/agentic-mcp-0930-pm-plugin-vs-hub");
...
try:
    posture = k.get("examples/agentic-mcp-0930-pm-plugin-vs-hub/enabled-set", "live")
    if not str(posture):
        raise PermissionError("Plugins/skills marketplaces are distribution. The hub is runtime. gated live — host not restarted")
finally:
    k.disconnect()
```
...
Runnable mesh: `https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-mcp-0930-pm-plugin-vs-hub` — `./gradlew test run`.
...
| User says “no writes” mid-turn | Restart MCP host | Set `enabled-set`; next call already denies |
...
`enabled-set` rarely moves alone on the mirror-phone wall. Pair it with `schema-budget`, `result-cap`, or `execute-gate` so you do not fix Plugins/skills marketplaces are distribution. The hub is runtime. by inventing a second incident.

## Why MCP host is the wrong restart target

MCP host is good at calling tools. It is not a control plane. Killing it to flip `enabled-set` teaches the on-call that judgment requires a process ID. The mirror-phone wall already disagrees.
...
3. `cd examples/java/agentic-mcp-0930-pm-plugin-vs-hub && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-mcp-0930-pm-plugin-vs-hub/enabled-set=...`
5. In the dashboard, change `enabled-set`. Keep MCP host up. No MCP reconnect.
6. Point the tool wrapper at the same leaf. Do not ship a new server binary to flip Plugins/skills marketplaces are distribution. The hub is runtime..
...
If flipping **Plugins/skills marketplaces are distribution. The hub is runtime.** requires restarting MCP host or reconnecting MCP, you do not have agentic control. You have a hope with a process ID.
...
How to try: `examples/java/agentic-mcp-0930-pm-plugin-vs-hub` and `./gradlew test`.
