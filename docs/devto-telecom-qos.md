...
description: "Live QoS class weights, bandwidth caps, and congestion policies in Java network control planes. NOC edits Kiponos; edge controllers read locally via WebSocket d"
...
**The Aha:** `qosClass` is not a property-file trophy. It is **incident posture** for telecom bandwidth routing — and posture that waits for a jar is already late.
...
When **QoS class** is frozen in a deploy unit, every incident becomes a process argument. Hotfixes still mean CI + roll. Feature flags often mean a second control plane with its own delay.
...
Domain: telecom bandwidth routing.
...
```java
Folder policy = kiponos.path("examples", "telecom-qos");
int qosClass = policy.getInt("qosClass"); // local get — no hub RTT
if (qosClass <= 0) {
    return failClosed();
}
return apply(qosClass);
```

Ops (or automation) sets `qosClass` in the hub. The **next** evaluation uses it. Same binary. Same tests for structure.
...
- Canonical source: [this article on GitHub](https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-telecom-qos.md)
...
Feature flags are often product gates. This essay is about **ops posture** on telecom bandwidth routing: **QoS class** — numbers war rooms already shout.
...
Treat `qosClass` as a slice of error budget. Raise when the world is healthy; lower when dependencies are sick. Write the number that survives a bad day.
...
**QoS class** that requires a deploy is optimistic documentation.
