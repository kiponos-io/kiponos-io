---
title: "Sre: Turn JWT clock skew leeway Into Incident Posture"
published: false
...
Domain: canary, drain, error budget. This essay maps hub key `leewaySec` to **JWT clock skew leeway** so the lesson stays concrete for sre operators.
...
```python
policy = kiponos.path("examples", "ops-sre-jwt-leeway-sec")
...
1. Name the hub path so humans find it under pressure (`examples/ops-sre-jwt-leeway-sec/leewaySec`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: JWT clock skew leeway for sre — numbers humans already change verbally in war rooms.
