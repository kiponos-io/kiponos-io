---
title: "We Stopped Shipping Outbox Poll Interval as a Constant (Retail)"
published: false
...
main_image: https://iili.io/nRf6nYN.jpg
---
...
Domain: checkout, merchandising, inventory. This essay maps hub key `outboxPollMs` to **outbox poll interval** so the lesson stays concrete for retail operators.
...
```python
policy = kiponos.path("examples", "ops-retail-outbox-poll-ms")
...
1. Name the hub path so humans find it under pressure (`examples/ops-retail-outbox-poll-ms/outboxPollMs`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: outbox poll interval for retail — numbers humans already change verbally in war rooms.
