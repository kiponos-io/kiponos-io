---
title: "We Stopped Shipping Cors Max-Age as a Constant (Saas)"
published: false
...
main_image: https://iili.io/Ctfg9AG.jpg
---
...
Domain: multi-tenant quotas, feature posture. This essay maps hub key `corsMaxAgeSec` to **CORS max-age** so the lesson stays concrete for saas operators.
...
```python
policy = kiponos.path("examples", "ops-saas-cors-max-age")
...
1. Name the hub path so humans find it under pressure (`examples/ops-saas-cors-max-age/corsMaxAgeSec`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: CORS max-age for saas — numbers humans already change verbally in war rooms.
