---
main_image: https://h.uguu.se/HGlvamPt.jpg
...
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-dev-1014-pm-agent-folder.md
---

I have sat next to the admin-dashboard wall at 00:40 while MCP host was *this close* to doing the wrong thing.
...
```text
examples/agentic-dev-1014-pm-agent-folder/owner-agent = live
```

Runnable proof: [`examples/java/agentic-dev-1014-pm-agent-folder`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-dev-1014-pm-agent-folder)
...
```yaml
examples/
  agentic-dev-1014-pm-agent-folder/
...
Folder gate = kip.getRootFolder()
        ...
        .folderOrCreate("agentic-dev-1014-pm-agent-folder");
...
try:
    posture = k.get("examples/agentic-dev-1014-pm-agent-folder/owner-agent", "live")
    if str(posture) == "live":
...
At 00:40 someone said, out loud: **the tile is not a rumor. it is the same tree**. That sentence is the whole product. If it cannot land in the running MCP host process in seconds, you do not have posture. You have a wiki.
...
3. `cd examples/java/agentic-dev-1014-pm-agent-folder && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-dev-1014-pm-agent-folder/owner-agent=...`
...
How to try: `examples/java/agentic-dev-1014-pm-agent-folder` and `./gradlew test`.
