---
title: "Saga Step Retry Budget Should Not Wait for a Redeploy — Telecom Live Posture"
published: false
...
main_image: https://iili.io/CD71ct2.jpg
---
...
Domain: QoS, rate classes, trunk limits. This essay maps hub key `sagaMaxAttempts` to **saga step retry budget** so the lesson stays concrete for telecom operators.
...
```java
Folder policy = kiponos.path("examples", "ops-telecom-saga-retry");
...
1. Name the hub path so humans find it under pressure (`examples/ops-telecom-saga-retry/sagaMaxAttempts`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: saga step retry budget for telecom — numbers humans already change verbally in war rooms.
