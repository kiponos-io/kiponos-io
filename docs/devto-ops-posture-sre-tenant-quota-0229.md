---
title: "Sre: Turn per-tenant quota Into Incident Posture"
published: false
tags: java, sre, devops, kiponos
description: "Live per-tenant quota via Kiponos Python SDK — sre posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-sre-tenant-quota-0229.md
main_image: ./devto-cover-ops-posture-sre-tenant-quota-0229.jpg
---
...
Domain: canary, drain, error budget. This essay maps hub key `quotaPerMin` to **per-tenant quota** so the lesson stays concrete for sre operators.
...
```python
policy = kiponos.path("examples", "ops-sre-tenant-quota")
...
1. Name the hub path so humans find it under pressure (`examples/ops-sre-tenant-quota/quotaPerMin`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: per-tenant quota for sre — numbers humans already change verbally in war rooms.
