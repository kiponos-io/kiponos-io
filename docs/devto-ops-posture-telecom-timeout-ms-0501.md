---
title: "We Stopped Shipping Outbound Call Timeout as a Constant (Telecom)"
published: false
tags: java, telecom, sre, kiponos
description: "Live outbound call timeout via Kiponos Java SDK — telecom posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-telecom-timeout-ms-0501.md
main_image: ./devto-cover-ops-posture-telecom-timeout-ms-0501.jpg
---
...
Domain: QoS, rate classes, trunk limits. This essay maps hub key `timeoutMs` to **outbound call timeout** so the lesson stays concrete for telecom operators.
...
```java
Folder policy = kiponos.path("examples", "ops-telecom-timeout-ms");
int timeoutMs = Math.min(policy.getInt("timeoutMs"), HARD_MAX);
limiter.setLimit(timeoutMs); // next evaluation sees it
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-telecom-timeout-ms/timeoutMs`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: outbound call timeout for telecom — numbers humans already change verbally in war rooms.
