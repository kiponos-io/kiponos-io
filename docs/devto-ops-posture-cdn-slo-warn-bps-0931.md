---
title: "We Stopped Shipping Slo Warn Burn Bps as a Constant (Cdn)"
published: false
...
main_image: https://files.catbox.moe/c9xowb.jpg
---
...
Domain: edge rules, cache TTL, purge. This essay maps hub key `errorBudgetWarnBps` to **SLO warn burn bps** so the lesson stays concrete for cdn operators.
...
```python
policy = kiponos.path("examples", "ops-cdn-slo-warn-bps")
...
1. Name the hub path so humans find it under pressure (`examples/ops-cdn-slo-warn-bps/errorBudgetWarnBps`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: SLO warn burn bps for cdn — numbers humans already change verbally in war rooms.
