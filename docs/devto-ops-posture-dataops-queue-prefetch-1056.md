---
title: "We Stopped Shipping Consumer Prefetch as a Constant (Dataops)"
published: false
...
main_image: https://iili.io/CgyBg0G.jpg
---
...
Domain: batch, lag, checkpoint. This essay maps hub key `prefetch` to **consumer prefetch** so the lesson stays concrete for dataops operators.
...
```java
Folder policy = kiponos.path("examples", "ops-dataops-queue-prefetch");
...
1. Name the hub path so humans find it under pressure (`examples/ops-dataops-queue-prefetch/prefetch`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: consumer prefetch for dataops — numbers humans already change verbally in war rooms.
