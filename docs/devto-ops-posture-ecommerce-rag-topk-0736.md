---
title: "We Stopped Shipping Rag Top-K as a Constant (Ecommerce)"
published: false
...
main_image: https://files.catbox.moe/z3l611.jpg
---
...
Domain: cart, promo weights, stock. This essay maps hub key `topK` to **RAG top-k** so the lesson stays concrete for ecommerce operators.
...
```java
Folder policy = kiponos.path("examples", "ops-ecommerce-rag-topk");
...
1. Name the hub path so humans find it under pressure (`examples/ops-ecommerce-rag-topk/topK`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: RAG top-k for ecommerce — numbers humans already change verbally in war rooms.
