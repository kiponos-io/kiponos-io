...
description: "Cursor on senses wall: First-class MCP in Grok Build is necessary, not sufficient. `grok mcp add` is delightful. The list is still load-time."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-mcp-1003-am-grok-native-mcp.md
---

I have sat next to the senses wall at 21:03 while Cursor was *this close* to calling the wrong MCP tool.

Not a model failure. A **posture** failure.

xAI docs: MCP beside built-in tools, namespaced. The TUI can toggle servers — that is UI, not a multi-host hub.

Someone on the floor said, out loud, **toggle it in /mcps**. Cursor had started earlier. The tool list was frozen in `config.toml` / `.mcp.json` / a skill file. The only “safe” move anyone trusted was:
...
That is the ceremony. **`grok mcp add` is delightful. The list is still load-time.**

**The Aha:** Toggles that do not fan out to the other Grok Build on the other laptop are local myths. Hub leaf fans out. Real-time nested state is now a **primitive** — as native as a hashmap — via [Kiponos.io](https://kiponos.io). The host stays up. The leaf moves. The next tool call already obeys.
...
`grok mcp add` is delightful. The list is still load-time.
...
Profile shape: `['my-app']['v1.0.0']['dev']['base']`. The leaf in this story is `tools-allow` under `examples/agentic-mcp-1003-am-grok-native-mcp/tools-allow`.

## Architecture

```mermaid
flowchart LR
  wall[senses wall] -->|set tools-allow| hub[Kiponos hub]
...
```yaml
examples/agentic-mcp-1003-am-grok-native-mcp:
...
Eight-plus keys when you pair `tools-allow` with sister dials (budget, mute, pause). They move together on the senses wall.
...
Folder gate = Kiponos.createForCurrentTeam()
    .getFolder("examples/agentic-mcp-1003-am-grok-native-mcp");
...
try:
    posture = k.get("examples/agentic-mcp-1003-am-grok-native-mcp/tools-allow", "live")
    if not str(posture):
        raise PermissionError("First-class MCP in Grok Build is necessary, not sufficient gated live — host not restarted")
finally:
    k.disconnect()
```
...
Runnable mesh: `https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-mcp-1003-am-grok-native-mcp` — `./gradlew test run`.
...
`tools-allow` rarely moves alone on the senses wall. Pair it with `schema-budget`, `result-cap`, or `execute-gate` so you do not fix First-class MCP in Grok Build is necessary, not sufficient by inventing a second incident.

## Why Cursor is the wrong restart target

Cursor is good at calling tools. It is not a control plane. Killing it to flip `tools-allow` teaches the on-call that judgment requires a process ID. The senses wall already disagrees.
...
3. `cd examples/java/agentic-mcp-1003-am-grok-native-mcp && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-mcp-1003-am-grok-native-mcp/tools-allow=...`
5. In the dashboard, change `tools-allow`. Keep Cursor up. No MCP reconnect.
6. Point the tool wrapper at the same leaf. Do not ship a new server binary to flip First-class MCP in Grok Build is necessary, not sufficient.
...
If flipping **First-class MCP in Grok Build is necessary, not sufficient** requires restarting Cursor or reconnecting MCP, you do not have agentic control. You have a hope with a process ID.
...
How to try: `examples/java/agentic-mcp-1003-am-grok-native-mcp` and `./gradlew test`.
