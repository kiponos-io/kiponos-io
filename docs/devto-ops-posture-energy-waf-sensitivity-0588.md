---
title: "The Energy War Room Already Knew wafSensitivity — The Jar Did Not"
published: false
tags: java, energy, architecture, kiponos
description: "Live WAF sensitivity 1-5 via Kiponos Java SDK — energy posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-energy-waf-sensitivity-0588.md
main_image: https://files.catbox.moe/ta64xj.jpg
---
...
The partner brownout did not wait for your change-management window. Your WAF sensitivity 1-5 did — until it lived in a hub.

Domain: grid dispatch, demand response. This essay maps hub key `wafSensitivity` to **WAF sensitivity 1-5** so the lesson stays concrete for energy operators.
...
```java
Folder policy = kiponos.path("examples", "ops-energy-waf-sensitivity");
int wafSensitivity = policy.getInt("wafSensitivity");
if (wafSensitivity <= 0) {
...
1. Name the hub path so humans find it under pressure (`examples/ops-energy-waf-sensitivity/wafSensitivity`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: WAF sensitivity 1-5 for energy — numbers humans already change verbally in war rooms.
