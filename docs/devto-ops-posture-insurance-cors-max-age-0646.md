---
title: "We Stopped Shipping Cors Max-Age as a Constant (Insurance)"
published: false
tags: java, insurance, architecture, kiponos
description: "Live CORS max-age via Kiponos Java SDK — insurance posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-insurance-cors-max-age-0646.md
main_image: ./devto-cover-ops-posture-insurance-cors-max-age-0646.jpg
---
...
Domain: underwriting, claims velocity. This essay maps hub key `corsMaxAgeSec` to **CORS max-age** so the lesson stays concrete for insurance operators.
...
```java
Folder policy = kiponos.path("examples", "ops-insurance-cors-max-age");
int corsMaxAgeSec = Math.min(policy.getInt("corsMaxAgeSec"), HARD_MAX);
limiter.setLimit(corsMaxAgeSec); // next evaluation sees it
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-insurance-cors-max-age/corsMaxAgeSec`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: CORS max-age for insurance — numbers humans already change verbally in war rooms.
...
Treat `corsMaxAgeSec` as a slice of **error budget**, not a comfort blanket. Raise it when the dependency is healthy; lower it when the dependency is already sick. Write the number that survives a bad day, not the number that flatters a sunny demo.
