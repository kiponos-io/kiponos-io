---
title: "Mediator as a Super Pattern — Live Topology"
published: false
...
**The Aha:** Who talks to whom is a hub matrix

Chatty services during an incident need a quieter topology — same mediator class, different routes.
...
```yaml
patterns/mediator/topology/
  # live knobs
  edges: cart->pricing,cart->tax
```

Hot path (local memory after connect — WebSocket deltas, no per-request hub RTT):

```java
Folder policy = kiponos.path('patterns', 'mediator', 'topology');
// local gets — never remote on the money path
for (Edge e : edges) mediator.route(e.from, e.to, msg);
```
...
Full tree: [https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/pattern-mediator-live-topology](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/pattern-mediator-live-topology)
...
Mediator reduces coupling. Super Mediator reduces redeploys when routes change.
...
*Runnable: [pattern-mediator-live-topology](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/pattern-mediator-live-topology) · [kiponos.io](https://kiponos.io)*
