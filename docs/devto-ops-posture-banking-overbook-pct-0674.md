---
title: "Banking: Turn overbooking percent Into Incident Posture"
published: false
...
Domain: AML thresholds, transfer velocity. This essay maps hub key `overbookPercent` to **overbooking percent** so the lesson stays concrete for banking operators.
...
```java
Folder policy = kiponos.path("examples", "ops-banking-overbook-pct");
...
1. Name the hub path so humans find it under pressure (`examples/ops-banking-overbook-pct/overbookPercent`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: overbooking percent for banking — numbers humans already change verbally in war rooms.
