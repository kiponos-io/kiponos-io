---
title: "Live Retry Budget for Healthcare — Mouth to Process in Seconds"
published: false
tags: java, healthcare, architecture, kiponos
description: "Live retry budget via Kiponos Java SDK — healthcare posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-healthcare-retry-max-0352.md
main_image: ./devto-cover-ops-posture-healthcare-retry-max-0352.jpg
---
...
Domain: routing, queue depth, triage. This essay maps hub key `maxAttempts` to **retry budget** so the lesson stays concrete for healthcare operators.
...
```java
Folder policy = kiponos.path("examples", "ops-healthcare-retry-max");
int maxAttempts = policy.getInt("maxAttempts");
if (maxAttempts <= 0) {
    return failClosed(); // money paths
}
return apply(maxAttempts);
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-healthcare-retry-max/maxAttempts`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: retry budget for healthcare — numbers humans already change verbally in war rooms.
