---
title: "The Cdn War Room Already Knew backoffMs — The Jar Did Not"
published: false
...
main_image: https://files.catbox.moe/f2b5re.jpg
---

**The Aha:** `backoffMs` is not a property file trophy. It is **incident posture** — and posture that waits for a jar is already late.

SEV-2 started as a metric spike and became a ceremony debate: who can change backoffMs without a PR.

Domain: edge rules, cache TTL, purge. This essay maps hub key `backoffMs` to **retry backoff base** so the lesson stays concrete for cdn operators.
...
```python
policy = kiponos.path("examples", "ops-cdn-backoff-ms")
...
1. Name the hub path so humans find it under pressure (`examples/ops-cdn-backoff-ms/backoffMs`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: retry backoff base for cdn — numbers humans already change verbally in war rooms.
