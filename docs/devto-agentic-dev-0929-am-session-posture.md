...
description: "Cursor kept finishing the turn blind on shopping-admin wall. Session posture shared across hosts is a live hub leaf."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-dev-0929-am-session-posture.md
---

I have sat next to the shopping-admin wall at 21:03 while Cursor was *this close* to doing the wrong thing.

Not a model failure. A **posture** failure.

The host had started with `session-posture` frozen in env / argv / a skill file. Someone on the floor said, out loud, **disable inventory writes — keep search**. Cursor was still holding the old process. The only “safe” move anyone trusted was:
...
The shopping-admin wall already *knew*. Cursor did not, because it had started earlier.
...
| Restart Cursor — it is cheap | Cheap until 21:03 ate cart forensics and the open SKU thread |
...
Kiponos holds a nested tree. Java and Python SDKs keep the latest values **in memory**, patched over WebSocket deltas. The hot path inside a Cursor tool is a **local get** — no HTTP RTT per shopping lookup.

Hub leaf for this essay:

```text
examples/agentic-dev-0929-am-session-posture/session-posture = shared
```

Runnable proof: [`examples/java/agentic-dev-0929-am-session-posture`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-dev-0929-am-session-posture)
...
```yaml
examples/
  agentic-dev-0929-am-session-posture/
...
Folder gate = kip.getRootFolder()
        ...
        .folderOrCreate("agentic-dev-0929-am-session-posture");
...
try:
    posture = k.get("examples/agentic-dev-0929-am-session-posture/session-posture", "shared")
...
|-------|-----------------|--------------|
| Flash-freeze on sku writes | Restart Cursor; lose cart forensics and the open SKU thread | Set `session-posture` live; next Cursor tool call already obeys |
| Peer host still on old session-posture | Paste the value into the other chat | One hub leaf; both processes `get()` locally |
| shopping-admin wall shows the new posture | Cursor started earlier so it writes anyway | Dashboard and tool share the same memory tree |
| Incident over, resume | Another Cursor restart | Set `session-posture` back; session continues |
...
At 21:03 someone said, out loud: **disable inventory writes — keep search**. That sentence is the whole product. If it cannot land in the running Cursor process in seconds, you do not have posture. You have a wiki.
...
In staging: set a painful `session-posture`, prove Cursor recovers without a host kill, prove clamps reject nonsense, prove last-known-good when the hub is firewalled. That drill ends half the architecture arguments about Session posture shared across hosts.
...
3. `cd examples/java/agentic-dev-0929-am-session-posture && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-dev-0929-am-session-posture/session-posture=...`
5. In the dashboard, change `session-posture`. Keep the process up. No rebuild.
6. Point your Cursor tool at the same leaf. Do not ship a new server binary to flip Session posture shared across hosts.
...
If flipping **Session posture shared across hosts** requires restarting Cursor, you do not have a gate. You have a hope with a process ID.
...
How to try: `examples/java/agentic-dev-0929-am-session-posture` and `./gradlew test`.
