---
title: "We Stopped Shipping Shadow Traffic Share as a Constant (Edtech)"
published: false
...
Domain: class size, exam windows. This essay maps hub key `shadowPercent` to **shadow traffic share** so the lesson stays concrete for edtech operators.
...
```java
var policy = kiponos.path("examples", "ops-edtech-shadow-percent");
...
1. Name the hub path so humans find it under pressure (`examples/ops-edtech-shadow-percent/shadowPercent`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: shadow traffic share for edtech — numbers humans already change verbally in war rooms.
