---
title: "Canary Traffic Share Should Not Wait for a Redeploy — Marketplace Live Posture"
published: false
tags: java, architecture, devops, kiponos
description: "Live canary traffic share via Kiponos Java SDK — marketplace posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-marketplace-canary-percent-1210.md
main_image: ./devto-cover-ops-posture-marketplace-canary-percent-1210.jpg
---
...
Domain: seller tiers, fee weights. This essay maps hub key `canaryPercent` to **canary traffic share** so the lesson stays concrete for marketplace operators.
...
```yaml
examples/
  ops-marketplace-canary-percent/
...
```java
Folder policy = kiponos.path("examples", "ops-marketplace-canary-percent");
int canaryPercent = Math.min(policy.getInt("canaryPercent"), HARD_MAX);
limiter.setLimit(canaryPercent); // next evaluation sees it
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-marketplace-canary-percent/canaryPercent`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: canary traffic share for marketplace — numbers humans already change verbally in war rooms.

Kiponos makes that verbal decision **executable** without a second control plane tax on every request.

## Pair with a sister dial

`canaryPercent` rarely moves alone. Pair with timeout/retry, canary share, or sampling so you do not fix one symptom by creating another.
