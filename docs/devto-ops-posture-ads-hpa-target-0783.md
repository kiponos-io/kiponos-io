---
title: "The Ads War Room Already Knew hpaTarget — The Jar Did Not"
published: false
...
Domain: bid caps, frequency, pacing. This essay maps hub key `hpaTarget` to **HPA CPU target percent** so the lesson stays concrete for ads operators.
...
```java
Folder policy = kiponos.path("examples", "ops-ads-hpa-target");
...
1. Name the hub path so humans find it under pressure (`examples/ops-ads-hpa-target/hpaTarget`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: HPA CPU target percent for ads — numbers humans already change verbally in war rooms.
