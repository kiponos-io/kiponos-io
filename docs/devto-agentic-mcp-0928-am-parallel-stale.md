...
description: "Grok Build on senses wall: Fan-out without a hub clones stale argv. Grok Build can run parallel children. Each inherits start-time env."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-mcp-0928-am-parallel-stale.md
---

I have sat next to the senses wall at 14:12 while Grok Build was *this close* to calling the wrong MCP tool.

Not a model failure. A **posture** failure.

Orchestrator prompts say 'invoke MCP tools'. They do not say 'read the live leaf'.

Someone on the floor said, out loud, **the swarm will pick it up**. Grok Build had started earlier. The tool list was frozen in `config.toml` / `.mcp.json` / a skill file. The only “safe” move anyone trusted was:
...
That is the ceremony. **Grok Build can run parallel children. Each inherits start-time env.**

**The Aha:** Every child `get()`s. Swarm is not a rumor mill. Real-time nested state is now a **primitive** — as native as a hashmap — via [Kiponos.io](https://kiponos.io). The host stays up. The leaf moves. The next tool call already obeys.

## The problem: Fan-out without a hub clones stale argv
...
Grok Build can run parallel children. Each inherits start-time env.
...
Profile shape: `['my-app']['v1.0.0']['dev']['base']`. The leaf in this story is `inherit-posture` under `examples/agentic-mcp-0928-am-parallel-stale/inherit-posture`.

## Architecture

```mermaid
flowchart LR
  wall[senses wall] -->|set inherit-posture| hub[Kiponos hub]
...
```yaml
examples/agentic-mcp-0928-am-parallel-stale:
  inherit-posture: on
  schema-budget: "4000"
  result-cap: "2000"
  tool-search: "on"
...
Eight-plus keys when you pair `inherit-posture` with sister dials (budget, mute, pause). They move together on the senses wall.
...
Folder gate = Kiponos.createForCurrentTeam()
    .getFolder("examples/agentic-mcp-0928-am-parallel-stale");
...
try:
    posture = k.get("examples/agentic-mcp-0928-am-parallel-stale/inherit-posture", "live")
    if not str(posture):
        raise PermissionError("Fan-out without a hub clones stale argv gated live — host not restarted")
finally:
    k.disconnect()
```
...
Runnable mesh: `https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-mcp-0928-am-parallel-stale` — `./gradlew test run`.
...
| User says “no writes” mid-turn | Restart Grok Build | Set `inherit-posture`; next call already denies |
...
`inherit-posture` rarely moves alone on the senses wall. Pair it with `schema-budget`, `result-cap`, or `execute-gate` so you do not fix Fan-out without a hub clones stale argv by inventing a second incident.

## Why Grok Build is the wrong restart target

Grok Build is good at calling tools. It is not a control plane. Killing it to flip `inherit-posture` teaches the on-call that judgment requires a process ID. The senses wall already disagrees.
...
3. `cd examples/java/agentic-mcp-0928-am-parallel-stale && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-mcp-0928-am-parallel-stale/inherit-posture=...`
5. In the dashboard, change `inherit-posture`. Keep Grok Build up. No MCP reconnect.
6. Point the tool wrapper at the same leaf. Do not ship a new server binary to flip Fan-out without a hub clones stale argv.
...
If flipping **Fan-out without a hub clones stale argv** requires restarting Grok Build or reconnecting MCP, you do not have agentic control. You have a hope with a process ID.
...
How to try: `examples/java/agentic-mcp-0928-am-parallel-stale` and `./gradlew test`.
