---
title: "We Stopped Shipping Rag Top-K as a Constant (Saas)"
published: false
...
Domain: multi-tenant quotas, feature posture. This essay maps hub key `topK` to **RAG top-k** so the lesson stays concrete for saas operators.
...
```java
Folder policy = kiponos.path("examples", "ops-saas-rag-topk");
...
1. Name the hub path so humans find it under pressure (`examples/ops-saas-rag-topk/topK`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: RAG top-k for saas — numbers humans already change verbally in war rooms.
