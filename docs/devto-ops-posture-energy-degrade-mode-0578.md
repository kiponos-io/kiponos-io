---
title: "The Energy War Room Already Knew degradeMode — The Jar Did Not"
published: false
...
Domain: grid dispatch, demand response. This essay maps hub key `degradeMode` to **degradation mode** so the lesson stays concrete for energy operators.
...
```java
Folder policy = kiponos.path("examples", "ops-energy-degrade-mode");
...
1. Name the hub path so humans find it under pressure (`examples/ops-energy-degrade-mode/degradeMode`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: degradation mode for energy — numbers humans already change verbally in war rooms.
