# Live Posture: aha-http-timeout

*A traveler’s note from the aha war room: a frozen dial under fire, frozen YAML, and the Super Pattern that lets ops move **timeout** without a jar.*
...
For aha, the painful dial was **timeout** (`timeout`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
```text
examples / aha-http-timeout / timeout
  timeout = baseline
```

- Every decision uses **local** `get()` after connect — no hub RTT on the hot path  
- Dashboard or remote SDK can change `timeout` while this process keeps running  
...
<!-- medium-img: diagram-aha-http-timeout-gof-vs-live.png -->
...
2. Move `timeout` to the emergency value (documented floor/ceiling).  
...
```bash
examples/java/aha-http-timeout
./gradlew test run
```

It prints the live `timeout` and a one-line decision trace. Change the hub. Run again. The jar is innocent.

Unit tests pin structure and clamps without tokens. Golden tests skip cleanly when `KIPONOS_ID` is a placeholder.

<!-- medium-img: diagram-aha-http-timeout-hub-flow.png -->
...
Timeout that requires a deploy is optimistic documentation.

People should not have to ship a release to make a decision the business already made in a sentence.

---

*Series: Kiponos Medium Super Patterns · Example: [`examples/java/aha-http-timeout`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/aha-http-timeout) · Product: [kiponos.io](https://kiponos.io)*
