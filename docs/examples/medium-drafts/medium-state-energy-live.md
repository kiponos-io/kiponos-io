# The State Pattern Transition Policy Live — Energy War Room

*A traveler’s note from the dispatch board: demand response, frozen YAML, and the Super Pattern that lets ops move **transition policy live** without a jar.*

---

At **03:41**, the energy war room already knew the number. The process still believed last week’s properties file.
...
For energy, the painful dial was **transition policy live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-state-energy-live-gof-vs-live.png -->
...
```bash
examples/java/medium-state-energy-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-state-energy-live-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-state-energy-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-state-energy-live) · Product: [kiponos.io](https://kiponos.io)*
