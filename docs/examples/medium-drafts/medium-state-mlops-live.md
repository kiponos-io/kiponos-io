# The State Pattern Transition Policy Live — Mlops War Room

*A traveler’s note from the score desk: batch size thrash, frozen YAML, and the Super Pattern that lets ops move **transition policy live** without a jar.*

---

At **03:41**, the mlops war room already knew the number. The process still believed last week’s properties file.
...
For mlops, the painful dial was **transition policy live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-state-mlops-live-gof-vs-live.png -->
...
```bash
examples/java/medium-state-mlops-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-state-mlops-live-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-state-mlops-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-state-mlops-live) · Product: [kiponos.io](https://kiponos.io)*
