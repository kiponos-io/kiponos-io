...
That sentence is the product brief for **live verbose probe flag**.
...
The Super Pattern is simpler: put **live verbose probe flag** (`verbose`) on [Kiponos.io](https://kiponos.io). Every peer uses **local `get()`** after bootstrap. Dashboard, agent, or any SDK `set()` when judgment arrives.

<!-- medium-img: diagram-agent-debug-probe-live-gof-vs-live.png -->
...
try {
    ...
    String v = f.hasKey("verbose") ? f.get("verbose") : "off";
...
# path/get style depends on kit — leaf is examples/agent-debug-probe-live/verbose
...
await kip.connect();
await kip.ensurePath('examples/agent-debug-probe-live');
// mirror to SPA via your API/SSE — SPA is a client of you, not of Connect
const v = /* local get after bootstrap */ 'off';
```

Angular **server** peer — same moral as React: process identity, thin UI mirror.

<!-- medium-img: diagram-agent-debug-probe-live-hub-flow.png -->

---

## The example (runnable)

Published under **`examples/java/agent-debug-probe-live`** on [github.com/kiponos-io/kiponos-io](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agent-debug-probe-live)

Python companion: `examples/python/agent-debug-probe-live`

Peers in this story: **python, java, react**
...
# Python agent peer
cd examples/python/agent-debug-probe-live
...
|------|-----------|-----------|
| Flip live verbose probe flag | Redeploy N services | One hub `set` |
...
Identity is **where the process runs**. The mesh is **one living tree**. Java, Python, React, Angular, and agents are peers. Change `verbose`. Watch every honest peer obey. Keep the release train for real code.
...
No. We’ll put **live verbose probe flag** on the hub, put **identity** on each process, let agents stay warm, and let every SDK speak the same leaf while the world is still on fire.

---

*Runnable: [https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agent-debug-probe-live](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agent-debug-probe-live)*
