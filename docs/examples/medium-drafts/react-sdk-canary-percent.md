...
*A traveler’s note on live canary traffic percent — with the Kiponos React SDK as a hub peer, plus Java and Python on the same living tree.*

---

There is a class of production decisions that are **too small for a release** and **too important for a chat thread**.

We needed five percent of traffic on the new path for twelve minutes. The flag lived in a React env build and a Java ConfigMap. Two deploys later, the outage was over — and we still had not learned anything.

Someone said the sentence that always costs a night:

**“If canary percent waits for a release train, it is not a canary. It is a delayed confession.”**

That sentence is the product brief.

I have always believed people should not have to ship a release to make a decision. This story is one concrete use case — **Java**, **Python**, and **@kiponos/react** (Node server peer) on one leaf: `release/canary-percent`.

Redeploying a frontend, restarting a BFF, or bouncing an agent just to move **live canary traffic percent** is how teams invent folklore. Tokens drift into public bundles. Agents lose session memory. Java keeps the old value until the next ConfigMap. The decision was simple; the ceremony was not.
...
The engineers were not stupid. They needed **live canary traffic percent** to move. The mistake was freezing it in a build-time env, a YAML file, or a process restart — or treating the **browser** as the hub participant.
...
try {
    ...
    // honor live live canary traffic percent
...
| Peer | Path |
|------|------|
| Java | `examples/java/react-sdk-canary-percent` |
| Python | `examples/python/react-sdk-canary-percent` |
| Node (react) | `examples/node/react-sdk-canary-percent` |
| Story | `docs/examples/medium-drafts/react-sdk-canary-percent.md` |

Public surface: **[kiponos.io](https://kiponos.io)** and [github.com/kiponos-io/kiponos-io](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/react-sdk-canary-percent).
...
cd examples/node/react-sdk-canary-percent
npm install && node peer.mjs 5

# 3) Java peer reads the same leaf
cd examples/java/react-sdk-canary-percent
./gradlew test
./gradlew run

# 4) Python peer (pure logic + optional live)
cd examples/python/react-sdk-canary-percent
python3 peer.py
```
...
|------|-----------|----------|
| Change live canary traffic percent | Ship SPA / restart BFF | Node or dashboard `set` |
...
**People should not have to ship a release to make a decision** — and they should not paste service tokens into a SPA to share **live canary traffic percent**.

Identity is **where the process runs**. The React SDK is a **hub peer**, not a glorified fetch wrapper. Change `release/canary-percent`. Watch the tree. Keep the release train for real code.
...
“We’ll fix live canary traffic percent in the next react release.”
...
Pain we retire: *canary percent frozen in SPA builds and YAML until the next train*.

---

*Example tree: [https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/react-sdk-canary-percent](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/react-sdk-canary-percent)*
