# The State Pattern Transition Policy Live — Retail War Room

*A traveler’s note from the checkout war room: Black Friday cart, frozen YAML, and the Super Pattern that lets ops move **transition policy live** without a jar.*
...
For retail, the painful dial was **transition policy live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-state-retail-live-gof-vs-live.png -->
...
```bash
examples/java/medium-state-retail-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-state-retail-live-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-state-retail-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-state-retail-live) · Product: [kiponos.io](https://kiponos.io)*
