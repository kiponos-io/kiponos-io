...
*A traveler’s note from the checkout war room: Black Friday cart, frozen YAML, and the Super Pattern that lets ops move **reorder handlers live** without a jar.*

---

At **11:08**, the retail war room already knew the number. The process still believed last week’s properties file.
...
For retail, the painful dial was **reorder handlers live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-chain-retail-live-gof-vs-live.png -->
...
```bash
examples/java/medium-chain-retail-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-chain-retail-live-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-chain-retail-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-chain-retail-live) · Product: [kiponos.io](https://kiponos.io)*
