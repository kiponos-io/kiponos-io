# The State Pattern Transition Policy Live — Healthcare War Room

*A traveler’s note from the triage desk: queue depth spike, frozen YAML, and the Super Pattern that lets ops move **transition policy live** without a jar.*
...
For healthcare, the painful dial was **transition policy live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-state-healthcare-live-gof-vs-live.png -->
...
```bash
examples/java/medium-state-healthcare-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-state-healthcare-live-hub-flow.png -->
...
Transition policy live that requires a deploy is optimistic documentation.

People should not have to ship a release to make a decision the business already made in a sentence.

---

*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-state-healthcare-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-state-healthcare-live) · Product: [kiponos.io](https://kiponos.io)*
