# The Decorator Pattern Rebuild Wrapper Chains Live — Security War Room

*A traveler’s note from the WAF desk: auth leeway fight, frozen YAML, and the Super Pattern that lets ops move **rebuild wrapper chains live** without a jar.*

---

At **00:33**, the security war room already knew the number. The process still believed last week’s properties file.
...
For security, the painful dial was **rebuild wrapper chains live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-decorator-security-live-gof-vs-live.png -->

---

## The night auth leeway fight met a fossil
...
```bash
examples/java/medium-decorator-security-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-decorator-security-live-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-decorator-security-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-decorator-security-live) · Product: [kiponos.io](https://kiponos.io)*
