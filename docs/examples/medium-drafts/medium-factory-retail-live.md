# The Factory Pattern Select Product Channel Live — Retail War Room

*A traveler’s note from the checkout war room: Black Friday cart, frozen YAML, and the Super Pattern that lets ops move **select product channel live** without a jar.*

---

At **00:33**, the retail war room already knew the number. The process still believed last week’s properties file.
...
For retail, the painful dial was **select product channel live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-factory-retail-live-gof-vs-live.png -->

---

## The night Black Friday cart met a fossil
...
```bash
examples/java/medium-factory-retail-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-factory-retail-live-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-factory-retail-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-factory-retail-live) · Product: [kiponos.io](https://kiponos.io)*
