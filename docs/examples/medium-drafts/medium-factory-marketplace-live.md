# The Factory Pattern Select Product Channel Live — Marketplace War Room

*A traveler’s note from the seller tiers: fee weight fight, frozen YAML, and the Super Pattern that lets ops move **select product channel live** without a jar.*

---

At **11:08**, the marketplace war room already knew the number. The process still believed last week’s properties file.
...
For marketplace, the painful dial was **select product channel live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-factory-marketplace-live-gof-vs-live.png -->
...
```bash
examples/java/medium-factory-marketplace-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-factory-marketplace-live-hub-flow.png -->
...
Select product channel live that requires a deploy is optimistic documentation.

People should not have to ship a release to make a decision the business already made in a sentence.

---

*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-factory-marketplace-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-factory-marketplace-live) · Product: [kiponos.io](https://kiponos.io)*
