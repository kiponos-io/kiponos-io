# The Proxy Pattern Gate Access Live — Saas War Room

*A traveler’s note from the tenant ops: noisy neighbor, frozen YAML, and the Super Pattern that lets ops move **gate access live** without a jar.*
...
For saas, the painful dial was **gate access live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-proxy-saas-live-gof-vs-live.png -->
...
```bash
examples/java/medium-proxy-saas-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-proxy-saas-live-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-proxy-saas-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-proxy-saas-live) · Product: [kiponos.io](https://kiponos.io)*
