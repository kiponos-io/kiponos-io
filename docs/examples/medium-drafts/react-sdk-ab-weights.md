...
*A traveler’s note on live A/B weight vector — with the Kiponos React SDK as a hub peer, plus Java and Python on the same living tree.*

---

There is a class of production decisions that are **too small for a release** and **too important for a chat thread**.

Variant B was hurting conversion. Product wanted 90/10 in five minutes. Marketing waited for the next frontend train.

Someone said the sentence that always costs a night:

**“If weights wait for a bundle, the experiment is already over — you just have not admitted it.”**

That sentence is the product brief.

I have always believed people should not have to ship a release to make a decision. This story is one concrete use case — **Java**, **Python**, and **@kiponos/react** (Node server peer) on one leaf: `experiments/ab-weights`.

Redeploying a frontend, restarting a BFF, or bouncing an agent just to move **live A/B weight vector** is how teams invent folklore. Tokens drift into public bundles. Agents lose session memory. Java keeps the old value until the next ConfigMap. The decision was simple; the ceremony was not.
...
The engineers were not stupid. They needed **live A/B weight vector** to move. The mistake was freezing it in a build-time env, a YAML file, or a process restart — or treating the **browser** as the hub participant.
...
await kip.path('experiments').set('ab-weights', '50,50');
// UI mirror via your BFF/SSE — never Connect tokens in the browser
// const v = useKiponosValue('experiments/ab-weights', { defaultValue: '50,50' });
```
...
try {
    ...
    String v = p.hasKey("ab-weights") ? p.get("ab-weights") : "50,50";
    // honor live live A/B weight vector
...
| Peer | Path |
|------|------|
| Java | `examples/java/react-sdk-ab-weights` |
| Python | `examples/python/react-sdk-ab-weights` |
| Node (react) | `examples/node/react-sdk-ab-weights` |
| Story | `docs/examples/medium-drafts/react-sdk-ab-weights.md` |

Public surface: **[kiponos.io](https://kiponos.io)** and [github.com/kiponos-io/kiponos-io](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/react-sdk-ab-weights).
...
cd examples/node/react-sdk-ab-weights
npm install && node peer.mjs 50,50

# 3) Java peer reads the same leaf
cd examples/java/react-sdk-ab-weights
./gradlew test
./gradlew run

# 4) Python peer (pure logic + optional live)
cd examples/python/react-sdk-ab-weights
python3 peer.py
```
...
|------|-----------|----------|
| Change live A/B weight vector | Ship SPA / restart BFF | Node or dashboard `set` |
...
**People should not have to ship a release to make a decision** — and they should not paste service tokens into a SPA to share **live A/B weight vector**.

Identity is **where the process runs**. The React SDK is a **hub peer**, not a glorified fetch wrapper. Change `experiments/ab-weights`. Watch the tree. Keep the release train for real code.
...
“We’ll fix live A/B weight vector in the next react release.”
...
Pain we retire: *experiment weights frozen in client bundles until the next SPA ship*.

---

*Example tree: [https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/react-sdk-ab-weights](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/react-sdk-ab-weights)*
