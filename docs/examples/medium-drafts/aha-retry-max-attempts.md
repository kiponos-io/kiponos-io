# Live Posture: aha-retry-max-attempts

*A traveler’s note from the aha war room: a frozen dial under fire, frozen YAML, and the Super Pattern that lets ops move **attempts** without a jar.*

---

At **03:41**, the aha war room already knew the number. The process still believed last week’s properties file.
...
For aha, the painful dial was **attempts** (`attempts`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
```text
examples / aha-retry-max-attempts / attempts
  attempts = baseline
```

- Every decision uses **local** `get()` after connect — no hub RTT on the hot path  
- Dashboard or remote SDK can change `attempts` while this process keeps running  
...
<!-- medium-img: diagram-aha-retry-max-attempts-gof-vs-live.png -->
...
2. Move `attempts` to the emergency value (documented floor/ceiling).  
...
```bash
examples/java/aha-retry-max-attempts
./gradlew test run
```

It prints the live `attempts` and a one-line decision trace. Change the hub. Run again. The jar is innocent.

Unit tests pin structure and clamps without tokens. Golden tests skip cleanly when `KIPONOS_ID` is a placeholder.

<!-- medium-img: diagram-aha-retry-max-attempts-hub-flow.png -->
...
Attempts that requires a deploy is optimistic documentation.

People should not have to ship a release to make a decision the business already made in a sentence.

---

*Series: Kiponos Medium Super Patterns · Example: [`examples/java/aha-retry-max-attempts`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/aha-retry-max-attempts) · Product: [kiponos.io](https://kiponos.io)*
