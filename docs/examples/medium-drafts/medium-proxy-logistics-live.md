# The Proxy Pattern Gate Access Live — Logistics War Room

*A traveler’s note from the dispatch floor: SLA breach window, frozen YAML, and the Super Pattern that lets ops move **gate access live** without a jar.*

---

At **03:41**, the logistics war room already knew the number. The process still believed last week’s properties file.
...
For logistics, the painful dial was **gate access live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-proxy-logistics-live-gof-vs-live.png -->
...
```bash
examples/java/medium-proxy-logistics-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-proxy-logistics-live-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-proxy-logistics-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-proxy-logistics-live) · Product: [kiponos.io](https://kiponos.io)*
