# The State Pattern Transition Policy Live — Sre War Room

*A traveler’s note from the error-budget room: canary bleed, frozen YAML, and the Super Pattern that lets ops move **transition policy live** without a jar.*
...
For sre, the painful dial was **transition policy live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-state-sre-live-gof-vs-live.png -->
...
```bash
examples/java/medium-state-sre-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-state-sre-live-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-state-sre-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-state-sre-live) · Product: [kiponos.io](https://kiponos.io)*
