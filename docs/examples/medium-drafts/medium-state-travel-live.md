# The State Pattern Transition Policy Live — Travel War Room

*A traveler’s note from the inventory desk: overbooking spike, frozen YAML, and the Super Pattern that lets ops move **transition policy live** without a jar.*
...
For travel, the painful dial was **transition policy live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-state-travel-live-gof-vs-live.png -->
...
```bash
examples/java/medium-state-travel-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-state-travel-live-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-state-travel-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-state-travel-live) · Product: [kiponos.io](https://kiponos.io)*
