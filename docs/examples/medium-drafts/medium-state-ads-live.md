# The State Pattern Transition Policy Live — Ads War Room

*A traveler’s note from the pacing ops: bid cap thrash, frozen YAML, and the Super Pattern that lets ops move **transition policy live** without a jar.*
...
For ads, the painful dial was **transition policy live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-state-ads-live-gof-vs-live.png -->
...
```bash
examples/java/medium-state-ads-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-state-ads-live-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-state-ads-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-state-ads-live) · Product: [kiponos.io](https://kiponos.io)*
