# The Factory Pattern Select Product Channel Live — Media War Room

*A traveler’s note from the origin ops: bitrate storm, frozen YAML, and the Super Pattern that lets ops move **select product channel live** without a jar.*

---

At **00:33**, the media war room already knew the number. The process still believed last week’s properties file.
...
For media, the painful dial was **select product channel live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-factory-media-live-gof-vs-live.png -->
...
```bash
examples/java/medium-factory-media-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-factory-media-live-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-factory-media-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-factory-media-live) · Product: [kiponos.io](https://kiponos.io)*
