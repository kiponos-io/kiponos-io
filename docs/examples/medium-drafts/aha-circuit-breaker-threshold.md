# Live Posture: aha-circuit-breaker-threshold

*A traveler’s note from the aha war room: a frozen dial under fire, frozen YAML, and the Super Pattern that lets ops move **threshold** without a jar.*

---

At **16:52**, the aha war room already knew the number. The process still believed last week’s properties file.
...
For aha, the painful dial was **threshold** (`threshold`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
```text
examples / aha-circuit-breaker-threshold / threshold
  threshold = baseline
```

- Every decision uses **local** `get()` after connect — no hub RTT on the hot path  
- Dashboard or remote SDK can change `threshold` while this process keeps running  
...
<!-- medium-img: diagram-aha-circuit-breaker-threshold-gof-vs-live.png -->
...
2. Move `threshold` to the emergency value (documented floor/ceiling).  
...
```bash
examples/java/aha-circuit-breaker-threshold
./gradlew test run
```

It prints the live `threshold` and a one-line decision trace. Change the hub. Run again. The jar is innocent.

Unit tests pin structure and clamps without tokens. Golden tests skip cleanly when `KIPONOS_ID` is a placeholder.

<!-- medium-img: diagram-aha-circuit-breaker-threshold-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/aha-circuit-breaker-threshold`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/aha-circuit-breaker-threshold) · Product: [kiponos.io](https://kiponos.io)*
