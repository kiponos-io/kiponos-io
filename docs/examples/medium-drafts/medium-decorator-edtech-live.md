# The Decorator Pattern Rebuild Wrapper Chains Live — Edtech War Room

*A traveler’s note from the exam window: class-size shock, frozen YAML, and the Super Pattern that lets ops move **rebuild wrapper chains live** without a jar.*

---

At **03:41**, the edtech war room already knew the number. The process still believed last week’s properties file.
...
For edtech, the painful dial was **rebuild wrapper chains live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-decorator-edtech-live-gof-vs-live.png -->

---

## The night class-size shock met a fossil
...
```bash
examples/java/medium-decorator-edtech-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-decorator-edtech-live-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-decorator-edtech-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-decorator-edtech-live) · Product: [kiponos.io](https://kiponos.io)*
