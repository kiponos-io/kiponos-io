# The Decorator Pattern Rebuild Wrapper Chains Live — Telecom War Room

*A traveler’s note from the QoS bridge: trunk saturation, frozen YAML, and the Super Pattern that lets ops move **rebuild wrapper chains live** without a jar.*

---

At **02:17**, the telecom war room already knew the number. The process still believed last week’s properties file.
...
For telecom, the painful dial was **rebuild wrapper chains live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-decorator-telecom-live-gof-vs-live.png -->
...
```bash
examples/java/medium-decorator-telecom-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-decorator-telecom-live-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-decorator-telecom-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-decorator-telecom-live) · Product: [kiponos.io](https://kiponos.io)*
