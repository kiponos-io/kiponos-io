# The Strategy Pattern Swap Algorithms Live — Banking War Room

*A traveler’s note from the AML desk: transfer storm, frozen YAML, and the Super Pattern that lets ops move **swap algorithms live** without a jar.*

---

At **22:09**, the banking war room already knew the number. The process still believed last week’s properties file.
...
For banking, the painful dial was **swap algorithms live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-strategy-banking-live-gof-vs-live.png -->

---

## The night transfer storm met a fossil
...
```bash
examples/java/medium-strategy-banking-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-strategy-banking-live-hub-flow.png -->
...
Swap algorithms live that requires a deploy is optimistic documentation.

People should not have to ship a release to make a decision the business already made in a sentence.

---

*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-strategy-banking-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-strategy-banking-live) · Product: [kiponos.io](https://kiponos.io)*
