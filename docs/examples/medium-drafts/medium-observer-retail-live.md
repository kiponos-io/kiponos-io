# The Observer Pattern Subscribe Topology Live — Retail War Room

*A traveler’s note from the checkout war room: Black Friday cart, frozen YAML, and the Super Pattern that lets ops move **subscribe topology live** without a jar.*

---

At **16:52**, the retail war room already knew the number. The process still believed last week’s properties file.
...
For retail, the painful dial was **subscribe topology live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-observer-retail-live-gof-vs-live.png -->
...
```bash
examples/java/medium-observer-retail-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-observer-retail-live-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-observer-retail-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-observer-retail-live) · Product: [kiponos.io](https://kiponos.io)*
