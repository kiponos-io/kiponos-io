...
That is the GoF-versus-live cut for `max-snapshots`.

<!-- medium-img: diagram-pattern-memento-live-retention-gof-vs-live.png -->

That is the hub-flow cut: one leaf, every peer.

<!-- medium-img: diagram-pattern-memento-live-retention-hub-flow.png -->
---
...
If `max-snapshots` only moves through a release train, every product conversation becomes a ceremony debate: who owns the PR, who merges, who rolls, who watches. That tax compounds across regions and on-call rotations.
...
## War-room addendum — pattern memento live retention

I have sat in a 03:50 bridge where `max-snapshots` was already decided in a sentence and the process still served last week's jar. Someone offered the usual escape: we will cut a PR. A PR. While the room is already paying for the old number.
...
1. Name the hub path so humans find `max-snapshots` under pressure.
