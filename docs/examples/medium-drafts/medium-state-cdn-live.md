# The State Pattern Transition Policy Live — Cdn War Room

*A traveler’s note from the edge ops: purge storm, frozen YAML, and the Super Pattern that lets ops move **transition policy live** without a jar.*

---

At **16:52**, the cdn war room already knew the number. The process still believed last week’s properties file.
...
For cdn, the painful dial was **transition policy live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-state-cdn-live-gof-vs-live.png -->
...
```bash
examples/java/medium-state-cdn-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-state-cdn-live-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-state-cdn-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-state-cdn-live) · Product: [kiponos.io](https://kiponos.io)*
