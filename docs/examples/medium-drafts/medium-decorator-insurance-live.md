# The Decorator Pattern Rebuild Wrapper Chains Live — Insurance War Room

*A traveler’s note from the claims floor: velocity fraud, frozen YAML, and the Super Pattern that lets ops move **rebuild wrapper chains live** without a jar.*
...
For insurance, the painful dial was **rebuild wrapper chains live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-decorator-insurance-live-gof-vs-live.png -->
...
```bash
examples/java/medium-decorator-insurance-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-decorator-insurance-live-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-decorator-insurance-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-decorator-insurance-live) · Product: [kiponos.io](https://kiponos.io)*
