# The Strategy Pattern Swap Algorithms Live — Sre War Room

*A traveler’s note from the error-budget room: canary bleed, frozen YAML, and the Super Pattern that lets ops move **swap algorithms live** without a jar.*

---

At **22:09**, the sre war room already knew the number. The process still believed last week’s properties file.
...
For sre, the painful dial was **swap algorithms live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-strategy-sre-live-gof-vs-live.png -->
...
```bash
examples/java/medium-strategy-sre-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-strategy-sre-live-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-strategy-sre-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-strategy-sre-live) · Product: [kiponos.io](https://kiponos.io)*
