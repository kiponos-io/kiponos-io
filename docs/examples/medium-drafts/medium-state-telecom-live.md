# The State Pattern Transition Policy Live — Telecom War Room

*A traveler’s note from the QoS bridge: trunk saturation, frozen YAML, and the Super Pattern that lets ops move **transition policy live** without a jar.*
...
For telecom, the painful dial was **transition policy live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-state-telecom-live-gof-vs-live.png -->
...
```bash
examples/java/medium-state-telecom-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-state-telecom-live-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-state-telecom-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-state-telecom-live) · Product: [kiponos.io](https://kiponos.io)*
