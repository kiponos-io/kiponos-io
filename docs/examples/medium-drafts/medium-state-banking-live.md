# The State Pattern Transition Policy Live — Banking War Room

*A traveler’s note from the AML desk: transfer storm, frozen YAML, and the Super Pattern that lets ops move **transition policy live** without a jar.*

---

At **11:08**, the banking war room already knew the number. The process still believed last week’s properties file.
...
For banking, the painful dial was **transition policy live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-state-banking-live-gof-vs-live.png -->
...
```bash
examples/java/medium-state-banking-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-state-banking-live-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-state-banking-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-state-banking-live) · Product: [kiponos.io](https://kiponos.io)*
