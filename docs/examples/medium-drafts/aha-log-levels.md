# Live Posture: aha-log-levels

*A traveler’s note from the aha war room: a frozen dial under fire, frozen YAML, and the Super Pattern that lets ops move **levels** without a jar.*
...
For aha, the painful dial was **levels** (`levels`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
```text
examples / aha-log-levels / levels
  levels = baseline
```

- Every decision uses **local** `get()` after connect — no hub RTT on the hot path  
- Dashboard or remote SDK can change `levels` while this process keeps running  
...
<!-- medium-img: diagram-aha-log-levels-gof-vs-live.png -->
...
2. Move `levels` to the emergency value (documented floor/ceiling).  
...
```bash
examples/java/aha-log-levels
./gradlew test run
```

It prints the live `levels` and a one-line decision trace. Change the hub. Run again. The jar is innocent.

Unit tests pin structure and clamps without tokens. Golden tests skip cleanly when `KIPONOS_ID` is a placeholder.

<!-- medium-img: diagram-aha-log-levels-hub-flow.png -->
...
Levels that requires a deploy is optimistic documentation.

People should not have to ship a release to make a decision the business already made in a sentence.

---

*Series: Kiponos Medium Super Patterns · Example: [`examples/java/aha-log-levels`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/aha-log-levels) · Product: [kiponos.io](https://kiponos.io)*
