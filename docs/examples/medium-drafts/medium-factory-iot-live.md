# The Factory Pattern Select Product Channel Live — Iot War Room

*A traveler’s note from the device fleet: firmware gate jam, frozen YAML, and the Super Pattern that lets ops move **select product channel live** without a jar.*

---

At **02:17**, the iot war room already knew the number. The process still believed last week’s properties file.
...
For iot, the painful dial was **select product channel live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-factory-iot-live-gof-vs-live.png -->
...
```bash
examples/java/medium-factory-iot-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-factory-iot-live-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-factory-iot-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-factory-iot-live) · Product: [kiponos.io](https://kiponos.io)*
