# Live Posture: aha-kafka-max-poll

*A traveler’s note from the aha war room: a frozen dial under fire, frozen YAML, and the Super Pattern that lets ops move **poll** without a jar.*
...
For aha, the painful dial was **poll** (`poll`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
```text
examples / aha-kafka-max-poll / poll
  poll = baseline
```

- Every decision uses **local** `get()` after connect — no hub RTT on the hot path  
- Dashboard or remote SDK can change `poll` while this process keeps running  
...
<!-- medium-img: diagram-aha-kafka-max-poll-gof-vs-live.png -->
...
2. Move `poll` to the emergency value (documented floor/ceiling).  
...
```bash
examples/java/aha-kafka-max-poll
./gradlew test run
```

It prints the live `poll` and a one-line decision trace. Change the hub. Run again. The jar is innocent.

Unit tests pin structure and clamps without tokens. Golden tests skip cleanly when `KIPONOS_ID` is a placeholder.

<!-- medium-img: diagram-aha-kafka-max-poll-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/aha-kafka-max-poll`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/aha-kafka-max-poll) · Product: [kiponos.io](https://kiponos.io)*
