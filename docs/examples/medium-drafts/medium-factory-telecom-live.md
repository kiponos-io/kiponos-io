# The Factory Pattern Select Product Channel Live — Telecom War Room

*A traveler’s note from the QoS bridge: trunk saturation, frozen YAML, and the Super Pattern that lets ops move **select product channel live** without a jar.*

---

At **22:09**, the telecom war room already knew the number. The process still believed last week’s properties file.
...
For telecom, the painful dial was **select product channel live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-factory-telecom-live-gof-vs-live.png -->

---

## The night trunk saturation met a fossil
...
```bash
examples/java/medium-factory-telecom-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-factory-telecom-live-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-factory-telecom-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-factory-telecom-live) · Product: [kiponos.io](https://kiponos.io)*
