...
*A traveler’s note from the triage desk: queue depth spike, frozen YAML, and the Super Pattern that lets ops move **subscribe topology live** without a jar.*

---

At **11:08**, the healthcare war room already knew the number. The process still believed last week’s properties file.
...
For healthcare, the painful dial was **subscribe topology live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-observer-healthcare-live-gof-vs-live.png -->
...
```bash
examples/java/medium-observer-healthcare-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-observer-healthcare-live-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-observer-healthcare-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-observer-healthcare-live) · Product: [kiponos.io](https://kiponos.io)*
