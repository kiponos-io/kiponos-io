# The Decorator Pattern Rebuild Wrapper Chains Live — Healthcare War Room

*A traveler’s note from the triage desk: queue depth spike, frozen YAML, and the Super Pattern that lets ops move **rebuild wrapper chains live** without a jar.*

---

At **16:52**, the healthcare war room already knew the number. The process still believed last week’s properties file.
...
For healthcare, the painful dial was **rebuild wrapper chains live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-decorator-healthcare-live-gof-vs-live.png -->

---

## The night queue depth spike met a fossil
...
```bash
examples/java/medium-decorator-healthcare-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-decorator-healthcare-live-hub-flow.png -->
...
Rebuild wrapper chains live that requires a deploy is optimistic documentation.

People should not have to ship a release to make a decision the business already made in a sentence.

---

*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-decorator-healthcare-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-decorator-healthcare-live) · Product: [kiponos.io](https://kiponos.io)*
