# The State Pattern Transition Policy Live — Marketplace War Room

*A traveler’s note from the seller tiers: fee weight fight, frozen YAML, and the Super Pattern that lets ops move **transition policy live** without a jar.*

---

At **22:09**, the marketplace war room already knew the number. The process still believed last week’s properties file.
...
For marketplace, the painful dial was **transition policy live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-state-marketplace-live-gof-vs-live.png -->
...
```bash
examples/java/medium-state-marketplace-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-state-marketplace-live-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-state-marketplace-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-state-marketplace-live) · Product: [kiponos.io](https://kiponos.io)*
