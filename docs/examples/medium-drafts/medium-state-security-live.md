# The State Pattern Transition Policy Live — Security War Room

*A traveler’s note from the WAF desk: auth leeway fight, frozen YAML, and the Super Pattern that lets ops move **transition policy live** without a jar.*

---

At **22:09**, the security war room already knew the number. The process still believed last week’s properties file.
...
For security, the painful dial was **transition policy live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-state-security-live-gof-vs-live.png -->
...
```bash
examples/java/medium-state-security-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-state-security-live-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-state-security-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-state-security-live) · Product: [kiponos.io](https://kiponos.io)*
