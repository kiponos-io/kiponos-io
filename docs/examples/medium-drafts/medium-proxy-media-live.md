# The Proxy Pattern Gate Access Live — Media War Room

*A traveler’s note from the origin ops: bitrate storm, frozen YAML, and the Super Pattern that lets ops move **gate access live** without a jar.*

---

At **11:08**, the media war room already knew the number. The process still believed last week’s properties file.
...
For media, the painful dial was **gate access live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-proxy-media-live-gof-vs-live.png -->
...
```bash
examples/java/medium-proxy-media-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-proxy-media-live-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-proxy-media-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-proxy-media-live) · Product: [kiponos.io](https://kiponos.io)*
