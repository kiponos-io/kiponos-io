# The Factory Pattern Select Product Channel Live — Fintech War Room

*A traveler’s note from the payments SOC: acquirer brownout, frozen YAML, and the Super Pattern that lets ops move **select product channel live** without a jar.*

---

At **03:41**, the fintech war room already knew the number. The process still believed last week’s properties file.
...
For fintech, the painful dial was **select product channel live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-factory-fintech-live-gof-vs-live.png -->
...
```bash
examples/java/medium-factory-fintech-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-factory-fintech-live-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-factory-fintech-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-factory-fintech-live) · Product: [kiponos.io](https://kiponos.io)*
