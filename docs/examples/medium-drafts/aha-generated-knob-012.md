# Live Posture: aha-generated-knob-012

*A traveler’s note from the aha war room: a frozen dial under fire, frozen YAML, and the Super Pattern that lets ops move **012** without a jar.*
...
**“We'll tune next sprint — we’ll cut a PR.”**
...
For aha, the painful dial was **012** (`012`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
```text
examples / aha-generated-knob-012 / 012
  012 = baseline
```

- Every decision uses **local** `get()` after connect — no hub RTT on the hot path  
- Dashboard or remote SDK can change `012` while this process keeps running  
...
<!-- medium-img: diagram-aha-generated-knob-012-gof-vs-live.png -->
...
2. Move `012` to the emergency value (documented floor/ceiling).  
...
```bash
examples/java/aha-generated-knob-012
./gradlew test run
```

It prints the live `012` and a one-line decision trace. Change the hub. Run again. The jar is innocent.

Unit tests pin structure and clamps without tokens. Golden tests skip cleanly when `KIPONOS_ID` is a placeholder.

<!-- medium-img: diagram-aha-generated-knob-012-hub-flow.png -->
...
012 that requires a deploy is optimistic documentation.

People should not have to ship a release to make a decision the business already made in a sentence.

---

*Series: Kiponos Medium Super Patterns · Example: [`examples/java/aha-generated-knob-012`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/aha-generated-knob-012) · Product: [kiponos.io](https://kiponos.io)*
