# Visitor Super Pattern — Visitor Registry Live

*A traveler’s note: which visitors run live.*

---

There is a class of production decisions that are **too small for a release** and **too important for a wiki**.

which visitors run live

Redeploying a jar to change `visitors` is how teams invent 3am folklore.
...
Full source + tests: [https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/pattern-visitor-live-registry](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/pattern-visitor-live-registry)
...
*Example + tests: [https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/pattern-visitor-live-registry](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/pattern-visitor-live-registry)*

That is the GoF-versus-live cut for `visitors`.

<!-- medium-img: diagram-pattern-visitor-live-registry-gof-vs-live.png -->

That is the hub-flow cut: one leaf, every peer.

<!-- medium-img: diagram-pattern-visitor-live-registry-hub-flow.png -->
---
...
If `visitors` only moves through a release train, every product conversation becomes a ceremony debate: who owns the PR, who merges, who rolls, who watches. That tax compounds across regions and on-call rotations.
...
## War-room addendum — pattern visitor live registry

I have sat in a 03:40 bridge where `visitors` was already decided in a sentence and the process still served last week's jar. Someone offered the usual escape: we will cut a PR. A PR. While the room is already paying for the old number.
...
1. Name the hub path so humans find `visitors` under pressure.
