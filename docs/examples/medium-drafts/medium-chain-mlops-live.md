# The Chain of Responsibility Pattern Reorder Handlers Live — Mlops War Room

*A traveler’s note from the score desk: batch size thrash, frozen YAML, and the Super Pattern that lets ops move **reorder handlers live** without a jar.*

---

At **00:33**, the mlops war room already knew the number. The process still believed last week’s properties file.
...
For mlops, the painful dial was **reorder handlers live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-chain-mlops-live-gof-vs-live.png -->
...
```bash
examples/java/medium-chain-mlops-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-chain-mlops-live-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-chain-mlops-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-chain-mlops-live) · Product: [kiponos.io](https://kiponos.io)*
