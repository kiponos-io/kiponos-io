# The Observer Pattern Subscribe Topology Live — Security War Room

*A traveler’s note from the WAF desk: auth leeway fight, frozen YAML, and the Super Pattern that lets ops move **subscribe topology live** without a jar.*

---

At **16:52**, the security war room already knew the number. The process still believed last week’s properties file.
...
For security, the painful dial was **subscribe topology live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-observer-security-live-gof-vs-live.png -->
...
```bash
examples/java/medium-observer-security-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-observer-security-live-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-observer-security-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-observer-security-live) · Product: [kiponos.io](https://kiponos.io)*
