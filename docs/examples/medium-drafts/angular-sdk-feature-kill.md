...
*A traveler’s note on Angular-adjacent feature kill flag — with the Kiponos Angular SDK as a hub peer, plus Java and Python on the same living tree.*

---

There is a class of production decisions that are **too small for a release** and **too important for a chat thread**.

We killed the Java route. The Angular admin still showed the action. An operator clicked it twice before anyone noticed the peers disagreed.

Someone said the sentence that always costs a night:

**“A kill switch that leaves the admin live is not a kill switch. It is a rumor with a button.”**

That sentence is the product brief.

I have always believed people should not have to ship a release to make a decision. This story is one concrete use case — **Java**, **Python**, and **@kiponos/angular** (Node server peer) on one leaf: `flags/feature-x`.

Redeploying a frontend, restarting a BFF, or bouncing an agent just to move **Angular-adjacent feature kill flag** is how teams invent folklore. Tokens drift into public bundles. Agents lose session memory. Java keeps the old value until the next ConfigMap. The decision was simple; the ceremony was not.
...
The engineers were not stupid. They needed **Angular-adjacent feature kill flag** to move. The mistake was freezing it in a build-time env, a YAML file, or a process restart — or treating the **browser** as the hub participant.
...
try {
    Folder p = k.getRootFolder().folderOrCreate("flags");
    String v = p.hasKey("feature-x") ? p.get("feature-x") : "off";
    // honor live Angular-adjacent feature kill flag
    System.out.println("feature-x=" + v);
} finally {
    k.disconnect();
}
```
...
# hub leaf: flags/feature-x
print("python peer online — change feature-x on the hub, keep this process")
...
| Peer | Path |
|------|------|
| Java | `examples/java/angular-sdk-feature-kill` |
| Python | `examples/python/angular-sdk-feature-kill` |
| Node (angular) | `examples/node/angular-sdk-feature-kill` |
| Story | `docs/examples/medium-drafts/angular-sdk-feature-kill.md` |

Public surface: **[kiponos.io](https://kiponos.io)** and [github.com/kiponos-io/kiponos-io](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/angular-sdk-feature-kill).
...
cd examples/node/angular-sdk-feature-kill
npm install && node peer.mjs off

# 3) Java peer reads the same leaf
cd examples/java/angular-sdk-feature-kill
./gradlew test
./gradlew run

# 4) Python peer (pure logic + optional live)
cd examples/python/angular-sdk-feature-kill
python3 peer.py
```
...
|------|-----------|----------|
| Change Angular-adjacent feature kill flag | Ship SPA / restart BFF | Node or dashboard `set` |
...
**People should not have to ship a release to make a decision** — and they should not paste service tokens into a SPA to share **Angular-adjacent feature kill flag**.

Identity is **where the process runs**. The Angular SDK is a **hub peer**, not a glorified fetch wrapper. Change `flags/feature-x`. Watch the tree. Keep the release train for real code.
...
“We’ll fix Angular-adjacent feature kill flag in the next angular release.”

No. We’ll put **live posture** in the hub, put **identity** on the process, and let every peer — Java, Python, Node, thin UI mirror — honor `flags/feature-x` without waiting for CI.

Pain we retire: *Angular admin keeps offering a path after Java already killed it*.

---

*Example tree: [https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/angular-sdk-feature-kill](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/angular-sdk-feature-kill)*
