# Live Posture: pattern-adapter-live-psp

*A traveler’s note from the pattern war room: a frozen dial under fire, frozen YAML, and the Super Pattern that lets ops move **psp** without a jar.*

---

At **03:41**, the pattern war room already knew the number. The process still believed last week’s properties file.
...
For pattern, the painful dial was **psp** (`psp`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
```text
examples / pattern-adapter-live-psp / psp
  psp = baseline
```

- Every decision uses **local** `get()` after connect — no hub RTT on the hot path  
- Dashboard or remote SDK can change `psp` while this process keeps running  
...
<!-- medium-img: diagram-pattern-adapter-live-psp-gof-vs-live.png -->
...
2. Move `psp` to the emergency value (documented floor/ceiling).  
...
```bash
examples/java/pattern-adapter-live-psp
./gradlew test run
```

It prints the live `psp` and a one-line decision trace. Change the hub. Run again. The jar is innocent.

Unit tests pin structure and clamps without tokens. Golden tests skip cleanly when `KIPONOS_ID` is a placeholder.

<!-- medium-img: diagram-pattern-adapter-live-psp-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/pattern-adapter-live-psp`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/pattern-adapter-live-psp) · Product: [kiponos.io](https://kiponos.io)*
