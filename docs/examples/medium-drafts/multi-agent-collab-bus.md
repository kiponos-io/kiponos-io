...
That sentence is the product brief for **live agent handoff ticket**.
...
The Super Pattern is simpler: put **live agent handoff ticket** (`handoff-ticket`) on [Kiponos.io](https://kiponos.io). Every peer uses **local `get()`** after bootstrap. Dashboard, agent, or any SDK `set()` when judgment arrives.

<!-- medium-img: diagram-multi-agent-collab-bus-gof-vs-live.png -->
...
try {
    ...
    String v = f.hasKey("handoff-ticket") ? f.get("handoff-ticket") : "idle";
    System.out.println("handoff-ticket=" + v);
} finally {
    k.disconnect();
}
```
...
# path/get style depends on kit — leaf is examples/multi-agent-collab-bus/handoff-ticket
...
await kip.connect();
await kip.ensurePath('examples/multi-agent-collab-bus');
// mirror to SPA via your API/SSE — SPA is a client of you, not of Connect
const v = /* local get after bootstrap */ 'idle';
```

Angular **server** peer — same moral as React: process identity, thin UI mirror.

<!-- medium-img: diagram-multi-agent-collab-bus-hub-flow.png -->

---

## The example (runnable)

Published under **`examples/java/multi-agent-collab-bus`** on [github.com/kiponos-io/kiponos-io](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/multi-agent-collab-bus)

Python companion: `examples/python/multi-agent-collab-bus`
...
# Python agent peer
cd examples/python/multi-agent-collab-bus
...
|------|-----------|-----------|
| Flip live agent handoff ticket | Redeploy N services | One hub `set` |
...
Identity is **where the process runs**. The mesh is **one living tree**. Java, Python, React, Angular, and agents are peers. Change `handoff-ticket`. Watch every honest peer obey. Keep the release train for real code.
...
No. We’ll put **live agent handoff ticket** on the hub, put **identity** on each process, let agents stay warm, and let every SDK speak the same leaf while the world is still on fire.

---

*Runnable: [https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/multi-agent-collab-bus](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/multi-agent-collab-bus)*
