# The State Pattern Transition Policy Live — Edtech War Room

*A traveler’s note from the exam window: class-size shock, frozen YAML, and the Super Pattern that lets ops move **transition policy live** without a jar.*

---

At **00:33**, the edtech war room already knew the number. The process still believed last week’s properties file.
...
For edtech, the painful dial was **transition policy live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-state-edtech-live-gof-vs-live.png -->
...
```bash
examples/java/medium-state-edtech-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-state-edtech-live-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-state-edtech-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-state-edtech-live) · Product: [kiponos.io](https://kiponos.io)*
