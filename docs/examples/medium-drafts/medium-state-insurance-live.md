# The State Pattern Transition Policy Live — Insurance War Room

*A traveler’s note from the claims floor: velocity fraud, frozen YAML, and the Super Pattern that lets ops move **transition policy live** without a jar.*

---

At **11:08**, the insurance war room already knew the number. The process still believed last week’s properties file.
...
For insurance, the painful dial was **transition policy live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-state-insurance-live-gof-vs-live.png -->
...
```bash
examples/java/medium-state-insurance-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-state-insurance-live-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-state-insurance-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-state-insurance-live) · Product: [kiponos.io](https://kiponos.io)*
