...
That is the GoF-versus-live cut for `max-entries`.
...
## War-room addendum — pattern flyweight live cache

I have sat in a 00:27 bridge where `max-entries` was already decided in a sentence and the process still served last week's jar. Someone offered the usual escape: we will cut a PR. A PR. While the room is already paying for the old number.
...
1. Name the hub path so humans find `max-entries` under pressure.
