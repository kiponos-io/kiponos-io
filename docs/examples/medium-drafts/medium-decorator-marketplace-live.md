# The Decorator Pattern Rebuild Wrapper Chains Live — Marketplace War Room

*A traveler’s note from the seller tiers: fee weight fight, frozen YAML, and the Super Pattern that lets ops move **rebuild wrapper chains live** without a jar.*
...
For marketplace, the painful dial was **rebuild wrapper chains live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
```text
patterns / decorator / marketplace
  active = baseline
```
...
<!-- medium-img: diagram-medium-decorator-marketplace-live-gof-vs-live.png -->
...
```bash
examples/java/medium-decorator-marketplace-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-decorator-marketplace-live-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-decorator-marketplace-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-decorator-marketplace-live) · Product: [kiponos.io](https://kiponos.io)*
