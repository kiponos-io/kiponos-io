# The State Pattern Transition Policy Live — Saas War Room

*A traveler’s note from the tenant ops: noisy neighbor, frozen YAML, and the Super Pattern that lets ops move **transition policy live** without a jar.*

---

At **22:09**, the saas war room already knew the number. The process still believed last week’s properties file.
...
For saas, the painful dial was **transition policy live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-state-saas-live-gof-vs-live.png -->
...
```bash
examples/java/medium-state-saas-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-state-saas-live-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-state-saas-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-state-saas-live) · Product: [kiponos.io](https://kiponos.io)*
