# The Observer Pattern Subscribe Topology Live — Sre War Room

*A traveler’s note from the error-budget room: canary bleed, frozen YAML, and the Super Pattern that lets ops move **subscribe topology live** without a jar.*
...
For sre, the painful dial was **subscribe topology live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-observer-sre-live-gof-vs-live.png -->
...
```bash
examples/java/medium-observer-sre-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-observer-sre-live-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-observer-sre-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-observer-sre-live) · Product: [kiponos.io](https://kiponos.io)*
