# The State Pattern Transition Policy Live — Fintech War Room

*A traveler’s note from the payments SOC: acquirer brownout, frozen YAML, and the Super Pattern that lets ops move **transition policy live** without a jar.*
...
For fintech, the painful dial was **transition policy live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-state-fintech-live-gof-vs-live.png -->
...
```bash
examples/java/medium-state-fintech-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-state-fintech-live-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-state-fintech-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-state-fintech-live) · Product: [kiponos.io](https://kiponos.io)*
