# The State Pattern Transition Policy Live — Media War Room

*A traveler’s note from the origin ops: bitrate storm, frozen YAML, and the Super Pattern that lets ops move **transition policy live** without a jar.*

---

At **16:52**, the media war room already knew the number. The process still believed last week’s properties file.
...
For media, the painful dial was **transition policy live** (`active`). Default in the jar: `baseline`. Correct for a demo. Wrong for a brownout.
...
<!-- medium-img: diagram-medium-state-media-live-gof-vs-live.png -->
...
```bash
examples/java/medium-state-media-live
./gradlew test run
```
...
<!-- medium-img: diagram-medium-state-media-live-hub-flow.png -->
...
*Series: Kiponos Medium Super Patterns · Example: [`examples/java/medium-state-media-live`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/medium-state-media-live) · Product: [kiponos.io](https://kiponos.io)*
