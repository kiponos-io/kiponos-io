---
title: "The Healthcare War Room Already Knew degradeMode — The Jar Did Not"
published: false
...
main_image: https://iili.io/nFL0qCb.jpg
---
...
Domain: routing, queue depth, triage. This essay maps hub key `degradeMode` to **degradation mode** so the lesson stays concrete for healthcare operators.
...
```python
policy = kiponos.path("examples", "ops-healthcare-degrade-mode")
...
1. Name the hub path so humans find it under pressure (`examples/ops-healthcare-degrade-mode/degradeMode`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: degradation mode for healthcare — numbers humans already change verbally in war rooms.
