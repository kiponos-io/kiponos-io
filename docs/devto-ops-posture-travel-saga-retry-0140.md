---
title: "Saga Step Retry Budget Should Not Wait for a Redeploy — Travel Live Posture"
published: false
tags: java, travel, architecture, kiponos
description: "Live saga step retry budget via Kiponos Python SDK — travel posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-travel-saga-retry-0140.md
main_image: https://files.catbox.moe/7teof2.jpg
---
...
Domain: inventory, overbooking, GDS. This essay maps hub key `sagaMaxAttempts` to **saga step retry budget** so the lesson stays concrete for travel operators.
...
```python
policy = kiponos.path("examples", "ops-travel-saga-retry")
sagaMaxAttempts = min(int(policy.get("sagaMaxAttempts", 4)), HARD_MAX)
worker_pool.resize(sagaMaxAttempts)  # or admission semaphore
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-travel-saga-retry/sagaMaxAttempts`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: saga step retry budget for travel — numbers humans already change verbally in war rooms.
