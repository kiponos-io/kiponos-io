---
title: "Inference Batch Size Should Not Wait for a Redeploy — Security Live Posture"
published: false
tags: java, security, devops, kiponos
description: "Live inference batch size via Kiponos Java SDK — security posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-security-inference-batch-1035.md
main_image: ./devto-cover-ops-posture-security-inference-batch-1035.jpg
---
...
Domain: WAF, RPS, auth leeway. This essay maps hub key `batchSize` to **inference batch size** so the lesson stays concrete for security operators.
...
```java
var policy = kiponos.path("examples", "ops-security-inference-batch");
String mode = policy.getString("degradeMode", "full");
int batchSize = policy.getInt("batchSize");
return router.decide(mode, batchSize);
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-security-inference-batch/batchSize`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: inference batch size for security — numbers humans already change verbally in war rooms.
...
2. Move `batchSize` to the emergency value (documented floor/ceiling).  
...
`batchSize` rarely moves alone. Pair with timeout/retry, canary share, or sampling so you do not fix one symptom by creating another.

## Failure budget vs this dial

Treat `batchSize` as a slice of **error budget**, not a comfort blanket. Raise it when the dependency is healthy; lower it when the dependency is already sick. Write the number that survives a bad day, not the number that flatters a sunny demo.
