---
title: "The Gaming War Room Already Knew velocityCap — The Jar Did Not"
published: false
...
Domain: matchmaking, session caps, tick rate. This essay maps hub key `velocityCap` to **fraud velocity cap** so the lesson stays concrete for gaming operators.
...
```python
policy = kiponos.path("examples", "ops-gaming-velocity-cap")
...
1. Name the hub path so humans find it under pressure (`examples/ops-gaming-velocity-cap/velocityCap`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: fraud velocity cap for gaming — numbers humans already change verbally in war rooms.
