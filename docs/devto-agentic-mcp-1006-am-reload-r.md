...
description: "Grok Build on senses wall: Refresh is a smell. Hub notify is the product.. Grok `/mcps` refresh after config edits. Other sessions do not hear it."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-mcp-1006-am-reload-r.md
---
...
Kiponos rule: no Refresh buttons as the design. Same moral for agent hosts.

Someone on the floor said, out loud, **hit r in the MCP tab**. Grok Build had started earlier. The tool list was frozen in `config.toml` / `.mcp.json` / a skill file. The only “safe” move anyone trusted was:
...
That is the ceremony. **Grok `/mcps` refresh after config edits. Other sessions do not hear it.**

**The Aha:** If you needed Refresh, the leaf did not move. Move the leaf. Every host already `get()`s. Real-time nested state is now a **primitive** — as native as a hashmap — via [Kiponos.io](https://kiponos.io). The host stays up. The leaf moves. The next tool call already obeys.
...
Grok `/mcps` refresh after config edits. Other sessions do not hear it.
...
Profile shape: `['my-app']['v1.0.0']['dev']['base']`. The leaf in this story is `tools-allow` under `examples/agentic-mcp-1006-am-reload-r/tools-allow`.
...
```yaml
examples/agentic-mcp-1006-am-reload-r:
...
Folder gate = Kiponos.createForCurrentTeam()
    .getFolder("examples/agentic-mcp-1006-am-reload-r");
...
try:
    posture = k.get("examples/agentic-mcp-1006-am-reload-r/tools-allow", "live")
    if not str(posture):
        raise PermissionError("Refresh is a smell. Hub notify is the product. gated live — host not restarted")
finally:
    k.disconnect()
```
...
Runnable mesh: `https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-mcp-1006-am-reload-r` — `./gradlew test run`.
...
`tools-allow` rarely moves alone on the senses wall. Pair it with `schema-budget`, `result-cap`, or `execute-gate` so you do not fix Refresh is a smell. Hub notify is the product. by inventing a second incident.

## Why Grok Build is the wrong restart target

Grok Build is good at calling tools. It is not a control plane. Killing it to flip `tools-allow` teaches the on-call that judgment requires a process ID. The senses wall already disagrees.
...
3. `cd examples/java/agentic-mcp-1006-am-reload-r && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-mcp-1006-am-reload-r/tools-allow=...`
5. In the dashboard, change `tools-allow`. Keep Grok Build up. No MCP reconnect.
6. Point the tool wrapper at the same leaf. Do not ship a new server binary to flip Refresh is a smell. Hub notify is the product..
...
If flipping **Refresh is a smell. Hub notify is the product.** requires restarting Grok Build or reconnecting MCP, you do not have agentic control. You have a hope with a process ID.
...
How to try: `examples/java/agentic-mcp-1006-am-reload-r` and `./gradlew test`.
