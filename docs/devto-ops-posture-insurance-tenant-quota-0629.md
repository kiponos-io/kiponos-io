---
title: "Insurance: Turn per-tenant quota Into Incident Posture"
published: false
tags: java, insurance, architecture, kiponos
description: "Live per-tenant quota via Kiponos Python SDK — insurance posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-insurance-tenant-quota-0629.md
main_image: ./devto-cover-ops-posture-insurance-tenant-quota-0629.jpg
---
...
Domain: underwriting, claims velocity. This essay maps hub key `quotaPerMin` to **per-tenant quota** so the lesson stays concrete for insurance operators.
...
```python
policy = kiponos.path("examples", "ops-insurance-tenant-quota")
...
1. Name the hub path so humans find it under pressure (`examples/ops-insurance-tenant-quota/quotaPerMin`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: per-tenant quota for insurance — numbers humans already change verbally in war rooms.
