...
**The Aha:** `lkgMode` is not a property-file trophy. It is **incident posture** for config-server blink — and posture that waits for a jar is already late.

At 02:14 the war room already knew the right **offline LKG reads**. The jar still believed last week's YAML.

## The problem: ceremony between judgment and effect

When **offline LKG reads** is frozen in a deploy unit, every incident becomes a process argument. Hotfixes still mean CI + roll. Feature flags often mean a second control plane with its own delay.
...
```yaml
examples/
  aha-api-10-offline-lkg-reads/
    lkgMode: <live>
```

```java
Folder policy = kiponos.path("examples", "aha-api-10-offline-lkg-reads");
int lkgMode = policy.getInt("lkgMode"); // local get — no hub RTT
if (lkgMode <= 0) {
    return failClosed();
}
return apply(lkgMode);
```

Ops (or automation) sets `lkgMode` in the hub. The **next** evaluation uses it. Same binary. Same tests for structure.
...
- Canonical source: [this article on GitHub](https://github.com/kiponos-io/kiponos-io/blob/master/docs/examples/medium-drafts/api-10-offline-lkg-reads.md)
...
Feature flags are often product gates. This essay is about **ops posture** on config-server blink: **offline LKG reads** — numbers war rooms already shout.
...
Treat `lkgMode` as a slice of error budget. Raise when the world is healthy; lower when dependencies are sick. Write the number that survives a bad day.
...
**Offline LKG reads** that requires a deploy is optimistic documentation.
