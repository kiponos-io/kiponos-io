...
description: "Tune saga step timeouts, retry budgets, and compensation triggers across Java services without redeploy. Shared Kiponos tree with zero-latency local reads on ev"
...
**The Aha:** `sagaMaxAttempts` is not a property-file trophy. It is **incident posture** for distributed sagas — and posture that waits for a jar is already late.

Redeploying to change `sagaMaxAttempts` mid-incident is how teams invent 3am folklore around distributed sagas.

## The problem: ceremony between judgment and effect

When **saga retry budget** is frozen in a deploy unit, every incident becomes a process argument. Hotfixes still mean CI + roll. Feature flags often mean a second control plane with its own delay.
...
examples/
  microservices-saga/
    sagaMaxAttempts: <live>
```

```java
Folder policy = kiponos.path("examples", "microservices-saga");
int sagaMaxAttempts = policy.getInt("sagaMaxAttempts"); // local get — no hub RTT
...
Ops (or automation) sets `sagaMaxAttempts` in the hub. The **next** evaluation uses it. Same binary. Same tests for structure.
...
- Canonical source: [this article on GitHub](https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-microservices-saga.md)
...
Feature flags are often product gates. This essay is about **ops posture** on distributed sagas: **saga retry budget** — numbers war rooms already shout.
...
Treat `sagaMaxAttempts` as a slice of error budget. Raise when the world is healthy; lower when dependencies are sick. Write the number that survives a bad day.
...
**Saga retry budget** that requires a deploy is optimistic documentation.
