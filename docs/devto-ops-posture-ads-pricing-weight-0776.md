---
title: "We Stopped Shipping Pricing / Promo Weight as a Constant (Ads)"
published: false
tags: java, advertising, devops, kiponos
description: "Live pricing / promo weight via Kiponos Java SDK — ads posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-ads-pricing-weight-0776.md
main_image: https://files.catbox.moe/etajhn.jpg
---
...
Domain: bid caps, frequency, pacing. This essay maps hub key `weight` to **pricing / promo weight** so the lesson stays concrete for ads operators.
...
```java
Folder policy = kiponos.path("examples", "ops-ads-pricing-weight");
int weight = Math.min(policy.getInt("weight"), HARD_MAX);
limiter.setLimit(weight); // next evaluation sees it
```
...
1. Name the hub path so humans find it under pressure (`examples/ops-ads-pricing-weight/weight`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: pricing / promo weight for ads — numbers humans already change verbally in war rooms.
