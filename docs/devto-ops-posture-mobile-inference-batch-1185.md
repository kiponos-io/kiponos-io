---
title: "Inference Batch Size Should Not Wait for a Redeploy — Mobile Live Posture"
published: false
...
main_image: ./devto-cover-ops-posture-mobile-inference-batch-1185.jpg
---
...
Domain: API timeouts, payload caps. This essay maps hub key `batchSize` to **inference batch size** so the lesson stays concrete for mobile operators.
...
```python
policy = kiponos.path("examples", "ops-mobile-inference-batch")
...
1. Name the hub path so humans find it under pressure (`examples/ops-mobile-inference-batch/batchSize`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: inference batch size for mobile — numbers humans already change verbally in war rooms.
