...
description: "Grok Build on admin-dashboard wall: Sandboxes isolate CPU. They do not isolate policy.. E2B/MCP code-exec is essential. Whether *this* turn may exec is a live g"
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-mcp-1005-pm-sandbox-not-posture.md
---
...
**The Aha:** Sandbox is where. `execute-gate` is whether. Two different bugs. Real-time nested state is now a **primitive** — as native as a hashmap — via [Kiponos.io](https://kiponos.io). The host stays up. The leaf moves. The next tool call already obeys.
...
```mermaid
flowchart LR
  wall[admin-dashboard wall] -->|set execute-gate| hub[Kiponos hub]
