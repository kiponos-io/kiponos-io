---
main_image: https://iili.io/nKbVJaf.jpg
title: "Cursor Finished the Turn Blind — Shopping incident pause as a leaf on the Mirror Phone Path"
published: false
tags: java, python, devops, ai, kiponos
description: "Cursor kept finishing the turn blind on mirror-phone wall. Shopping incident pause as a leaf is a live hub leaf."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-dev-1031-am-shopping-pause.md
---

I have sat next to the mirror-phone wall at 22:19 while Cursor was *this close* to doing the wrong thing.

Not a model failure. A **posture** failure.

The host had started with `incident-pause` frozen in env / argv / a skill file. Someone on the floor said, out loud, **the phone already knew the posture — the host did not**. Cursor was still holding the old process. The only “safe” move anyone trusted was:
...
| Restart Cursor — it is cheap | Cheap until 22:19 ate live device context and the current tool call |
...
```text
examples/agentic-dev-1031-am-shopping-pause/incident-pause = paused
```

Runnable proof: [`examples/java/agentic-dev-1031-am-shopping-pause`](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-dev-1031-am-shopping-pause)
...
```yaml
examples/
  agentic-dev-1031-am-shopping-pause/
...
Folder gate = kip.getRootFolder()
        ...
        .folderOrCreate("agentic-dev-1031-am-shopping-pause");
...
try:
    posture = k.get("examples/agentic-dev-1031-am-shopping-pause/incident-pause", "paused")
...
|-------|-----------------|--------------|
| Device leaf gone stale while the agent is mid-turn | Restart Cursor; lose live device context and the current tool call | Set `incident-pause` live; next Cursor tool call already obeys |
...
| Incident over, resume | Another Cursor restart | Set `incident-pause` back; session continues |
...
In staging: set a painful `incident-pause`, prove Cursor recovers without a host kill, prove clamps reject nonsense, prove last-known-good when the hub is firewalled. That drill ends half the architecture arguments about Shopping incident pause as a leaf.

## Why Cursor is the wrong restart target

Cursor is good at calling tools. It is not a control plane. Killing it to flip `incident-pause` teaches the on-call that judgment requires a process ID. The mirror-phone wall already disagrees.
...
3. `cd examples/java/agentic-dev-1031-am-shopping-pause && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-dev-1031-am-shopping-pause/incident-pause=...`
5. In the dashboard, change `incident-pause`. Keep the process up. No rebuild.
6. Point your Cursor tool at the same leaf. Do not ship a new server binary to flip Shopping incident pause as a leaf.
...
If flipping **Shopping incident pause as a leaf** requires restarting Cursor, you do not have a gate. You have a hope with a process ID.
...
How to try: `examples/java/agentic-dev-1031-am-shopping-pause` and `./gradlew test`.
