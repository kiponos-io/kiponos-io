---
title: "We Stopped Shipping Shadow Traffic Share as a Constant (Ads)"
published: false
...
Domain: bid caps, frequency, pacing. This essay maps hub key `shadowPercent` to **shadow traffic share** so the lesson stays concrete for ads operators.
...
```python
policy = kiponos.path("examples", "ops-ads-shadow-percent")
...
1. Name the hub path so humans find it under pressure (`examples/ops-ads-shadow-percent/shadowPercent`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: shadow traffic share for ads — numbers humans already change verbally in war rooms.
