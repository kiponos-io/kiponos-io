---
title: "Mlops: Turn search scroll timeout Into Incident Posture"
published: false
...
main_image: https://iili.io/CQLGhKl.jpg
---
...
Domain: batch size, score thresholds. This essay maps hub key `scrollTimeoutSec` to **search scroll timeout** so the lesson stays concrete for mlops operators.
...
```java
Folder policy = kiponos.path("examples", "ops-mlops-scroll-timeout");
...
1. Name the hub path so humans find it under pressure (`examples/ops-mlops-scroll-timeout/scrollTimeoutSec`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: search scroll timeout for mlops — numbers humans already change verbally in war rooms.
