---
title: "We Stopped Shipping Outbox Poll Interval as a Constant (Energy)"
published: false
tags: java, energy, architecture, kiponos
description: "Live outbox poll interval via Kiponos Java SDK — energy posture without redeploy."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-ops-posture-energy-outbox-poll-ms-0591.md
main_image: https://files.catbox.moe/txun9d.jpg
---
...
Domain: grid dispatch, demand response. This essay maps hub key `outboxPollMs` to **outbox poll interval** so the lesson stays concrete for energy operators.
...
```java
Folder policy = kiponos.path("examples", "ops-energy-outbox-poll-ms");
...
1. Name the hub path so humans find it under pressure (`examples/ops-energy-outbox-poll-ms/outboxPollMs`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: outbox poll interval for energy — numbers humans already change verbally in war rooms.
