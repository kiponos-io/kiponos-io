---
title: "Healthcare: Turn OAuth token TTL Into Incident Posture"
published: false
...
main_image: https://iili.io/Cszfq9s.jpg
---
...
Domain: routing, queue depth, triage. This essay maps hub key `tokenTtlSec` to **OAuth token TTL** so the lesson stays concrete for healthcare operators.
...
```python
policy = kiponos.path("examples", "ops-healthcare-oauth-ttl")
...
1. Name the hub path so humans find it under pressure (`examples/ops-healthcare-oauth-ttl/tokenTtlSec`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: OAuth token TTL for healthcare — numbers humans already change verbally in war rooms.
