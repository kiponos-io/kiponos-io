---
title: "We Stopped Shipping Slo Warn Burn Bps as a Constant (Telecom)"
published: false
...
main_image: https://iili.io/CD71h9j.jpg
---
...
Domain: QoS, rate classes, trunk limits. This essay maps hub key `errorBudgetWarnBps` to **SLO warn burn bps** so the lesson stays concrete for telecom operators.
...
```java
Folder policy = kiponos.path("examples", "ops-telecom-slo-warn-bps");
...
1. Name the hub path so humans find it under pressure (`examples/ops-telecom-slo-warn-bps/errorBudgetWarnBps`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: SLO warn burn bps for telecom — numbers humans already change verbally in war rooms.
