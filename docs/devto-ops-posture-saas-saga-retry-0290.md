---
title: "Saga Step Retry Budget Should Not Wait for a Redeploy — Saas Live Posture"
published: false
...
The partner brownout did not wait for your change-management window. Your saga step retry budget did — until it lived in a hub.

Domain: multi-tenant quotas, feature posture. This essay maps hub key `sagaMaxAttempts` to **saga step retry budget** so the lesson stays concrete for saas operators.
...
```python
policy = kiponos.path("examples", "ops-saas-saga-retry")
...
1. Name the hub path so humans find it under pressure (`examples/ops-saas-saga-retry/sagaMaxAttempts`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: saga step retry budget for saas — numbers humans already change verbally in war rooms.
