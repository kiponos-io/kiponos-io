---
title: "Agents That Hand Off Work Without a Restart Handshake"
published: false
...
Agent A finished a diagnosis at 02:11. Agent B never saw it — the handoff file was on a dead worker.

Someone said:

> Handoff is not a filesystem. Handoff is a living leaf every peer can read.

Domain: multi-SDK distributed systems, agents, MCP, observability, war-room truth. This essay maps hub key `handoff-ticket` under `examples/multi-agent-collab-bus/` so the lesson stays concrete.

## The problem: ceremony between judgment and effect

You already know the right value for **live agent handoff ticket**. The war room knows. The agent almost knows. What you do not have is a path from **mouth → every running peer** that is shorter than a release train and safer than pasting tokens into a SPA.

When `handoff-ticket` is frozen in YAML (or baked into a browser build), every incident becomes a process argument. When it lives in a hub with clamps, the argument ends and the work begins — for Java services, Python agents, React BFFs, and Angular admin hosts **at once**.
...
examples/
  multi-agent-collab-bus/
    handoff-ticket: idle   # live agent handoff ticket
...
```text
examples / multi-agent-collab-bus
  handoff-ticket = idle
...
String posture = f.hasKey("handoff-ticket")
    ? f.get("handoff-ticket")
    : "idle";
// honor posture on the hot path — no hub RTT
```

Runnable: [examples/java/multi-agent-collab-bus](https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/multi-agent-collab-bus)
...
# Read examples/multi-agent-collab-bus/handoff-ticket live.
# Enabling a skill, tightening a budget, or pausing MCP writes
# must not require killing this process.
```

Companion: [examples/python/multi-agent-collab-bus](https://github.com/kiponos-io/kiponos-io/tree/master/examples/python/multi-agent-collab-bus)
...
await kip.connect();
// set/get under examples/multi-agent-collab-bus — inject mirror to SPA; never embed Connect tokens in bundles
```
...
3. `cd examples/java/multi-agent-collab-bus && cp kiponos.local.env.example kiponos.local.env` and fill tokens.  
4. `./gradlew test run` — print `handoff-ticket`.  
5. Flip the leaf on the dashboard; re-run or leave a long-lived peer connected — no rebuild.  
6. Optional: `examples/python/multi-agent-collab-bus` agent peer; `@kiponos/react` / `@kiponos/angular` on Node.
...
**live agent handoff ticket** belongs on the hub — not in four redeploys, not in a dead agent session, not in a browser secret. The multi-SDK mesh is here. Use it while the process still runs.
