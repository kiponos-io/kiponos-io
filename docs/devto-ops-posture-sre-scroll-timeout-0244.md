---
title: "Sre: Turn search scroll timeout Into Incident Posture"
published: false
...
Domain: canary, drain, error budget. This essay maps hub key `scrollTimeoutSec` to **search scroll timeout** so the lesson stays concrete for sre operators.
...
```python
policy = kiponos.path("examples", "ops-sre-scroll-timeout")
...
1. Name the hub path so humans find it under pressure (`examples/ops-sre-scroll-timeout/scrollTimeoutSec`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: search scroll timeout for sre — numbers humans already change verbally in war rooms.
