---
main_image: https://iili.io/n2yauTP.jpg
title: "Cursor Had the New Tool Roster. Grok Build Started Earlier and Still Called Write"
published: false
tags: ai, mcp, devops, java, kiponos
description: "Claude Code on admin-dashboard wall: Two agent hosts, one live posture. Paste the allow-list into the other chat. Two ceremonies, two lies."
canonical_url: https://github.com/kiponos-io/kiponos-io/blob/master/docs/devto-agentic-mcp-0914-am-two-hosts.md
---

I have sat next to the admin-dashboard wall at 21:03 while Claude Code was *this close* to calling the wrong MCP tool.

Not a model failure. A **posture** failure.

Every framework ships tools. None ships a shared instant hub. That is the whole product.

Someone on the floor said, out loud, **paste tools-allow into the other session**. Claude Code had started earlier. The tool list was frozen in `config.toml` / `.mcp.json` / a skill file. The only “safe” move anyone trusted was:
...
That is the ceremony. **Paste the allow-list into the other chat. Two ceremonies, two lies.**

**The Aha:** One leaf. Both processes `get()` locally after one WebSocket lifetime. Real-time nested state is now a **primitive** — as native as a hashmap — via [Kiponos.io](https://kiponos.io). The host stays up. The leaf moves. The next tool call already obeys.

## The problem: Two agent hosts, one live posture
...
Paste the allow-list into the other chat. Two ceremonies, two lies.
...
Profile shape: `['my-app']['v1.0.0']['dev']['base']`. The leaf in this story is `session-posture` under `examples/agentic-mcp-0914-am-two-hosts/session-posture`.

## Architecture

```mermaid
flowchart LR
  wall[admin-dashboard wall] -->|set session-posture| hub[Kiponos hub]
  hub -->|delta WS| gb[Claude Code process]
  gb -->|get session-posture| tool[MCP / Skill / tool wrapper]
...
```yaml
examples/agentic-mcp-0914-am-two-hosts:
  session-posture: focus=tools,write=off
  schema-budget: "4000"
  result-cap: "2000"
  tool-search: "on"
  execute-gate: "plan"
...
Folder gate = Kiponos.createForCurrentTeam()
    .getFolder("examples/agentic-mcp-0914-am-two-hosts");
...
try:
    posture = k.get("examples/agentic-mcp-0914-am-two-hosts/session-posture", "live")
    if not str(posture):
        raise PermissionError("Two agent hosts, one live posture gated live — host not restarted")
finally:
    k.disconnect()
```
...
Runnable mesh: `https://github.com/kiponos-io/kiponos-io/tree/master/examples/java/agentic-mcp-0914-am-two-hosts` — `./gradlew test run`.
...
| User says “no writes” mid-turn | Restart Claude Code | Set `session-posture`; next call already denies |
...
`session-posture` rarely moves alone on the admin-dashboard wall. Pair it with `schema-budget`, `result-cap`, or `execute-gate` so you do not fix Two agent hosts, one live posture by inventing a second incident.

## Why Claude Code is the wrong restart target

Claude Code is good at calling tools. It is not a control plane. Killing it to flip `session-posture` teaches the on-call that judgment requires a process ID. The admin-dashboard wall already disagrees.
...
3. `cd examples/java/agentic-mcp-0914-am-two-hosts && cp kiponos.local.env.example kiponos.local.env`
4. `./gradlew test run` — prints `examples/agentic-mcp-0914-am-two-hosts/session-posture=...`
5. In the dashboard, change `session-posture`. Keep Claude Code up. No MCP reconnect.
6. Point the tool wrapper at the same leaf. Do not ship a new server binary to flip Two agent hosts, one live posture.
...
If flipping **Two agent hosts, one live posture** requires restarting Claude Code or reconnecting MCP, you do not have agentic control. You have a hope with a process ID.
...
How to try: `examples/java/agentic-mcp-0914-am-two-hosts` and `./gradlew test`.
