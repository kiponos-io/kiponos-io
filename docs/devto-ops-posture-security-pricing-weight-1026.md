---
title: "We Stopped Shipping Pricing / Promo Weight as a Constant (Security)"
published: false
...
Domain: WAF, RPS, auth leeway. This essay maps hub key `weight` to **pricing / promo weight** so the lesson stays concrete for security operators.
...
```python
policy = kiponos.path("examples", "ops-security-pricing-weight")
...
1. Name the hub path so humans find it under pressure (`examples/ops-security-pricing-weight/weight`).  
...
Feature flags are often product gates. This essay is about **ops posture on a hot path**: pricing / promo weight for security — numbers humans already change verbally in war rooms.
